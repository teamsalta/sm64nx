...
def SURFACE_IS_QUICKSAND(cmd):
	return (cmd >= 0x21 and cmd < 0x28)   

def SURFACE_IS_NOT_HARD(cmd):
	return(cmd != SURFACE_HARD and not (cmd >= 0x35 and cmd <= 0x37))
	
def SURFACE_IS_PAINTING_WARP(cmd):
...
def TERRAIN_LOAD_IS_SURFACE_TYPE_LOW(cmd):
	return (cmd < 0x40)
	
def TERRAIN_LOAD_IS_SURFACE_TYPE_HIGH(cmd):
	return (cmd >= 0x65)
...
def COL_VERTEX_INIT(vtxNum):
	return vtxNum


def COL_VERTEX(x, y, z):
	return [x, y, z]


def COL_TRI_INIT(surfType, triNum):
	return [surfType, triNum]


def COL_TRI(v1, v2, v3):
	return [v1, v2, v3]


def COL_TRI_SPECIAL(v1, v2, v3, param):
	return [v1, v2, v3, param]
...
def COL_SPECIAL_INIT(num):
	return [TERRAIN_LOAD_OBJECTS, num]


def COL_WATER_BOX_INIT(num):
	return [TERRAIN_LOAD_ENVIRONMENT, num]


def COL_WATER_BOX(id, x1, z1, x2, z2, y):
