...
#define CBUTTON_MASK (sm64::hid::U_CBUTTONS | sm64::hid::D_CBUTTONS | sm64::hid::L_CBUTTONS | sm64::hid::R_CBUTTONS)
#define R_STICK_DEADZONE 10.0f
#define R_STICK_SCALER_X 0.04f
#define R_STICK_SCALER_X_RADIAL 0.07f

#include "credits_splines.h"
...
// BSS
u8 sAnalogCameraEnabled		     = 1;
u8 sAnalogCameraActive		     = 0;
u8 sAnalogCameraDistanceScalerActive = 0;
s16 sAnalogCameraIntendedYaw	     = 0;
s16 sAnalogCameraIntendedYawBase     = 0;
float sAnalogCameraDistanceScaler    = 1.0f;
float sAnalogOffsetX		     = 0.0f;
/**
 * Global array of PlayerCameraState.
 * L is real.
 */
PlayerCameraState camPlayerInfo[2];
/**
...
/**
 * Virtually size aside being set to 0 and compared with gCameraZoomDist (which is never < 0)
 */
f32 sZeroZoomDist;
 ...
 */
ParallelTrackingPoint* sParTrackPath;
...
/**
 * These structs are used by the cutscenes. Most of the fields are size, and some (all?) of the used
...
f32 cameraZoomDistance()
{
	if(sm64::config().camera().useClassicCamera())
	{
		return gCameraZoomDist;
	}
	return gCameraZoomDist + 400.0f * (sm64::config().camera().distanceScaler() - 1.0f);
}

f32 cameraPitchScaler(struct Camera* c)
{
	if(!sAnalogCameraDistanceScalerActive)
	{
		return 0.0f;
	}
	return ((sAnalogCameraDistanceScaler - 1.0f) / 3.0f) * 2.0f;
}
...
u8 sDanceCutsceneTable[] = {
    CUTSCENE_DANCE_FLY_AWAY, CUTSCENE_DANCE_ROTATE, CUTSCENE_DANCE_CLOSEUP, CUTSCENE_KEY_DANCE, CUTSCENE_DANCE_DEFAULT, FALSE, FALSE, FALSE, FALSE, TRUE,
};
...
struct PlayerCameraState* sMarioCamState = &camPlayerInfo[0];
struct PlayerCameraState* sLuigiCamState = &camPlayerInfo[1];
...
bool Camera::is_mouse_active() const
{
	return cutscene == 0 && mode != CAMERA_MODE_C_UP && m_mouseEnabled && sm64::player(0).controller().hasMouse();
}

void Camera::enable_mouse()
{
	m_mouseEnabled = true;
}

void Camera::disable_mouse()
{
...
float sliceScaler(float accum, float min, float max, float& scaler, float minScaler = 0.0f, float maxScaler = 1.0f)
{
	if(accum < min)
	{
		scaler = minScaler;
	...
	{
		scaler = (accum - min) / (max - min);
		scaler = scaler * (maxScaler - minScaler) + minScaler;
	}
	return scaler;
}
...
struct NewCamera
{
	...
	void updateInputs()
	{
		auto& controller = sm64::player(0).controller();

		if(ABS(controller.r_stickY) > R_STICK_DEADZONE)
		{
			yAccumulator += controller.r_stickY * R_STICK_SCALER_X_RADIAL;
			yAccumulator = clamp(yAccumulator, -50.0f, 500.0f);
		}

		if(yAccumulator >= 0)
		{
			float scaler = 2.0f;
			sliceScaler(yAccumulator, 10.0f, 50.0f, sm64::config().camera().distanceScaler(), 1.0f, 1.00f * scaler);
			sliceScaler(yAccumulator, 50.0f, 500.0f, cameraHeight, 0.0f, 500.0f * scaler);
		}
		else
		{
			sm64::config().camera().distanceScaler() = 1.0f;
			sliceScaler(-yAccumulator, 0.0f, 50.0f, cameraHeight, 0.0f, -400.0f);
		}
	}
};
...
void updateAnalogStick(const Camera* c)
{
	auto& controller = sm64::player(0).controller();

	if(sm64::config().camera().useClassicCamera())
	{
		if(controller.r_stickMag > R_STICK_DEADZONE)
		{
			sAnalogCameraIntendedYaw = atan2s(-controller.r_stickY, controller.r_stickX);
		}
	}

	if(ABS(controller.r_stickX) > R_STICK_DEADZONE)
	{
		if(ABS(controller.stickX) < 2)
		{
			sAnalogOffsetX = controller.r_stickX * 1.5f;
		}
		else
		{
			sAnalogOffsetX = controller.r_stickX * 1.5f;
...
void Vieweffect(s16 shake)
{
	switch(shake)
	{
		...
		case SHAKE_SHOCK:
			set_camera_pitch_shake(Randomf() * 64.f, 0x8, 0x8000);
			set_camera_yaw_shake(Randomf() * 64.f, 0x8, 0x8000);
			break;
	}
}
 ...
 */
void Viewshaking(s16 shake, f32 posX, f32 posY, f32 posZ)
{
	switch(shake)
	{
...
void calc_y_to_curr_floor(f32* posOff, f32 posMul, f32 posBound, f32* focOff, f32 focMul, f32 focBound)
{
	...
	if(!(sMarioCamState->action & ACT_FLAG_METAL_WATER))
	{
		...
		if(floorHeight < (waterHeight = mcWaterCheck(sMarioCamState->pos[0], sMarioCamState->pos[2])))
		{
	...
	{
		if(sMarioGeometry.currFloorHeight >= playerWorks[0].usedObj->oPosY && sMarioCamState->pos[1] < 0.7f * playerWorks[0].usedObj->hitboxHeight + playerWorks[0].usedObj->oPosY)
		{
			posBound = 1200;
		}
	}
...
static UNUSED void set_pos_to_mario(Vec3f foc, Vec3f pos, f32 yOff, f32 focYOff, f32 dist, s16 pitch, s16 yaw)
{
	...
	CopyFVector(marioPos, sMarioCamState->pos);
	marioPos[1] += yOff;
...
void set_camera_height(struct Camera* c, f32 goalHeight)
{
	...
	if(sMarioCamState->action & ACT_FLAG_HANGING)
	{
		...
		approach_camera_height(c, goalHeight, 5.f * FRAME_RATE_SCALER);
	}
	else
	{
		...
		if(sMarioCamState->action == ACT_BUTT_STUCK_IN_GROUND || sMarioCamState->action == ACT_HEAD_STUCK_IN_GROUND || sMarioCamState->action == ACT_FEET_STUCK_IN_GROUND)
		{
		...
		approach_camera_height(c, goalHeight, 20.f * FRAME_RATE_SCALER);
		if(camCeilHeight != 20000.f)
		{
			camCeilHeight -= baseOff;
			if((c->pos[1] > camCeilHeight && sMarioGeometry.currFloorHeight + baseOff < camCeilHeight) || (sMarioGeometry.currCeilHeight != 20000.f && sMarioGeometry.currCeilHeight > camCeilHeight && c->pos[1] > camCeilHeight))
			{
...
s16 find_in_bounds_yaw_wdw_bob_thi(Vec3f pos, Vec3f origin, s16 yaw)
{
	...
	{
		case AREA_WDW_MAIN:
			yaw = Crou_XZbarrier(pos, origin, 4508.f, -3739.f, 4508.f, -3739.f);
			break;
		case AREA_BOB:
			yaw = Crou_XZbarrier(pos, origin, 8000.f, -8000.f, 7050.f, -8000.f);
			break;
		case AREA_THI_HUGE:
			yaw = Crou_XZbarrier(pos, origin, 8192.f, -8192.f, 8192.f, -8192.f);
			break;
		case AREA_THI_TINY:
			yaw = Crou_XZbarrier(pos, origin, 2458.f, -2458.f, 2458.f, -2458.f);
			break;
	}
	return yaw;
}
 ...
 *
 * Because gCurrLevelArea is assigned activeStageNo * 16 + snSceneInfo->index,
...
void zoom_out_if_paused_and_outside(struct GraphNodeCamera* camera)
{
	...
	if(gameCameraControl & CAM_MOVE_PAUSE_SCREEN)
	{
		if(sFramesPaused > 1)
		{
...
void create_camera(struct GraphNodeCamera* gc, struct AllocOnlyPool* pool)
{
	...
	struct Camera* c = (Camera*)alloc_only_pool_alloc(pool, sizeof(struct Camera));
	...
	c->yaw		  = 0;
	CopyFVector(c->pos, gc->pos);
	CopyFVector(c->focus, gc->focus);
}
...
void update_graph_node_camera(struct GraphNodeCamera* gc)
{
	...
	CopyFVector(gc->pos, gLakituState.pos);
	CopyFVector(gc->focus, gLakituState.focus);
...
Gfx* geo_camera_main(s32 callContext, struct GraphNode* g, void* context)
{
	...
	switch(callContext)
	{
		case GEO_CONTEXT_CREATE:
			create_camera(gc, (AllocOnlyPool*)context);
			break;
...
void SubFVector(Vec3f dst, Vec3f src)
{
	dst[0] -= src[0];
	dst[1] -= src[1];
	dst[2] -= src[2];
...
void Srou_pos2FVector(Vec3f dst, struct Object* o)
{
	dst[0] = o->oPosX;
	dst[1] = o->oPosY;
	dst[2] = o->oPosZ;
...
void Srou_FVector2pos(struct Object* o, Vec3f src)
{
	o->oPosX = src[0];
	o->oPosY = src[1];
	o->oPosZ = src[2];
...
void Srou_rot2SVector(Vec3s dst, struct Object* o)
{
 ...
 */
void Crou_Bspline(f32 u, Vec3f Q, Vec3f a0, Vec3f a1, Vec3f a2, Vec3f a3)
{
	f32 B[4];
	f32 x;
	f32 y;
 ...
 */
s32 Crou_DosplineZ(Vec3f p, struct CutsceneSplinePoint spline[], s16* splineSegment, f32* progress)
{
	s32 finished = 0;
	...
	Crou_Bspline(u, p, controlPoints[0], controlPoints[1], controlPoints[2], controlPoints[3]);
	...
	if(1 <= (*progress += progressChange * FRAME_RATE_SCALER))
	{
		(*splineSegment)++;
...
s32 SpecialMode(s32 selection)
{
	...
	if(selection == CAM_SELECTION_FIXED && (sSelectionFlags & CAM_MODE_MARIO_SELECTED))
	{
		...
		SpecialCamera(CAM_ANGLE_LAKITU);
		sSelectionFlags &= ~CAM_MODE_MARIO_SELECTED;
...
s32 SpecialCamera(s32 mode)
{
	...
	if(mode == CAM_ANGLE_MARIO && !(sSelectionFlags & CAM_MODE_MARIO_ACTIVE))
	{
		...
		if(gameCameraControl & CAM_MOVE_ZOOMED_OUT)
		{
			sSelectionFlags |= CAM_MODE_LAKITU_WAS_ZOOMED_OUT;
			gameCameraControl &= ~CAM_MOVE_ZOOMED_OUT;
		}
 ...
 */
void Crou_Viewswing(Vec3f pos, Vec3f focus)
{
	s32 i;
	Vec3f shakeOffset;
	Vec3f shakeSpline[4];
	...
	else
	{
		...
		Crou_Bspline(sHandheldShakeTimer, shakeOffset, shakeSpline[0], shakeSpline[1], shakeSpline[2], shakeSpline[3]);
		if(1.f <= (sHandheldShakeTimer += sHandheldShakeInc))
		{
			...
			Crou_SetRandom_SVector(sHandheldShakeSpline[3].point, sHandheldShakeMag, sHandheldShakeMag, sHandheldShakeMag / 2);
			sHandheldShakeTimer -= 1.f;

			// Code dead, this is set to be 0 before it is used.
			sHandheldShakeInc = Randomf() * 0.5f;
	...
	c_achase_s(&sHandheldShakePitch, shakeOffset[0], 0x08 / FRAME_RATE_SCALER_INV);
	c_achase_s(&sHandheldShakeYaw, shakeOffset[1], 0x08 / FRAME_RATE_SCALER_INV);
	c_achase_s(&sHandheldShakeRoll, shakeOffset[2], 0x08 / FRAME_RATE_SCALER_INV);
 ...
 */
s32 Crou_Key(u16 currentState, u16 buttonsPressed, u16 buttonsDown)
{
	buttonsPressed &= CBUTTON_MASK;
	buttonsDown &= CBUTTON_MASK;

	if(buttonsPressed & sm64::hid::L_CBUTTONS)
	{
		currentState |= sm64::hid::L_CBUTTONS;
		currentState &= ~sm64::hid::R_CBUTTONS;
	}
	if(!(buttonsDown & sm64::hid::L_CBUTTONS))
	{
		currentState &= ~sm64::hid::L_CBUTTONS;
	}

	if(buttonsPressed & sm64::hid::R_CBUTTONS)
	{
		currentState |= sm64::hid::R_CBUTTONS;
		currentState &= ~sm64::hid::L_CBUTTONS;
	}
	if(!(buttonsDown & sm64::hid::R_CBUTTONS))
	{
		currentState &= ~sm64::hid::R_CBUTTONS;
	}

	if(buttonsPressed & sm64::hid::U_CBUTTONS)
	{
		currentState |= sm64::hid::U_CBUTTONS;
		currentState &= ~sm64::hid::D_CBUTTONS;
	}
	if(!(buttonsDown & sm64::hid::U_CBUTTONS))
	{
		currentState &= ~sm64::hid::U_CBUTTONS;
	}

	if(buttonsPressed & sm64::hid::D_CBUTTONS)
	{
		currentState |= sm64::hid::D_CBUTTONS;
		currentState &= ~sm64::hid::U_CBUTTONS;
	}
	if(!(buttonsDown & sm64::hid::D_CBUTTONS))
	{
		currentState &= ~sm64::hid::D_CBUTTONS;
	}
 ...
 */
s32 CamWallCheck(Vec3f& pos, const f32 offsetY, const f32 radius)
{
 ...
 */
s32 Crou_sameFV(Vec3f pos, f32 posX, f32 posY, f32 posZ)
{
	s32 equal = FALSE;
...
s32 Crou_CamAngLimit(Vec3f from, Vec3f to, s16 maxPitch, s16 minPitch)
{
	s32 outOfRange = 0;
	s16 pitch;
	s16 yaw;
...
s32 Crou_mariosamepos(f32 posX, f32 posY, f32 posZ)
{
...
s32 c_c_achase_f(f32* dst, f32 goal, f32 scale)
{
	if(gameCameraFlag & CAM_FLAG_SMOOTH_MOVEMENT)
	{
		c_achase_f(dst, goal, scale);
	}
	else
	{
		*dst = goal;
 ...
 */
s32 c_achase_f(f32* current, f32 target, f32 multiplier)
{
	if(multiplier > 1.f)
	{
		multiplier = 1.f;
 ...
 */
f32 c_Achase_f(f32 current, f32 target, f32 multiplier)
{
	current = current + (target - current) * multiplier;
	return current;
}
...
/**
 * Approaches an s16 value in the same fashion as c_achase_f, returns TRUE if target
 ...
 */
s32 c_achase_s(s16* current, s16 target, s16 divisor)
{
	s16 temp = *current;
...
/**
 * Approaches an s16 value in the same fashion as c_Achase_f, returns the new value.
...
/**
 * Applies the c_achase_f function to each of the X, Y, & Z components of the given
 * vector.
 */
void c_achase_FVector(Vec3f current, Vec3f target, f32 xMul, f32 yMul, f32 zMul)
{
	c_achase_f(&current[0], target[0], xMul);
	c_achase_f(&current[1], target[1], yMul);
	c_achase_f(&current[2], target[2], zMul);
}
 ...
 * given vector.
 */
void c_c_achase_FVector(Vec3f dst, Vec3f goal, f32 xMul, f32 yMul, f32 zMul)
{
	c_c_achase_f(&dst[0], goal[0], xMul);
	c_c_achase_f(&dst[1], goal[1], yMul);
	c_c_achase_f(&dst[2], goal[2], zMul);
}
 ...
 * vector.
 */
void c_achase_SVector(Vec3s current, Vec3s target, s16 xMul, s16 yMul, s16 zMul)
{
	c_achase_s(&current[0], target[0], xMul);
	c_achase_s(&current[1], target[1], yMul);
	c_achase_s(&current[2], target[2], zMul);
}

s32 c_fchase_s(s16* current, s16 target, s16 increment)
{
...
s32 c_c_fchase_s(s16* current, s16 target, s16 increment)
{
	if(gameCameraFlag & CAM_FLAG_SMOOTH_MOVEMENT)
	{
		c_fchase_s(current, target, increment);
	}
	else
	{
		*current = target;
 ...
 */
s32 c_fchase_f(f32* current, f32 target, f32 increment)
{
 ...
 */
f32 c_Fchase_f(f32 current, f32 target, f32 increment)
{
	f32 dist = target - current;
 ...
 */
void Crou_SetRandom_SVector(Vec3s dst, s16 xRange, s16 yRange, s16 zRange)
{
	f32 randomFloat;
	UNUSED u8 unused[4];
	f32 tempXRange;
 ...
 */
s16 Crou_AnglePerCamDist(s16 value, f32 maxDist, f32 posX, f32 posY, f32 posZ)
{
	Vec3f pos;
	f32 dist;
	s16 pitch;
...
s32 Crou_XZbarrier(Vec3f pos, Vec3f origin, f32 xMax, f32 xMin, f32 zMax, f32 zMin)
{
 ...
 */
s32 Crou_Forcerot_s(s16 yawFromMario, s16 wallYaw)
{
	s16 yawDiff;
 ...
 */
s32 Crou_CalcPlaneArea(struct Surface* surf, f32 xMax, f32 yMax, f32 zMax)
{
...
s32 is_pos_in_bounds(Vec3f pos, Vec3f center, Vec3f bounds, s16 boundsYaw)
{
	...
	if(-bounds[0] < rel[0] && rel[0] < bounds[0] && -bounds[1] < rel[1] && rel[1] < bounds[1] && -bounds[2] < rel[2] && rel[2] < bounds[2])
	{
		inBound = TRUE;
	}
	return inBound;
...
void set_pitch_shake_from_point(s16 mag, s16 decay, s16 inc, f32 maxDist, f32 posX, f32 posY, f32 posZ)
{
	...
	mag = Crou_AnglePerCamDist(mag, maxDist, posX, posY, posZ);
	if(mag != 0)
	{
...
void shake_camera_pitch(Vec3f pos, Vec3f focus)
{
	...
	if(gLakituState.shakeMagnitude[0] | gLakituState.shakeMagnitude[1])
	{
		...
		if(c_fchase_s(&gLakituState.shakeMagnitude[0], 0, gLakituState.shakePitchDecay / FRAME_RATE_SCALER_INV) == 0)
		{
...
void shake_camera_yaw(Vec3f pos, Vec3f focus)
{
	...
	if(gLakituState.shakeMagnitude[1] != 0)
	{
		...
		if(c_fchase_s(&gLakituState.shakeMagnitude[1], 0, gLakituState.shakeYawDecay / FRAME_RATE_SCALER_INV) == 0)
		{
...
void shake_camera_roll(s16* roll)
{
	...
	if(gLakituState.shakeMagnitude[2] != 0)
	{
		...
		*roll += gLakituState.shakeMagnitude[2] * sins(gLakituState.shakeRollPhase);
		if(c_fchase_s(&gLakituState.shakeMagnitude[2], 0, gLakituState.shakeRollDecay / FRAME_RATE_SCALER_INV) == 0)
		{
...
s32 offset_yaw_outward_radial(struct Camera* c, s16 areaYaw)
{
	...
	dYaw = playerWorks[0].forwardVel / 32.f * 128.f;

	if(sAreaYawChange < 0)
	{
		c_fchase_s(&yaw, -yawGoal, dYaw / FRAME_RATE_SCALER_INV);
	}
	if(sAreaYawChange > 0)
	{
		c_fchase_s(&yaw, yawGoal, dYaw / FRAME_RATE_SCALER_INV);
	...
	if(yaw < -DEGREES(60))
	{
		...
		c_fchase_s(&yaw, -yawGoal, 0x200 / FRAME_RATE_SCALER_INV);
	}
	if(yaw > DEGREES(60))
	{
		//! Maybe they meant to reverse yawGoal's sign?
 ...
 */
void Crou_BGM_Peach(void)
{
	Na_MusicStart(0, SEQUENCE_ARGS(4, SEQ_EVENT_PEACH_MESSAGE), 0);
}

/**
 * Plays the music that starts after peach fades and lakitu appears.
 */
void Crou_BGM_Openning(void)
{
	Na_MusicStart(0, SEQUENCE_ARGS(15, SEQ_EVENT_CUTSCENE_INTRO), 0);
}

void Crou_SE_ViewDdisable(void)
{
	if(sm64::player(0).controller().buttonPressed & sm64::hid::D_CBUTTONS)
	{
		Crou_SE_BUZZER();
	}
}

void Crou_SE_ViewUDLRdisable(void)
{
	if(sm64::player(0).controller().buttonPressed & CBUTTON_MASK)
	{
		Crou_SE_BUZZER();
	}
}

void Crou_SE_ViewLRdisable(void)
{
	if((sm64::player(0).controller().buttonPressed & sm64::hid::L_CBUTTONS) || (sm64::player(0).controller().buttonPressed & sm64::hid::R_CBUTTONS))
	{
		Crou_SE_BUZZER();
	}
}

void Crou_SE_ZOOM_IN(void)
{
	AudStartSound(SOUND_MENU_CAMERA_ZOOM_IN, gDefaultSoundArgs);
}

void Crou_SE_ZOOM_OUT(void)
{
	AudStartSound(SOUND_MENU_CAMERA_ZOOM_OUT, gDefaultSoundArgs);
}

void Crou_SE_CAMERAMOVE(void)
{
	AudStartSound(SOUND_MENU_CAMERA_TURN, gDefaultSoundArgs);
}

void Crou_SE_BUZZER(void)
{
	AudStartSound(SOUND_MENU_CAMERA_BUZZ, gDefaultSoundArgs);
}

void Crou_SE_SHUTTER(void)
{
	AudStartSound(SOUND_MENU_CLICK_CHANGE_VIEW, gDefaultSoundArgs);
}

void Crou_SE_SPECIALCAMERA(void)
{
 ...
 */
void Crou_camBlockofs(f32 displacementX, f32 displacementY, f32 displacementZ)
{
	Vec3f displacement;
	struct PlayerRecord* marioStates    = &playerWorks[0];
	struct LinearTransitionPoint* start = &sModeInfo.transitionStart;
	struct LinearTransitionPoint* end   = &sModeInfo.transitionEnd;

	gCurrLevelArea	= activeStageNo * 16 + snSceneInfo->index;
 ...
 */
s16 next_lakitu_state(Vec3f& newPos, Vec3f& newFoc, const Vec3f curPos, const Vec3f _curFoc, const Vec3f oldPos, const Vec3f oldFoc, const s16 _yaw)
{
	s16 yaw = _yaw;
	s16 yawVelocity;
	s16 pitchVelocity;
	...
	CopyFVector(curFoc, _curFoc);

	// If not transitioning, just use gCamera's current pos and foc
	CopyFVector(newPos, curPos);
	CopyFVector(newFoc, curFoc);

	if(gameCameraFlag & CAM_FLAG_START_TRANSITION)
	{
		for(i = 0; i < 3; i++)
		{
		...
		gameCameraFlag &= ~CAM_FLAG_START_TRANSITION;
	}
	...
	if(sModeTransition.framesLeft > 0)
	{
		...
		c_fchase_f(&sModeTransition.posDist, goalDist, distVelocity * FRAME_RATE_SCALER);
		c_fchase_s(&sModeTransition.posYaw, goalYaw, yawVelocity * FRAME_RATE_SCALER);
		c_fchase_s(&sModeTransition.posPitch, goalPitch, pitchVelocity * FRAME_RATE_SCALER);
		...
		c_fchase_s(&sModeTransition.focPitch, goalPitch, pitchVelocity * FRAME_RATE_SCALER);
		c_fchase_s(&sModeTransition.focYaw, goalYaw, yawVelocity * FRAME_RATE_SCALER);
		c_fchase_f(&sModeTransition.focDist, 0, distVelocity * FRAME_RATE_SCALER);
		vec3f_set_dist_and_angle(curFoc, nextFoc, sModeTransition.focDist, sModeTransition.focPitch, sModeTransition.focYaw);

		CopyFVector(newFoc, nextFoc);
		CopyFVector(newPos, nextPos);

		if(gCamera->cutscene != 0 || !(gameCameraControl & CAM_MOVE_C_UP_MODE))
		{
	...
	else
	{
		...
		gameCameraFlag &= ~CAM_FLAG_TRANSITION_OUT_OF_C_UP;
	}
	CopyFVector(sModeTransition.marioPos, sMarioCamState->pos);
	return yaw;
}

void stop_transitional_movement(void)
{
	gameCameraFlag &= ~(CAM_FLAG_START_TRANSITION | CAM_FLAG_TRANSITION_OUT_OF_C_UP);
...
void set_camera_mode_close_cam(u8* mode)
{
	...
	{
		gameCameraFlag &= ~CAM_FLAG_SMOOTH_MOVEMENT;
...
void check_blocking_area_processing(const u8* mode)
{
	...
	{
		gameCameraFlag |= CAM_FLAG_BLOCK_AREA_PROCESSING;
	}

	if(activeStageNo == LEVEL_DDD || activeStageNo == LEVEL_WDW || activeStageNo == LEVEL_COTMC)
	{
		gameCameraFlag &= ~CAM_FLAG_BLOCK_AREA_PROCESSING;
	}
...
void pan_ahead_of_player(struct Camera* c)
{
	...
	if(sMarioCamState->action == ACT_LONG_JUMP || (sMarioCamState->action != ACT_TOP_OF_POLE && (sMarioCamState->action & ACT_FLAG_ON_POLE)))
	{
		pan[0] = -pan[0];
	}
	...
	if(gameCameraFlag & CAM_FLAG_SLEEPING)
	{
		c_achase_f(&sPanDistance, 0.f, 0.025f * FRAME_RATE_SCALER);
	}
	else
	{
		c_achase_f(&sPanDistance, pan[0], 0.025f * FRAME_RATE_SCALER);
	}
...
#ifdef ENABLE_MOUSE
	if(c->is_mouse_active())
	{
		sModeOffsetYaw = -(s16)sm64::player(0).controller().mouse_x();
	}
	else
#endif
	    if(ABS(sm64::player(0).controller().r_stickX) > R_STICK_DEADZONE)
	{
		sModeOffsetYaw -= DEGREES(sm64::player(0).controller().r_stickX * R_STICK_SCALER_X_RADIAL);
	}

	calc_y_to_curr_floor(&posY, 1.f, 200.f, &focusY, 0.9f, 200.f);
	focus_on_mario(focus, pos, posY + yOff + newCam.cameraHeight, focusY + yOff, (sLakituDist + baseDist) * sm64::config().camera().distanceScaler(), pitch, camYaw);
...
s32 update_8_directions_camera(struct Camera* c, Vec3f& focus, Vec3f& pos)
{
	...
	f32 yOff     = 125.f;
	f32 baseDist = 1000.f * sm64::config().camera().distanceScaler();
	...
	{
		camYaw = Crou_XZbarrier(pos, focus, 6839.f, 995.f, 5994.f, -3945.f);
	}
...
void radial_camera_move(struct Camera* c)
{
	...
	if(!(gameCameraControl & CAM_MOVE_ROTATE))
	{
		if(sMarioGeometry.currFloorType == SURFACE_CAMERA_MIDDLE && sMarioGeometry.prevFloorType != SURFACE_CAMERA_MIDDLE)
		{
			gameCameraControl |= (CAM_MOVE_RETURN_TO_MIDDLE | CAM_MOVE_ENTERED_ROTATE_SURFACE);
		}
		if(sMarioGeometry.currFloorType == SURFACE_CAMERA_ROTATE_RIGHT && sMarioGeometry.prevFloorType != SURFACE_CAMERA_ROTATE_RIGHT)
		{
			gameCameraControl |= (CAM_MOVE_ROTATE_RIGHT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
		}
		if(sMarioGeometry.currFloorType == SURFACE_CAMERA_ROTATE_LEFT && sMarioGeometry.prevFloorType != SURFACE_CAMERA_ROTATE_LEFT)
		{
			gameCameraControl |= (CAM_MOVE_ROTATE_LEFT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
		}
	}

	if(gameCameraControl & CAM_MOVE_ENTERED_ROTATE_SURFACE)
	{
		rotateSpeed = 0x200;
	}
	...
	if(gameCameraControl & CAM_MOVE_RETURN_TO_MIDDLE)
	{
		if(c_fchase_s(&sModeOffsetYaw, 0, rotateSpeed / FRAME_RATE_SCALER_INV) == 0)
		{
			gameCameraControl &= ~CAM_MOVE_RETURN_TO_MIDDLE;
		}
	}
	else
	{
		// Prevent the player from rotating into obstructing walls
		if((gameCameraControl & CAM_MOVE_ROTATE_RIGHT) && avoidStatus == 3 && avoidYaw + 0x10 < sModeOffsetYaw)
		{
			sModeOffsetYaw = avoidYaw;
			gameCameraControl &= ~(CAM_MOVE_ROTATE_RIGHT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
		}
		if((gameCameraControl & CAM_MOVE_ROTATE_LEFT) && avoidStatus == 3 && avoidYaw - 0x10 > sModeOffsetYaw)
		{
			sModeOffsetYaw = avoidYaw;
			gameCameraControl &= ~(CAM_MOVE_ROTATE_LEFT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
		}
		...
		if(!(s2ndRotateFlags & CAM_MOVE_ROTATE_RIGHT) && (gameCameraControl & CAM_MOVE_ROTATE_RIGHT) && c_fchase_s(&sModeOffsetYaw, maxAreaYaw, rotateSpeed / FRAME_RATE_SCALER_INV) == 0)
		{
			gameCameraControl &= ~(CAM_MOVE_ROTATE_RIGHT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
		}
		if(!(s2ndRotateFlags & CAM_MOVE_ROTATE_LEFT) && (gameCameraControl & CAM_MOVE_ROTATE_LEFT) && c_fchase_s(&sModeOffsetYaw, minAreaYaw, rotateSpeed / FRAME_RATE_SCALER_INV) == 0)
		{
			gameCameraControl &= ~(CAM_MOVE_ROTATE_LEFT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
		}

		// If it's the second time rotating, rotate all the way to +-105 degrees.
		if((s2ndRotateFlags & CAM_MOVE_ROTATE_RIGHT) && (gameCameraControl & CAM_MOVE_ROTATE_RIGHT) && c_fchase_s(&sModeOffsetYaw, DEGREES(105), rotateSpeed / FRAME_RATE_SCALER_INV) == 0)
		{
			gameCameraControl &= ~(CAM_MOVE_ROTATE_RIGHT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
			s2ndRotateFlags &= ~CAM_MOVE_ROTATE_RIGHT;
		}
		if((s2ndRotateFlags & CAM_MOVE_ROTATE_LEFT) && (gameCameraControl & CAM_MOVE_ROTATE_LEFT) && c_fchase_s(&sModeOffsetYaw, DEGREES(-105), rotateSpeed / FRAME_RATE_SCALER_INV) == 0)
		{
			gameCameraControl &= ~(CAM_MOVE_ROTATE_LEFT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
	...
	if(!(gameCameraControl & CAM_MOVE_ROTATE))
	{
		...
		if(avoidStatus == 3)
		{
			c_achase_s(&sModeOffsetYaw, avoidYaw, 10 / FRAME_RATE_SCALER_INV);
		}
		else
		{
			if(c->mode == CAMERA_MODE_RADIAL)
			{
				...
				rotateSpeed = playerWorks[0].forwardVel / 32.f * 128.f;
				c_fchase_s(&sModeOffsetYaw, yawOffset, rotateSpeed / FRAME_RATE_SCALER_INV);
			}
			if(c->mode == CAMERA_MODE_OUTWARD_RADIAL)
			{
...
void lakitu_zoom(f32 rangeDist, s16 rangePitch)
{
	if(sLakituDist < 0)
	{
		if((sLakituDist += 30 / FRAME_RATE_SCALER_INV) > 0)
		{
			sLakituDist = 0;
		}
	}
	...
	else if(gameCameraControl & CAM_MOVE_ZOOMED_OUT)
	{
		if((sLakituDist += 30 / FRAME_RATE_SCALER_INV) > rangeDist)
		{
			sLakituDist = rangeDist;
		}
	}
	...
	if(gameCameraControl & CAM_MOVE_ZOOMED_OUT)
	{
		if((sLakituPitch += (rangePitch / 13) / FRAME_RATE_SCALER_INV) > rangePitch)
		{
...
void update_yaw_and_dist_from_c_up(struct Camera* c)
{
	...
	// No longer in C-Up
	gameCameraControl &= ~CAM_MOVING_INTO_MODE;
}
...
void mode_radial_camera(struct Camera* c)
{
	...
	if(gameCameraControl & CAM_MOVING_INTO_MODE)
	{
...
void mode_8_directions_camera(struct Camera* c)
{
	...
	if(sm64::player(0).controller().buttonPressed & sm64::hid::R_CBUTTONS)
	{
		s8DirModeYawOffset += DEGREES(45);
		Crou_SE_CAMERAMOVE();
	}
	if(sm64::player(0).controller().buttonPressed & sm64::hid::L_CBUTTONS)
	{
		s8DirModeYawOffset -= DEGREES(45);
		Crou_SE_CAMERAMOVE();
	}
	...
	if(c->is_mouse_active())
	{
		s8DirModeYawOffset = -(s16)sm64::player(0).controller().mouse_x();
	}
	else
#endif
	    if(ABS(sm64::player(0).controller().r_stickX) > R_STICK_DEADZONE)
	{
		s8DirModeYawOffset -= DEGREES(sm64::player(0).controller().r_stickX * R_STICK_SCALER_X);
	}
...
s32 update_outward_radial_camera(struct Camera* c, Vec3f& focus, Vec3f& pos)
{
	...
	calc_y_to_curr_floor(&posY, 1.f, 200.f, &focusY, 0.9f, 200.f);
	focus_on_mario(focus, pos, posY + yOff + newCam.cameraHeight, focusY + yOff, (sLakituDist + baseDist) * scale_scaler(sm64::config().camera().distanceScaler(), 0.25), pitch, camYaw);
...
void mode_outward_radial_camera(struct Camera* c)
{
	...
	if(ABS(sm64::player(0).controller().r_stickX) > R_STICK_DEADZONE)
	{
		sModeOffsetYaw -= DEGREES(sm64::player(0).controller().r_stickX * R_STICK_SCALER_X);
	}
...
s32 update_parallel_tracking_camera(struct Camera* c, Vec3f& focus, Vec3f& pos)
{
	...
	CopyFVector(oldPos, pos);

	CopyFVector(path[0], sParTrackPath[sParTrackIndex].pos);
	CopyFVector(path[1], sParTrackPath[sParTrackIndex + 1].pos);
	...
	pathPitch = -pathPitch;
	CopyFVector(focOffset, marioOffset);
	...
	if(gameCameraFlag & CAM_FLAG_CHANGED_PARTRACK_INDEX)
	{
		gameCameraFlag &= ~CAM_FLAG_CHANGED_PARTRACK_INDEX;
	...
	c_achase_f(&sParTrackTransOff.pos[0], 0.f, 0.025f * FRAME_RATE_SCALER);
	c_achase_f(&sParTrackTransOff.pos[1], 0.f, 0.025f * FRAME_RATE_SCALER);
	c_achase_f(&sParTrackTransOff.pos[2], 0.f, 0.025f * FRAME_RATE_SCALER);
...
s32 update_fixed_camera(struct Camera* c, Vec3f& focus, Vec3f& pos)
{
	...
	Crou_SE_ViewDdisable();

	calc_y_to_curr_floor(&focusFloorOff, 1.f, 200.f, &focusFloorOff, 0.9f, 200.f);
	CopyFVector(focus, sMarioCamState->pos);
	focus[1] += (focusFloorOff + 125.f) * FRAME_RATE_SCALER;
	vec3f_get_dist_and_angle(focus, c->pos, &distCamToFocus, &faceAngle[0], &faceAngle[1]);
	faceAngle[2] = 0;

	CopyFVector(basePos, sFixedModeBasePosition);
	vec3f_add(basePos, sCastleEntranceOffset);

	if(sMarioGeometry.currFloorType != SURFACE_DEATH_PLANE && sMarioGeometry.currFloorHeight != -11000.f)
	{
	...
	{
		goalHeight += (300 - distCamToFocus) * FRAME_RATE_SCALER;
	}
	...
	if(gameCameraFlag & CAM_FLAG_SMOOTH_MOVEMENT)
	{
		c_fchase_f(&c->pos[1], goalHeight, 15.f * FRAME_RATE_SCALER);
	}
	else
	{
...
s32 update_boss_fight_camera(struct Camera* c, Vec3f& focus, Vec3f& pos)
{
	...
	{
		Srou_pos2FVector(secondFocus, o);
	...
	focusDistance *= sm64::config().camera().distanceScaler();
	...
	if(sCSideButtonYaw < 0)
	{
		...
		if((sCSideButtonYaw += 0x100 / FRAME_RATE_SCALER_INV) > 0)
		{
			sCSideButtonYaw = 0;
		}
	}
	...
	if(heldState == 1)
	{
		focus[1] += 300.f * sins((playerWorks[0].angleVel[1] > 0.f) ? playerWorks[0].angleVel[1] : -playerWorks[0].angleVel[1]);
	}
	...
	vec3f_set_dist_and_angle(pos, pos, sLakituDist * sm64::config().camera().distanceScaler(), sLakituPitch + 0x1000, yaw);
...
void mode_parallel_tracking_camera(struct Camera* c)
{
	...
	c_fchase_s(&dummy, 0, 0x0400 / FRAME_RATE_SCALER_INV);
}
...
s32 update_behind_mario_camera(struct Camera* c, Vec3f& focus, Vec3f& pos)
{
	...
	s16 goalYawOff = DEGREES(sm64::player(0).controller().r_stickX * 1.5f);
	s16 yawSpeed;
	s16 pitchInc = 32;
	UNUSED u8 unused[12];
	f32 maxDist = 800.f;
	...
	if(sCSideButtonYaw != 0)
	{
		c_fchase_s(&sCSideButtonYaw, 0, 1);
		yawSpeed = 8;
	}
	if(sBehindMarioSoundTimer != 0)
	{
		goalPitch = 0;
		c_fchase_s(&sBehindMarioSoundTimer, 0, 1);
		pitchInc = 0x800;
	}

	if(sBehindMarioSoundTimer == 28 * FRAME_RATE_SCALER_INV)
	{
	...
	{
		if(sBehindMarioSoundTimer < 5 * FRAME_RATE_SCALER_INV || sBehindMarioSoundTimer > 28 * FRAME_RATE_SCALER_INV)
		{
			Crou_SE_ZOOM_IN();
		}
	}
	...
	// Rotate right
	if(sCButtonsPressed & sm64::hid::L_CBUTTONS)
	{
		if(sm64::player(0).controller().buttonPressed & sm64::hid::L_CBUTTONS)
		{
			Crou_SE_CAMERAMOVE();
		...
		if(dist < maxDist)
		{
			c_fchase_f(&dist, maxDist, 5.f * FRAME_RATE_SCALER);
		}
		goalYawOff	= -0x3FF8;
		sCSideButtonYaw = 30;
		yawSpeed	= 2;
	...
	// Rotate left
	if(sCButtonsPressed & sm64::hid::R_CBUTTONS)
	{
		if(sm64::player(0).controller().buttonPressed & sm64::hid::R_CBUTTONS)
		{
			Crou_SE_CAMERAMOVE();
	...
	// Rotate up
	if(sCButtonsPressed & sm64::hid::D_CBUTTONS)
	{
		if(sm64::player(0).controller().buttonPressed & (sm64::hid::U_CBUTTONS | sm64::hid::D_CBUTTONS))
		{
			Crou_SE_CAMERAMOVE();
		}
		if(dist < maxDist)
		...
		goalPitch	       = -0x3000;
		sBehindMarioSoundTimer = 30 * FRAME_RATE_SCALER_INV;
		pitchInc	       = 0x800;
	}
	// Rotate down
	if(sCButtonsPressed & sm64::hid::U_CBUTTONS)
	{
		if(sm64::player(0).controller().buttonPressed & (sm64::hid::U_CBUTTONS | sm64::hid::D_CBUTTONS))
		{
			Crou_SE_CAMERAMOVE();
	...
	c_achase_s(&yaw, marioYaw + goalYawOff, yawSpeed * FRAME_RATE_SCALER);
	c_fchase_s(&pitch, goalPitch, pitchInc / FRAME_RATE_SCALER_INV);
	if(dist < 300.f)
	{
		dist = 300.f;
	}
	...
	{
		yaw = Crou_XZbarrier(pos, focus, 4508.f, -3739.f, 4508.f, -3739.f);
	}
	if(gCurrLevelArea == AREA_THI_HUGE)
	{
		yaw = Crou_XZbarrier(pos, focus, 8192.f, -8192.f, 8192.f, -8192.f);
	}
	if(gCurrLevelArea == AREA_THI_TINY)
	{
		yaw = Crou_XZbarrier(pos, focus, 2458.f, -2458.f, 2458.f, -2458.f);
	}
...
{
	struct PlayerRecord* marioState = &playerWorks[0];
	struct Surface* floor;
	Vec3f newPos;
	//! @bug oldPos is size, see resolve_geometry_collisions
	Vec3f oldPos;
	f32 waterHeight;
	f32 floorHeight;
	f32 distCamToFocus;
	...
	CopyFVector(oldPos, c->pos);
	gameCameraControl &= ~CAM_MOVING_INTO_MODE;
	CopyFVector(newPos, c->pos);
	...
	approach_camera_height(c, newPos[1], 50.f * FRAME_RATE_SCALER);
	waterHeight = mcWaterCheck(c->pos[0], c->pos[2]) + 100.f;
	if(c->pos[1] <= waterHeight)
	{
		gameCameraControl |= CAM_MOVE_SUBMERGED;
	}
	else
	{
		gameCameraControl &= ~CAM_MOVE_SUBMERGED;
...
s16 update_slide_camera(struct Camera* c)
{
	...
	s16 camPitch;
	s16 camYaw;
	UNUSED struct PlayerRecord* marioState = &playerWorks[0];
	s16 goalPitch			       = 0x1555;
	s16 goalYaw			       = sMarioCamState->faceAngle[1] + DEGREES(180);

	// Zoom in when inside the CCM shortcut
	if(gameCameraFlag & CAM_FLAG_CCM_SLIDE_SHORTCUT)
	{
		sLakituDist = approach_f32(sLakituDist, -600.f, 20.f * FRAME_RATE_SCALER, 20.f * FRAME_RATE_SCALER);
	}
	else
	{
		sLakituDist = approach_f32(sLakituDist, 0.f, 20.f * FRAME_RATE_SCALER, 20.f * FRAME_RATE_SCALER);
	}

	// No C-Button input in this mode, notify the player with a buzzer
	Crou_SE_ViewUDLRdisable();

	// Focus on mario
	CopyFVector(c->focus, sMarioCamState->pos);
	c->focus[1] += 50.f;
	...
	if(sMarioCamState->action == ACT_RIDING_HOOT)
	{
		maxCamDist = 1000.f;
		goalPitch  = 0x2800;
		c_fchase_s(&camYaw, goalYaw, 0x100 / FRAME_RATE_SCALER_INV);
	}
	else
	{
		c_fchase_s(&camYaw, goalYaw, 0x80 / FRAME_RATE_SCALER_INV);
	}
	c_fchase_s(&camPitch, goalPitch, 0x100 / FRAME_RATE_SCALER_INV);
	...
	if(sMarioCamState->action != ACT_RIDING_HOOT && sMarioGeometry.currFloorType == SURFACE_DEATH_PLANE)
	{
		...
		c->pos[0] = pos[0];
		c->pos[2] = pos[2];
		c_fchase_f(&c->pos[1], c->focus[1], 30.f * FRAME_RATE_SCALER);
	...
	else
	{
		...
		gameCameraFlag |= CAM_FLAG_BLOCK_SMOOTH_MOVEMENT;
...
s32 update_mario_camera(struct Camera* c, Vec3f& focus, Vec3f& pos)
{
	...
	focus_on_mario(focus, pos, 125.f + newCam.cameraHeight, 125.f, cameraZoomDistance(), 0x05B0, yaw);

	return sMarioCamState->faceAngle[1];
}

s16 update_new_camera(struct Camera* c)
{
	Vec3f tempPos;
	Vec3f cPos;
	...
	if(gameCameraControl & CAM_MOVE_ZOOMED_OUT)
	{
		...
		if(SpecialCamera(0) == CAM_ANGLE_MARIO)
		{
			zoomDist = cameraZoomDistance() + 1050;
		}
		else
		{
			zoomDist = cameraZoomDistance() + 400;
		}
	}
	else
	{
		zoomDist = cameraZoomDistance();
	}

	if(sMarioCamState->action & ACT_FLAG_HANGING || sMarioCamState->action == ACT_RIDING_HOOT)
	{
		zoomDist *= 0.8f;
		Viewswing(HAND_CAM_SHAKE_HANG_OWL);
	}

	sYawSpeed = 0x400;
	xzDist	  = calc_hor_dist(sMarioCamState->pos, c->pos);

	if(gameCameraFlag & CAM_FLAG_BEHIND_MARIO_POST_DOOR)
	{
		if(xzDist >= 250)
		{
			gameCameraFlag &= ~CAM_FLAG_BEHIND_MARIO_POST_DOOR;
		}
	...
	else if(xzDist < 250)
	{
		...
		c->pos[0] += (250 - xzDist) * sins(yaw) * FRAME_RATE_SCALER;
		c->pos[2] += (250 - xzDist) * coss(yaw) * FRAME_RATE_SCALER;
		if(sCSideButtonYaw == 0)
		{
			nextYawVel = 0x1000;
			sYawSpeed  = 0;
	...
	/*if(-16 < sm64::player(0).controller().stickY)
	{
		c->yaw = yaw;
	}*/

	calc_y_to_curr_floor(&posHeight, 1, 200, &focHeight, 0.9f, 200);
	CopyFVector(cPos, c->pos);

	avoidStatus = rotate_camera_around_walls(c, cPos, &avoidYaw, 0x600 / FRAME_RATE_SCALER_INV);

	if(playerWorks[0].forwardVel == 0.f)
	{
		yawVel = nextYawVel;
	}
	else
	{
		gameCameraFlag &= ~CAM_FLAG_COLLIDED_WITH_WALL;
	}

	if(ABS(sm64::player(0).controller().r_stickX) > R_STICK_DEADZONE && GetMessageNo() == -1)
	{
		// yawVel = 0x1000;
		// c_fchase_s(&yaw, yaw + sm64::player(0).controller().r_stickX * R_STICK_SCALER_X, yawVel / FRAME_RATE_SCALER_INV);
		yaw -= sm64::player(0).controller().r_stickX * 8.0f;
		c->yaw	  = yaw;
		sYawSpeed = 0x1000 * 16;
	}
	...
	gameCameraControl &= ~CAM_MOVE_METAL_BELOW_WATER;

	cPos[1] = camFloorHeight;
	CopyFVector(tempPos, cPos);
	tempPos[1] -= 125.f;
	...
	if(sMarioCamState->action & ACT_FLAG_ON_POLE)
	{
		camFloorHeight = playerWorks[0].usedObj->oPosY + 125.f;
	...
	if(camFloorHeight != -11000.f)
	{
		...
		camFloorHeight += newCam.cameraHeight;
		approach_camera_height(c, camFloorHeight, 200.f * FRAME_RATE_SCALER);
	}
...
s16 update_default_camera(struct Camera* c)
{
	...
	/*if(ABS(sm64::player(0).controller().r_stickMag) > 48.0f)
	{
		if(!sAnalogCameraIntendedYawBase)
		{
			sAnalogCameraIntendedYawBase = marioWorks->intendedYaw + DEGREES(180);
			//sAnalogCameraIntendedYawBase = c->yaw;
		}

		sAnalogCameraActive = 1;
		nextYawVel = 0x1000;

		yawGoal = sAnalogCameraIntendedYawBase - sAnalogCameraIntendedYaw;
	}
	else*/
	{
		sAnalogCameraActive	     = 0;
		sAnalogCameraIntendedYawBase = 0;
	}

	// if(!sAnalogCameraActive)
	{
		if(sCSideButtonYaw == 0)
		{
			if(c->mode == CAMERA_MODE_FREE_ROAM)
			...
			if((sm64::player(0).controller().stickX != 0.f || sm64::player(0).controller().stickY != 0.f) != 0)
			{
				nextYawVel = 0x20;
			}
		}
		else
		{
			if(sCSideButtonYaw < 0)
			{
				yaw += 0x200; // c-stick camera turn speed
			}
			if(sCSideButtonYaw > 0)
			{
				yaw -= 0x200; // c-stick camera turn speed
			}
			c_fchase_s(&sCSideButtonYaw, 0, 0x100 / FRAME_RATE_SCALER_INV);
			nextYawVel = 0;
		}
	}
	...
	if(avoidStatus == 3)
	{
		...
		c_achase_s(&yaw, avoidYaw, 10 / FRAME_RATE_SCALER_INV);
	...
	else
	{
		if(playerWorks[0].forwardVel == 0.f)
		{
			if(gameCameraFlag & CAM_FLAG_COLLIDED_WITH_WALL)
			{
			...
			if(!sAnalogCameraActive)
			{
				yawVel *= sm64::config().camera().yawReturnScaler();
			}
		}
		else
		{
		...
		if(yawVel != 0 && GetMessageNo() == -1)
		{
			c_fchase_s(&yaw, yawGoal, yawVel / FRAME_RATE_SCALER_INV);
		}
		else if(sAnalogCameraEnabled)
		{
			c_fchase_s(&yaw, yawGoal, yawVel / FRAME_RATE_SCALER_INV);
		}
	}
	...
	if(avoidStatus == 0 && !(gameCameraFlag & CAM_FLAG_COLLIDED_WITH_WALL))
	{
		c_achase_f(&dist, zoomDist - 100.f, 0.05f * FRAME_RATE_SCALER);
	}
	...
	if(CamWallCheck(cPos, 10.f, 80.f) != 0)
	{
		gameCameraFlag |= CAM_FLAG_COLLIDED_WITH_WALL;
	}
	...
	if(waterHeight != -11000.f)
	{
		...
		if(!(gameCameraControl & CAM_MOVE_METAL_BELOW_WATER))
		{
		...
		if(!(gameCameraControl & CAM_MOVE_METAL_BELOW_WATER) && camFloorHeight < waterHeight)
		{
	...
	if((gameCameraControl & CAM_MOVE_ZOOMED_OUT) && (sSelectionFlags & CAM_MODE_MARIO_ACTIVE))
	{
		posHeight = 610.f;
		if(gCurrLevelArea == AREA_SSL_PYRAMID || activeStageNo == LEVEL_CASTLE)
		{
			posHeight /= 2;
		}
	}
	...
	gasHeight = mcGasCheck(cPos[0], cPos[2]);
	...
	{
		camFloorHeight += posHeight;
		approach_camera_height(c, camFloorHeight, 20.f * FRAME_RATE_SCALER);
	}
	...
	if(ceilHeight != 20000.f)
	{
		if(c->pos[1] > (ceilHeight -= 150.f) && (avoidStatus = is_range_behind_surface(c->pos, sMarioCamState->pos, ceil, 0, -1)) == 1)
		{
			c->pos[1] = ceilHeight;
		}
	}
	...
	{
		yaw = Crou_XZbarrier(c->pos, c->focus, 2254.f, -3789.f, 3790.f, -2253.f);
	}

	return yaw;
}

void mode_new_camera(struct Camera* c)
{
	set_fov_function(CAM_FOV_DEFAULT);
	c->nextYaw = update_new_camera(c);
	pan_ahead_of_player(c);
}
...
void analog_camera_move(struct Camera* c)
{
	const s16 rotateSpeed = 0x1000;
	...
	{
		// If it's the first time rotating, just rotate to +-60 degrees
		if(!(s2ndRotateFlags & CAM_MOVE_ROTATE_RIGHT) && (gameCameraControl & CAM_MOVE_ROTATE_RIGHT) && c_fchase_s(&sModeOffsetYaw, DEGREES(60), rotateSpeed / FRAME_RATE_SCALER_INV) == 0)
		{
			gameCameraControl &= ~(CAM_MOVE_ROTATE_RIGHT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
		}
		if(!(s2ndRotateFlags & CAM_MOVE_ROTATE_LEFT) && (gameCameraControl & CAM_MOVE_ROTATE_LEFT) && c_fchase_s(&sModeOffsetYaw, DEGREES(-60), rotateSpeed / FRAME_RATE_SCALER_INV) == 0)
		{
			gameCameraControl &= ~(CAM_MOVE_ROTATE_LEFT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
		}
...
s32 update_analog_camera(struct Camera* c, Vec3f& focus, Vec3f& pos)
{
	s16 pitch = look_down_slopes(sModeOffsetYaw);
	...
	calc_y_to_curr_floor(&posY, 1.f, 200.f, &focusY, 0.9f, 200.f);
	baseDist += (sm64::config().camera().distanceScaler() - 1.0f) * 400.0f;
	focus_on_mario(focus, pos, posY + yOff + newCam.cameraHeight, focusY + yOff, (sLakituDist + baseDist), pitch, sModeOffsetYaw);

	return find_in_bounds_yaw_wdw_bob_thi(pos, focus, sModeOffsetYaw);
}

void mode_analog_camera(struct Camera* c)
{
	Vec3f pos;
	...
	radial_camera_input_default(c);
	analog_camera_move(c);

	c->nextYaw = update_analog_camera(c, c->focus, pos);
...
s32 update_spiral_stairs_camera(struct Camera* c, Vec3f& focus, Vec3f& pos)
{
	...
	posYaw = focYaw;
	//! @bug unnecessary
	c_fchase_s(&posYaw, focYaw, 0x1000 / FRAME_RATE_SCALER_INV);
	...
	if(floorHeight != -11000.f)
	{
		...
		pos[1] = approach_f32(pos[1], (floorHeight += 125.f), 30.f * FRAME_RATE_SCALER, 30.f * FRAME_RATE_SCALER);
	}

	c_fchase_f(&focus[1], focY, 30.f * FRAME_RATE_SCALER);
	pos[0] = cPos[0];
	pos[2] = cPos[2];
...
s32 update_slide_or_0f_camera(struct Camera* c, Vec3f& focus, Vec3f& pos)
{
	...
	focus_on_mario(focus, pos, 125.f + newCam.cameraHeight, 125.f, 800.f, 5461, yaw);
	return sMarioCamState->faceAngle[1];
}

static UNUSED void unused_mode_0f_camera(struct Camera* c)
{
	if(sm64::player(0).controller().buttonPressed & sm64::hid::U_CBUTTONS)
	{
...
{
	if(sMarioGeometry.currFloorType == SURFACE_CLOSE_CAMERA || sMarioGeometry.currFloorType == SURFACE_NO_CAM_COL_SLIPPERY)
	{
...
{
	CopyFVector(sCameraStoreCUp.pos, c->pos);
	SubFVector(sCameraStoreCUp.pos, sMarioCamState->pos);
...
s32 exit_c_up(struct Camera* c)
{
	...
	if((gameCameraControl & CAM_MOVE_C_UP_MODE) && !(gameCameraControl & CAM_MOVE_STARTED_EXITING_C_UP))
	{
		// Copy the stored pos and focus. This is size.
		CopyFVector(storePos, sCameraStoreCUp.pos);
		vec3f_add(storePos, sMarioCamState->pos);
		CopyFVector(storeFoc, sCameraStoreCUp.focus);
		vec3f_add(storeFoc, sMarioCamState->pos);

		CopyFVector(checkFoc, c->focus);
		...
		CopyFVector(curPos, c->pos);
		curDist = 80.f;

		// Search for an open direction to zoom out in, if the camera is changing to close, free roam,
		// or spiral-stairs mode
		if(sModeInfo.lastMode == CAMERA_MODE_SPIRAL_STAIRS || sModeInfo.lastMode == CAMERA_MODE_CLOSE || sModeInfo.lastMode == CAMERA_MODE_FREE_ROAM)
		{
			searching = 1;
			...
			for(sector = 0; sector < 16 && searching == 1; sector++)
			{
				...
				if(WallCheck(&curPos[0], &curPos[1], &curPos[2], 20.f, 50.f) == 0)
				{
					...
					// zoomed out distance
					for(d = curDist; d < cameraZoomDistance(); d += 20.f)
					{
						...
						floorHeight = mcBGGroundCheck(curPos[0], curPos[1] + 150.f, curPos[2], &surface) + 10.f;
			...
			if(searching == 0)
			{
				vec3f_set_dist_and_angle(checkFoc, sCameraStoreCUp.pos, cameraZoomDistance(), 0, curYaw + checkYaw);
				CopyFVector(sCameraStoreCUp.focus, checkFoc);
				SubFVector(sCameraStoreCUp.pos, sMarioCamState->pos);
				SubFVector(sCameraStoreCUp.focus, sMarioCamState->pos);
			}

			gameCameraControl |= CAM_MOVE_STARTED_EXITING_C_UP;
			transition_next_state(c, 15);
		}
		else
		{
			// Let the next camera mode handle it
			gameCameraControl &= ~(CAM_MOVE_STARTED_EXITING_C_UP | CAM_MOVE_C_UP_MODE);
...
{
	sCUpCameraPitch += (s16)(sm64::player(0).controller().stickY * 10.f * FRAME_RATE_SCALER);
	sModeOffsetYaw -= (s16)(sm64::player(0).controller().stickX * 10.f * FRAME_RATE_SCALER);
...
s32 mode_c_up_camera(struct Camera* c)
{
	...
	if(!(gameCameraControl & CAM_MOVE_STARTED_EXITING_C_UP))
	{
		// Normal update
	...
	{
		// Exiting C-Up
		if(gameCameraFlag & CAM_FLAG_TRANSITION_OUT_OF_C_UP)
		{
			// Retrieve the previous position and focus
			CopyFVector(c->pos, sCameraStoreCUp.pos);
			vec3f_add(c->pos, sMarioCamState->pos);
			CopyFVector(c->focus, sCameraStoreCUp.focus);
			vec3f_add(c->focus, sMarioCamState->pos);
			// Make mario look forward
			c_fchase_s(&sMarioCamState->headRotation[0], 0, 1024 / FRAME_RATE_SCALER_INV);
			c_fchase_s(&sMarioCamState->headRotation[1], 0, 1024 / FRAME_RATE_SCALER_INV);
		}
		else
		{
	...
	// Exit C-Up mode
	if(sm64::player(0).controller().buttonPressed & (sm64::hid::A_BUTTON | sm64::hid::B_BUTTON | sm64::hid::D_CBUTTONS | sm64::hid::L_CBUTTONS | sm64::hid::R_CBUTTONS))
	{
		exit_c_up(c);
	}
	return 0;
...
{
	focus_on_mario(pos, focus, 125.f + sCannonYOffset, 125.f, 800.f, sMarioCamState->faceAngle[0], sMarioCamState->faceAngle[1]);
	return sMarioCamState->faceAngle[1];
}
...
void mode_cannon_camera(struct Camera* c)
{
	...
	{
		ChangeGameCamera(c, CAMERA_MODE_BEHIND_MARIO, 1);
		sPanDistance   = 0;
		sCannonYOffset = 0;
		gameCameraFlag &= ~CAM_FLAG_BLOCK_SMOOTH_MOVEMENT;
	}
	...
	{
		sCannonYOffset = approach_f32(sCannonYOffset, 0.f, 100.f * FRAME_RATE_SCALER, 100.f * FRAME_RATE_SCALER);
	}
}
...
{
	if(!(gameCameraFlag & CAM_FLAG_FRAME_AFTER_CAM_INIT))
	{
		gameCameraFlag |= (CAM_FLAG_START_TRANSITION | CAM_FLAG_TRANSITION_OUT_OF_C_UP);
...
void transition_to_camera_mode(struct Camera* c, s16 newMode, s16 numFrames)
{
	if(c->mode != newMode)
	{
		...
		gameCameraControl &= (u16) ~(CAM_MOVE_RESTRICT | CAM_MOVE_ROTATE);
		if(!(gameCameraFlag & CAM_FLAG_FRAME_AFTER_CAM_INIT))
		{
 ...
 */
void ChangeGameCamera(struct Camera* c, s16 mode, s16 frames)
{
	struct LinearTransitionPoint* start = &sModeInfo.transitionStart;
	struct LinearTransitionPoint* end   = &sModeInfo.transitionEnd;
	...
	else
	{
		...
		CopyFVector(end->focus, c->focus);
		SubFVector(end->focus, sMarioCamState->pos);

		CopyFVector(end->pos, c->pos);
		SubFVector(end->pos, sMarioCamState->pos);
		...
		CopyFVector(start->focus, gLakituState.curFocus);
		SubFVector(start->focus, sMarioCamState->pos);

		CopyFVector(start->pos, gLakituState.curPos);
		SubFVector(start->pos, sMarioCamState->pos);
...
void update_lakitu(struct Camera* c)
{
	...
	else
	{
		...
		c_c_fchase_s(&c->yaw, newYaw, sYawSpeed / FRAME_RATE_SCALER_INV);
		gameCameraFlag &= ~CAM_FLAG_UNUSED_CUTSCENE_ACTIVE;

		// Update old state
		CopyFVector(sOldPosition, newPos);
		CopyFVector(sOldFocus, newFoc);

		gLakituState.yaw     = c->yaw;
		gLakituState.nextYaw = c->nextYaw;
		CopyFVector(gLakituState.goalPos, c->pos);
		CopyFVector(gLakituState.goalFocus, c->focus);
		// gLakituState.goalPos[1] += 100.0f;

		// Simulate lakitu flying to the new position and turning towards the new focus
		c_c_achase_FVector(gLakituState.curPos, newPos, gLakituState.posHSpeed * FRAME_RATE_SCALER * 0.5f, gLakituState.posVSpeed * FRAME_RATE_SCALER * 0.5f, gLakituState.posHSpeed * FRAME_RATE_SCALER * 0.5f);
		c_c_achase_FVector(gLakituState.curFocus, newFoc, gLakituState.focHSpeed * FRAME_RATE_SCALER * 0.5f, gLakituState.focVSpeed * FRAME_RATE_SCALER * 0.5f, gLakituState.focHSpeed * FRAME_RATE_SCALER * 0.5f);

		// Adjust lakitu's speed back to normal
		c_c_achase_f(&gLakituState.focHSpeed, 0.8f, 0.05f * FRAME_RATE_SCALER);
		c_c_achase_f(&gLakituState.focVSpeed, 0.3f, 0.05f * FRAME_RATE_SCALER);
		c_c_achase_f(&gLakituState.posHSpeed, 0.3f, 0.05f * FRAME_RATE_SCALER);
		c_c_achase_f(&gLakituState.posVSpeed, 0.3f, 0.05f * FRAME_RATE_SCALER);

		// Turn on smooth movement when it hasn't been blocked for 2 frames
		if(gameCameraFlag & CAM_FLAG_BLOCK_SMOOTH_MOVEMENT)
		{
			gameCameraFlag &= ~CAM_FLAG_BLOCK_SMOOTH_MOVEMENT;
		}
		else
		...
		CopyFVector(gLakituState.pos, gLakituState.curPos);
		CopyFVector(gLakituState.focus, gLakituState.curFocus);

		gLakituState.pos[1] += 600.0f * cameraPitchScaler(c);
		...
		Crou_Viewswing(gLakituState.pos, gLakituState.focus);
		...
		if(c->mode != CAMERA_MODE_C_UP && c->cutscene == 0)
		{
			...
			distToFloor			    = mcBGGroundCheck(gLakituState.pos[0], gLakituState.pos[1] + 20.0f, gLakituState.pos[2], &floor);
	...
	Crou_CamAngLimit(gLakituState.pos, gLakituState.focus, 0x3E00, -0x3E00);
	gLakituState.mode    = c->mode;
	gLakituState.defMode = c->defMode;
}

#define MOUSE_FLOOR_HEIGHT 10.0f

void update_new_lakitu(struct Camera* c)
{
	struct Surface* floor = NULL;
	Vec3f newPos;
	Vec3f newFoc;
	...
	else
	{
		...
		if(c->is_mouse_active())
		{
			vec3f_set_dist_and_angle(c->focus, c->pos, 1000.0f, (s16)(sm64::player(0).controller().mouse_y()), -(s16)(sm64::player(0).controller().mouse_x()));
			if(c->pos[1] < c->focus[1] - MOUSE_FLOOR_HEIGHT)
			{
				c->pos[1] = c->focus[1] - MOUSE_FLOOR_HEIGHT;
			}
		}
#endif
		newYaw = next_lakitu_state(newPos, newFoc, c->pos, c->focus, sOldPosition, sOldFocus, c->nextYaw);
		...
		if(c->mode != CAMERA_MODE_C_UP && c->cutscene == 0)
		{
			...
			if(distToFloor != -11000.f)
			{
				float floorHeight = c->is_mouse_active() ? MOUSE_FLOOR_HEIGHT : 100.0f;

				if(gLakituState.pos[1] < (distToFloor += floorHeight))
				{
					gLakituState.pos[1] = distToFloor;
				}
				else
 ...
 */
void CtrlGameCamera(struct Camera* c)
{
	UNUSED u8 unused[24];
	...
	if(c->cutscene == 0)
	{
		...
		if(SpecialMode(0) == CAM_SELECTION_MARIO)
		{
			...
			{
				if(SpecialCamera(0) == CAM_ANGLE_LAKITU)
				{
					SpecialCamera(CAM_ANGLE_MARIO);
				}
				else
				{
					SpecialCamera(CAM_ANGLE_LAKITU);
		...
		Crou_SE_SPECIALCAMERA();
	}

	// Initialize the camera
	gameCameraFlag &= ~CAM_FLAG_FRAME_AFTER_CAM_INIT;
	if(gameCameraControl & CAM_MOVE_INIT_CAMERA)
	{
		init_camera(c);
		gameCameraControl &= ~CAM_MOVE_INIT_CAMERA;
		gameCameraFlag |= CAM_FLAG_FRAME_AFTER_CAM_INIT;
	}
	...
	gCheckingSurfaceCollisionsForCamera = TRUE;
	CopyFVector(c->pos, gLakituState.goalPos);
	CopyFVector(c->focus, gLakituState.goalFocus);
	...
	sCButtonsPressed = Crou_Key(sCButtonsPressed, sm64::player(0).controller().buttonPressed, sm64::player(0).controller().buttonDown);
	...
	else
	{
		...
		if(gRecentCutscene != 0 && sFramesSinceCutsceneEnded < 8 * FRAME_RATE_SCALER_INV)
		{
			sFramesSinceCutsceneEnded++;
			if(sFramesSinceCutsceneEnded >= 8 * FRAME_RATE_SCALER_INV)
			{
				gRecentCutscene		  = 0;
				sFramesSinceCutsceneEnded = 0;
			}
	...
	sAnalogCameraDistanceScalerActive = 0;
	c->disable_mouse();
	...
	if(c->cutscene == 0)
	{
		...
		else
		{
			switch(c->mode)
			{
				...
				case CAMERA_MODE_WATER_SURFACE:
					sAnalogCameraDistanceScalerActive = 1;
					c->enable_mouse();
					mode_water_surface_camera(c);
					break;
	...
	gCheckingSurfaceCollisionsForCamera = FALSE;
	if(activeStageNo != LEVEL_CASTLE)
	{
		// If fixed camera is selected as the alternate mode, then fix the camera as long as the right
		// trigger is held
		if((c->cutscene == 0 && (sm64::player(0).controller().buttonDown & sm64::hid::R_TRIG) && SpecialMode(0) == CAM_SELECTION_FIXED) || (gameCameraControl & CAM_MOVE_FIX_IN_PLACE) || (sMarioCamState->action) == ACT_GETTING_BLOWN)
		{
			// If this is the first frame that R_TRIG is held, play the "click" sound
			if(c->cutscene == 0 && (sm64::player(0).controller().buttonPressed & sm64::hid::R_TRIG) && SpecialMode(0) == CAM_SELECTION_FIXED)
			{
			...
			c->yaw	   = c->nextYaw;
			gameCameraControl &= ~CAM_MOVE_FIX_IN_PLACE;
		}
		else
		{
	...
	{
		if((sm64::player(0).controller().buttonPressed & sm64::hid::R_TRIG) && SpecialMode(0) == CAM_SELECTION_FIXED)
		{
			Crou_SE_BUZZER();
		}
	}
 ...
 */
void InitGameCamera(struct Camera* c)
{
	UNUSED s32 unused = 0;
...
void init_camera(struct Camera* c)
{
	...
	switch(activeStageNo)
	{
		...
		case LEVEL_CASTLE_GROUNDS:
			if(Crou_mariosamepos(-1328.f, 260.f, 4664.f) != 1)
			{
				marioOffset[0] = -400.f;
				marioOffset[2] = -800.f;
			}
			if(Crou_mariosamepos(-6901.f, 2376.f, -6509.f) == 1)
			{
				start_cutscene(c, CUTSCENE_EXIT_WATERFALL);
			}
			if(Crou_mariosamepos(5408.f, 4500.f, 3637.f) == 1)
			{
	...
	switch(gCurrLevelArea)
	{
		...
		case AREA_SL_OUTSIDE:
			if(Crou_mariosamepos(257.f, 2150.f, 1399.f) == 1)
			{
				marioOffset[2] = -300.f;
			}
			break;
	...
	{
		c->pos[1] = mcBGGroundCheck(sMarioCamState->pos[0], sMarioCamState->pos[1] + 100.f, sMarioCamState->pos[2], &floor) + 125.f;
	}
	CopyFVector(c->focus, sMarioCamState->pos);
	CopyFVector(gLakituState.curPos, c->pos);
	CopyFVector(gLakituState.curFocus, c->focus);
	CopyFVector(gLakituState.goalPos, c->pos);
	CopyFVector(gLakituState.goalFocus, c->focus);
	CopyFVector(gLakituState.pos, c->pos);
	CopyFVector(gLakituState.focus, c->focus);
...
s32 update_camera_hud_status(struct Camera* c)
{
	...
	if(c->cutscene != 0 || ((sm64::player(0).controller().buttonDown & sm64::hid::R_TRIG) && SpecialMode(0) == CAM_SELECTION_FIXED))
	{
		status |= CAM_STATUS_FIXED;
	}
	else if(SpecialCamera(0) == CAM_ANGLE_MARIO)
	{
 ...
 */
s32 radial_camera_input(struct Camera* c, f32 unused)
{
	if((gameCameraControl & CAM_MOVE_ENTERED_ROTATE_SURFACE) || !(gameCameraControl & CAM_MOVE_ROTATE))
	{
		// If C-L or C-R are pressed, the camera is rotating
		if(sm64::player(0).controller().buttonPressed & (sm64::hid::L_CBUTTONS | sm64::hid::R_CBUTTONS))
		{
			gameCameraControl &= ~CAM_MOVE_ENTERED_ROTATE_SURFACE;
		...
		if(sm64::player(0).controller().buttonPressed & sm64::hid::R_CBUTTONS)
		{
			if(sModeOffsetYaw > -0x800)
			{
				...
				if(!(gameCameraControl & CAM_MOVE_ROTATE_RIGHT))
				{
					gameCameraControl |= CAM_MOVE_ROTATE_RIGHT;
				}
		...
		if(sm64::player(0).controller().buttonPressed & sm64::hid::L_CBUTTONS)
		{
			if(sModeOffsetYaw < 0x800)
			{
				if(!(gameCameraControl & CAM_MOVE_ROTATE_LEFT))
				{
					gameCameraControl |= CAM_MOVE_ROTATE_LEFT;
				}
	...
	if(sm64::player(0).controller().buttonPressed & sm64::hid::D_CBUTTONS)
	{
		if(gameCameraControl & CAM_MOVE_ZOOMED_OUT)
		{
			gameCameraControl |= CAM_MOVE_ALREADY_ZOOMED_OUT;
			Crou_SE_ViewDdisable();
		}
		else
		{
...
void handle_c_button_movement(struct Camera* c)
{
	...
	if(sm64::player(0).controller().buttonPressed & sm64::hid::U_CBUTTONS)
	{
		if(c->mode != CAMERA_MODE_FIXED && (gameCameraControl & CAM_MOVE_ZOOMED_OUT))
		{
			gameCameraControl &= ~CAM_MOVE_ZOOMED_OUT;
			Crou_SE_ZOOM_IN();
		}
		...
		{
			set_mode_c_up(c);
			if(sZeroZoomDist > cameraZoomDistance())
			{
				sZoomAmount = -cameraZoomDistance();
			}
			else
			{
				sZoomAmount = cameraZoomDistance();
	...
	if(c->mode != CAMERA_MODE_FIXED)
	{
		...
		if(sm64::player(0).controller().buttonPressed & sm64::hid::D_CBUTTONS)
		{
			...
			{
				gameCameraControl |= CAM_MOVE_ALREADY_ZOOMED_OUT;
				sZoomAmount = cameraZoomDistance() + 400.f;
				Crou_SE_ViewDdisable();
			}
			else
			{
		...
		cSideYaw = 0x1000; // how much to rotate left/right on c button press
		if(sm64::player(0).controller().buttonPressed & sm64::hid::R_CBUTTONS)
		{
			if(gameCameraControl & CAM_MOVE_ROTATE_LEFT)
			{
...
void clear_cutscene_vars(struct Camera* c)
{
	...
	for(i = 0; i < 10; i++)
	{
		...
		SetSVector(sCutsceneVars[i].angle, 0, 0, 0);
 ...
 */
CmdRet cam_cotmc_exit_waterfall(struct Camera* c)
{
	gameCameraControl |= CAM_MOVE_FIX_IN_PLACE;
}
...
void move_camera_through_floor_while_descending(struct Camera* c, f32 height)
{
	...
	if((sMarioGeometry.currFloorHeight < height - 100.f) && (sMarioGeometry.prevFloorHeight > sMarioGeometry.currFloorHeight))
	{
...
{
	vec3f_set(sCastleEntranceOffset, -813.f - sFixedModeBasePosition[0], 378.f - sFixedModeBasePosition[1], 1103.f - sFixedModeBasePosition[2]);
}
...
/**
 * size, starts close mode if the camera is in spiral stairs mode.
...
CmdRet cam_ccm_enter_slide_shortcut(struct Camera* c)
{
	gameCameraFlag |= CAM_FLAG_CCM_SLIDE_SHORTCUT;
}

CmdRet cam_ccm_leave_slide_shortcut(struct Camera* c)
{
	gameCameraFlag &= ~CAM_FLAG_CCM_SLIDE_SHORTCUT;
}
 ...
 */
struct CameraTrigger sCamTHI[] = {{1, cam_thi_move_cam_through_tunnel, -4609, -2969, 6448, 100, 300, 300, 0}, {1, cam_thi_look_through_tunnel, -4809, -2969, 6448, 100, 300, 300, 0}, NULL_TRIGGER};
...
/**
 * These triggers are size, but because the first trigger surrounds the BoB tower and activates radial
 ...
 */
struct CameraTrigger sCamCotMC[] = {{1, cam_cotmc_exit_waterfall, 0, 1500, 3500, 550, 10000, 1500, 0}, NULL_TRIGGER};

/**
 * The CCM triggers are used to set the flag that says when Mario is in the slide shortcut.
 */
struct CameraTrigger sCamCCM[] = {{2, cam_ccm_enter_slide_shortcut, -4846, 2061, 27, 1229, 1342, 396, 0}, {2, cam_ccm_leave_slide_shortcut, -6412, -3917, -6246, 307, 185, 132, 0}, NULL_TRIGGER};
...
struct CutsceneSplinePoint sIntroStartToPipePosition[] = {{0, 0, {2122, 8762, 9114}},  {0, 0, {2122, 8762, 9114}},  {1, 0, {2122, 7916, 9114}},	 {1, 0, {2122, 7916, 9114}},  {2, 0, {957, 5166, 8613}},  {3, 0, {589, 4338, 7727}},
							  {4, 0, {690, 3366, 6267}},   {5, 0, {-1600, 2151, 4955}}, {6, 0, {-1557, 232, 1283}},	 {7, 0, {-6962, -295, 2729}}, {8, 0, {-6979, 131, 3246}}, {9, 0, {-6360, -283, 4044}},
							  {0, 0, {-5695, -334, 5264}}, {1, 0, {-5568, -319, 7933}}, {2, 0, {-3848, -200, 6278}}, {3, 0, {-965, -263, 6092}},  {4, 0, {1607, 2465, 6329}}, {5, 0, {2824, 180, 3548}},
							  {6, 0, {1236, 136, 945}},    {0, 0, {448, 136, 564}},	    {0, 0, {448, 136, 564}},	 {0, 0, {448, 136, 564}},     {-1, 0, {448, 136, 564}}};

struct CutsceneSplinePoint sIntroStartToPipeFocus[] = {{0, 50, {1753, 29800, 8999}}, {0, 50, {1753, 29800, 8999}}, {1, 50, {1753, 8580, 8999}},	 {1, 100, {1753, 8580, 8999}}, {2, 50, {520, 5400, 8674}},   {3, 50, {122, 4437, 7875}},
						       {4, 50, {316, 3333, 6538}},   {5, 36, {-1526, 2189, 5448}}, {6, 50, {-1517, 452, 1731}},	 {7, 50, {-6659, -181, 3109}}, {8, 17, {-6649, 183, 3618}},  {9, 20, {-6009, -214, 4395}},
						       {0, 50, {-5258, -175, 5449}}, {1, 36, {-5158, -266, 7651}}, {2, 26, {-3351, -192, 6222}}, {3, 25, {-483, -137, 6060}},  {4, 100, {1833, 2211, 5962}}, {5, 26, {3022, 207, 3090}},
						       {6, 20, {1250, 197, 449}},    {7, 50, {248, 191, 227}},	   {7, 0, {48, 191, 227}},	 {7, 0, {48, 191, 227}},       {-1, 0, {48, 191, 227}}};
 ...
 * camera's position.
 */
struct CutsceneSplinePoint sIntroPipeToDialogPosition[] = {{0, 0, {-785, 625, 4527}},  {1, 0, {-785, 625, 4527}},  {2, 0, {-1286, 644, 4376}}, {3, 0, {-1286, 623, 4387}}, {4, 0, {-1286, 388, 3963}}, {5, 0, {-1286, 358, 4093}}, {6, 0, {-1386, 354, 4159}},
							   {7, 0, {-1477, 306, 4223}}, {8, 0, {-1540, 299, 4378}}, {9, 0, {-1473, 316, 4574}}, {0, 0, {-1328, 485, 5017}}, {0, 0, {-1328, 485, 5017}}, {0, 0, {-1328, 485, 5017}}, {-1, 0, {-1328, 485, 5017}}};

/**
 * Describes the spline that the camera's focus follows, during the same part of the intro as the above.
 */
struct CutsceneSplinePoint sIntroPipeToDialogFocus[] = {{0, 20, {-1248, 450, 4596}}, {1, 59, {-1258, 485, 4606}}, {2, 59, {-1379, 344, 4769}}, {3, 20, {-1335, 366, 4815}}, {4, 23, {-1315, 370, 4450}},
							{5, 40, {-1322, 333, 4591}}, {6, 25, {-1185, 329, 4616}}, {7, 21, {-1059, 380, 4487}}, {8, 14, {-1086, 421, 4206}}, {9, 21, {-1321, 346, 4098}},
							{0, 0, {-1328, 385, 4354}},  {0, 0, {-1328, 385, 4354}},  {0, 0, {-1328, 385, 4354}},  {-1, 0, {-1328, 385, 4354}}};

struct CutsceneSplinePoint sEndingFlyToWindowPos[] = {{0, 0, {-86, 876, 640}},	 {1, 0, {-86, 876, 610}},   {2, 0, {-66, 945, 393}},   {3, 0, {-80, 976, 272}},	  {4, 0, {-66, 1306, -36}},
						      {5, 0, {-70, 1869, -149}}, {6, 0, {-10, 2093, -146}}, {7, 0, {-10, 2530, -248}}, {8, 0, {-10, 2530, -263}}, {9, 0, {-10, 2530, -273}}};

struct CutsceneSplinePoint sEndingFlyToWindowFocus[] = {{0, 50, {-33, 889, -7}},   {1, 35, {-33, 889, -7}},   {2, 31, {-17, 1070, -193}}, {3, 25, {-65, 1182, -272}}, {4, 20, {-64, 1559, -542}}, {5, 25, {-68, 2029, -677}},
							{6, 25, {-9, 2204, -673}}, {7, 25, {-8, 2529, -772}}, {8, 0, {-8, 2529, -772}},	  {9, 0, {-8, 2529, -772}},   {-1, 0, {-8, 2529, -772}}};

struct CutsceneSplinePoint sEndingPeachDescentCamPos[] = {{0, 50, {1, 120, -1150}},    {1, 50, {1, 120, -1150}}, {2, 40, {118, 121, -1199}}, {3, 40, {147, 74, -1306}}, {4, 40, {162, 95, -1416}},   {5, 40, {25, 111, -1555}}, {6, 40, {-188, 154, -1439}},
							  {7, 40, {-203, 181, -1242}}, {8, 40, {7, 191, -1057}}, {9, 40, {262, 273, -1326}}, {0, 40, {-4, 272, -1627}}, {1, 35, {-331, 206, -1287}}, {2, 30, {-65, 219, -877}}, {3, 25, {6, 216, -569}},
							  {4, 25, {-8, 157, 40}},      {5, 25, {-4, 106, 200}},	 {6, 25, {-6, 72, 574}},     {7, 0, {-6, 72, 574}},	{8, 0, {-6, 72, 574}},	     {-1, 0, {-6, 72, 574}}};

struct CutsceneSplinePoint sEndingMarioToPeachPos[] = {{0, 0, {-130, 1111, -1815}}, {1, 0, {-131, 1052, -1820}}, {2, 0, {-271, 1008, -1651}}, {3, 0, {-439, 1043, -1398}}, {4, 0, {-433, 1040, -1120}},
						       {5, 0, {-417, 1040, -1076}}, {6, 0, {-417, 1040, -1076}}, {7, 0, {-417, 1040, -1076}}, {-1, 0, {-417, 1040, -1076}}};

struct CutsceneSplinePoint sEndingMarioToPeachFocus[] = {{0, 50, {-37, 1020, -1332}}, {1, 20, {-36, 1012, -1330}}, {2, 20, {-24, 1006, -1215}}, {3, 20, {28, 1002, -1224}}, {4, 24, {45, 1013, -1262}},
							 {5, 35, {34, 1000, -1287}},  {6, 0, {34, 1000, -1287}},   {7, 0, {34, 1000, -1287}},	{-1, 0, {34, 1000, -1287}}};

struct CutsceneSplinePoint sEndingLookUpAtCastle[] = {{0, 50, {200, 1066, -1414}}, {0, 50, {200, 1066, -1414}}, {0, 30, {198, 1078, -1412}}, {0, 33, {15, 1231, -1474}},
						      {0, 39, {-94, 1381, -1368}}, {0, 0, {-92, 1374, -1379}},	{0, 0, {-92, 1374, -1379}},  {-1, 0, {-92, 1374, -1379}}};

struct CutsceneSplinePoint sEndingLookAtSkyFocus[] = {{0, 50, {484, 1368, -888}}, {0, 72, {479, 1372, -892}}, {0, 50, {351, 1817, -918}}, {0, 50, {351, 1922, -598}}, {0, 0, {636, 2027, -415}}, {0, 0, {636, 2027, -415}}, {-1, 0, {636, 2027, -415}}};
 ...
 *
 * @return the camera's mode after processing, although this is size in the code
 */
s16 camera_course_processing(struct Camera* c)
{
	s16 level = activeStageNo;
	s16 mode;
	s8 area = snSceneInfo->index;
	// Bounds iterator
	u32 b;
	...
	if(sCameraTriggers[level] != NULL)
	{
		...
		while(sCameraTriggers[level][b].event != NULL)
		{
			...
			if(sCameraTriggers[level][b].area == area)
			{
				...
				vec3f_set(center, sCameraTriggers[level][b].centerX, sCameraTriggers[level][b].centerY, sCameraTriggers[level][b].centerZ);
				vec3f_set(bounds, sCameraTriggers[level][b].boundsX, sCameraTriggers[level][b].boundsY, sCameraTriggers[level][b].boundsZ);

				// Check if mario is inside the bounds
				if(is_pos_in_bounds(sMarioCamState->pos, center, bounds, sCameraTriggers[level][b].boundsYaw) == TRUE)
				{
					...
					if(!(gameCameraFlag & CAM_FLAG_BLOCK_AREA_PROCESSING))
					{
	...
	if(!(gameCameraFlag & CAM_FLAG_BLOCK_AREA_PROCESSING))
	{
		switch(gCurrLevelArea)
		{
			...
			case AREA_BBH:
				...
				if(Crou_sameFV(sFixedModeBasePosition, 210.f, 420.f, 3109.f) == 1)
				{
			...
			case AREA_DDD_SUB:
				...
				{
					if(((sMarioCamState->action & ACT_FLAG_ON_POLE) != 0) || (sMarioGeometry.currFloorHeight > 800.f))
					{
						transition_to_camera_mode(c, CAMERA_MODE_8_DIRECTIONS, 60);
					}
					else
 ...
 * @param lastGood size, passed as the last position the camera was in
 */
...
s32 rotate_camera_around_walls(struct Camera* c, Vec3f cPos, s16* avoidYaw, s16 yawRange)
{
	...
	gameCameraFlag &= ~CAM_FLAG_CAM_NEAR_WALL;
	...
	for(step = 0; step < 8; step++)
	{
		...
		c_fchase_f(&coarseRadius, 250.f, 30.f * FRAME_RATE_SCALER);
		...
		if(find_wall_collisions(&colData) != 0)
		{
			...
			if(step >= 5)
			{
				...
				if(status <= 0)
				{
					...
					// degrees.
					*avoidYaw = Crou_Forcerot_s(yawFromMario, wallYaw) + DEGREES(180);
				}
			}
			...
			c_fchase_f(&fineRadius, 200.f, 20.f * FRAME_RATE_SCALER);
			...
			if(find_wall_collisions(&colData) != 0)
			{
				...
				if((is_range_behind_surface(sMarioCamState->pos, cPos, wall, yawRange, SURFACE_WALL_MISC) == 0) &&
				   ...
				   && (Crou_CalcPlaneArea(wall, -1.f, 150.f, -1.f) == FALSE))
				{
					// Calculate the avoid direction. The function returns the opposite direction so add 180
					// degrees.
					*avoidYaw = Crou_Forcerot_s(yawFromMario, wallYaw) + DEGREES(180);
					c_fchase_s(avoidYaw, horWallNorm, yawRange / FRAME_RATE_SCALER_INV);
					status = 3;
					step   = 8;
				}
			}
 ...
 *
 * Note: Also finds the water level, but waterHeight is size
 */
void find_mario_floor_and_ceil(struct PlayerGeometry* pg)
{
	...
	if(mcBGGroundCheck(sMarioCamState->pos[0], sMarioCamState->pos[1] + 10.f, sMarioCamState->pos[2], &surf) != -11000.f)
	{
	...
	gCheckingSurfaceCollisionsForCamera = FALSE;
	pg->currFloorHeight		    = mcBGGroundCheck(sMarioCamState->pos[0], sMarioCamState->pos[1] + 10.f, sMarioCamState->pos[2], &pg->currFloor);
...
s16 cameraDemoStratMsgNum(u8 cutscene, struct Object* o, s16 dialogID)
{
	s16 response = 0;
...
{
	s16 response = cameraDemoStratMsgNum(cutscene, o, -1);
	return response;
}
...
void stop_cutscene_and_retrieve_stored_info(struct Camera* c)
{
	...
	c->cutscene    = 0;
	CopyFVector(c->focus, sCameraStoreCutscene.focus);
	CopyFVector(c->pos, sCameraStoreCutscene.pos);
}
...
void player2_rotate_cam(struct Camera* c, s16 minPitch, s16 maxPitch, s16 minYaw, s16 maxYaw)
{
	...
	c_achase_s(&sCreditsPlayer2Yaw, -(s16)(sm64::player(1).controller().stickX * 250.f), 4 / FRAME_RATE_SCALER_INV);
	c_achase_s(&sCreditsPlayer2Pitch, -(s16)(sm64::player(1).controller().stickY * 265.f), 4 / FRAME_RATE_SCALER_INV);
	...
	SubFVector(sPlayer2FocusOffset, c->focus);
}
...
{
	CopyFVector(sCameraStoreCutscene.pos, c->pos);
	CopyFVector(sCameraStoreCutscene.focus, c->focus);
...
{
	Crou_DosplineZ(c->pos, sEndingFlyToWindowPos, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
	Crou_DosplineZ(c->focus, sEndingFlyToWindowFocus, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
}
...
CmdRet cutscene_ending_peach_appears(struct Camera* c)
{
	...
	c_achase_f(&c->pos[1], gCutsceneFocus->oPosY + 35.f, 0.02f * FRAME_RATE_SCALER);
	c_achase_f(&c->focus[1], gCutsceneFocus->oPosY + 125.f, 0.15f * FRAME_RATE_SCALER);
	player2_rotate_cam(c, -0x2000, 0x2000, -0x2000, 0x2000);
}
...
{
	Crou_DosplineZ(c->pos, sEndingPeachDescentCamPos, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
...
{
	c_fchase_f(&(sCutsceneVars[2].point[1]), 90.f, 0.5f * FRAME_RATE_SCALER);
}
...
CmdRet cutscene_ending_peach_descent_back_to_mario(struct Camera* c)
{
	...
	Crou_DosplineZ(pos, sEndingPeachDescentCamPos, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
	c->pos[0] = pos[0];
	c->pos[2] = pos[2];
	c_achase_f(&c->pos[1], (pos[1] += gCutsceneFocus->oPosY), 0.07f * FRAME_RATE_SCALER);
}
...
{
	cutscene_event(cutscene_ending_reset_spline, c, 0, 0);
	Crou_DosplineZ(c->pos, sEndingMarioToPeachPos, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
	Crou_DosplineZ(c->focus, sEndingMarioToPeachFocus, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
	player2_rotate_cam(c, -0x2000, 0x2000, -0x2000, 0x2000);
}
...
{
	Crou_DosplineZ(c->focus, sEndingLookUpAtCastle, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
}
...
CmdRet cutscene_ending_kiss_here_we_go(struct Camera* c)
{
	...
	//! another double typo
	c_achase_FVector(c->pos, pos, 0.2, 0.1f, 0.2f);
	c_achase_FVector(c->focus, foc, 0.2, 0.1f, 0.2f);
}
...
{
	Crou_DosplineZ(c->focus, sEndingLookAtSkyFocus, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
...
CmdRet cutscene_grand_star_front_of_mario(struct Camera* c)
{
	...
	vec3f_get_dist_and_angle(sMarioCamState->pos, c->pos, &dist, &pitch, &yaw);
	c_achase_f(&dist, goalDist, 0.1f * FRAME_RATE_SCALER);
	c_achase_s(&pitch, goalPitch, 32 / FRAME_RATE_SCALER_INV);
	c_achase_s(&yaw, goalYaw + 0x1200, 20 / FRAME_RATE_SCALER_INV);
...
{
	SetSVector(sCutsceneVars[0].angle, 0, sMarioCamState->faceAngle[1], 0);
...
{
	c_fchase_f(&sCutsceneVars[2].point[2], -40.f, 2.0f * FRAME_RATE_SCALER);
...
{
	c_fchase_f(&sCutsceneVars[2].point[2], 0.f, 2.f * FRAME_RATE_SCALER);
	sCutsceneVars[2].point[0] = 0.f;
	c_achase_f(&c->pos[0], sMarioCamState->pos[0], 0.01f * FRAME_RATE_SCALER);
	c_achase_f(&c->pos[2], sMarioCamState->pos[2], 0.01f * FRAME_RATE_SCALER);
}
...
CmdRet cutscene_grand_star_focus_mario(struct Camera* c)
{
	...
	c_achase_FVector(c->focus, foc, 0.5f, 0.8f, 0.5f);
}
...
{
	//! cvar7 is size in grand star
	vec3f_set(sCutsceneVars[7].point, 0.5f, 0.5f, 0.5f);
	//! cvar6 is size in grand star
...
CmdRet cutscene_grand_star_fly_move_to_mario(UNUSED struct Camera* c)
{
	...
	c_achase_FVector(sCutsceneVars[4].point, posOff, 0.05f, 0.05f, 0.05f);
	c_fchase_f(&sCutsceneVars[5].point[1], 0.f, 2.f * FRAME_RATE_SCALER);
	c_fchase_f(&sCutsceneVars[5].point[2], -200.f, 6.f * FRAME_RATE_SCALER);
}
...
{
	c_fchase_f(&sCutsceneVars[8].point[0], 15.f, 0.1f * FRAME_RATE_SCALER);

	c_fchase_f(&sCutsceneVars[4].point[0], -2000.f, sCutsceneVars[8].point[0] * FRAME_RATE_SCALER);
	c_fchase_f(&sCutsceneVars[4].point[1], 1200.f, sCutsceneVars[8].point[0] * FRAME_RATE_SCALER / 10.f);
	c_fchase_f(&sCutsceneVars[4].point[2], 1000.f, sCutsceneVars[8].point[0] * FRAME_RATE_SCALER / 10.f);

	c_fchase_f(&sCutsceneVars[5].point[0], 0.f, sCutsceneVars[8].point[0] * FRAME_RATE_SCALER);
	c_fchase_f(&sCutsceneVars[5].point[1], 1200.f, sCutsceneVars[8].point[0] / 2.f * FRAME_RATE_SCALER);
	c_fchase_f(&sCutsceneVars[5].point[2], 1000.f, sCutsceneVars[8].point[0] / 1.5f * FRAME_RATE_SCALER);
}
...
CmdRet cutscene_grand_star_fly_app_cvars(struct Camera* c)
{
	...
	c_fchase_f(&sCutsceneVars[8].point[2], 90.f, 2.5f * FRAME_RATE_SCALER);
	...
	c_fchase_f(&dist, 0, sCutsceneVars[8].point[2] * FRAME_RATE_SCALER);
	vec3f_set_dist_and_angle(goalPos, c->pos, dist, pitch, yaw);

	c_achase_FVector(c->pos, goalPos, 0.01f, 0.01f, 0.01f);
	c_achase_FVector(c->focus, goalFoc, 0.5f, 0.8f, 0.5f);
}
...
void focus_in_front_of_mario(struct Camera* c, f32 dist, f32 speed)
{
	...
	c_achase_FVector(c->focus, goalFocus, speed, speed, speed);
}
...
CmdRet cutscene_dance_move_to_mario(struct Camera* c)
{
	...
	vec3f_get_dist_and_angle(sMarioCamState->pos, c->pos, &dist, &pitch, &yaw);
	c_achase_f(&dist, 600.f, 0.3f * FRAME_RATE_SCALER);
	c_achase_s(&pitch, 0x1000, 0x10 / FRAME_RATE_SCALER_INV);
	vec3f_set_dist_and_angle(sMarioCamState->pos, c->pos, dist, pitch, yaw);
}
...
CmdRet cutscene_dance_default_rotate(struct Camera* c)
{
	...
	else
	{
		...
		if((sMarioCamState->action != ACT_STAR_DANCE_NO_EXIT) && (sMarioCamState->action != ACT_STAR_DANCE_WATER) && (sMarioCamState->action != ACT_STAR_DANCE_EXIT))
		{
			gCutsceneTimer = CUTSCENE_STOP;
			c->cutscene    = 0;
			transition_next_state(c, 20);
...
CmdRet cutscene_dance_closeup_start(struct Camera* c)
{
	...
	if((gLastCompletedStarNum == 4) && (activeCourseNo == COURSE_JRB))
	{
		star_dance_bound_yaw(c, 0x0, 0x4000);
	}
	if((gLastCompletedStarNum == 1) && (activeCourseNo == COURSE_DDD))
	{
		star_dance_bound_yaw(c, 0x8000, 0x5000);
	}
	if((gLastCompletedStarNum == 5) && (activeCourseNo == COURSE_WDW))
	{
		star_dance_bound_yaw(c, 0x8000, 0x800);
	}

	CopyFVector(sCutsceneVars[9].point, c->focus);
	//! cvar8 is size in the closeup cutscene
...
CmdRet cutscene_dance_closeup_focus_mario(struct Camera* c)
{
	...
	c_achase_FVector(sCutsceneVars[9].point, marioPos, 0.2f, 0.2f, 0.2f);
	CopyFVector(c->focus, sCutsceneVars[9].point);
}
...
CmdRet cutscene_dance_closeup_fly_above(struct Camera* c)
{
	...
	if((gLastCompletedStarNum == 6 && activeCourseNo == COURSE_SL) || (gLastCompletedStarNum == 4 && activeCourseNo == COURSE_TTC))
	{
		goalPitch = 0x800;
	}

	vec3f_get_dist_and_angle(sMarioCamState->pos, c->pos, &dist, &pitch, &yaw);
	c_achase_f(&dist, 800.f, 0.05f * FRAME_RATE_SCALER);
	c_achase_s(&pitch, goalPitch, 16 / FRAME_RATE_SCALER_INV);
	c_achase_s(&yaw, c->yaw, 8 / FRAME_RATE_SCALER_INV);
	vec3f_set_dist_and_angle(sMarioCamState->pos, c->pos, dist, pitch, yaw);
}
...
CmdRet cutscene_dance_closeup_fly_closer(struct Camera* c)
{
	...
	vec3f_get_dist_and_angle(sMarioCamState->pos, c->pos, &dist, &pitch, &yaw);
	c_achase_f(&dist, 240.f, 0.4f * FRAME_RATE_SCALER);
	c_achase_s(&yaw, c->yaw, 8 / FRAME_RATE_SCALER_INV);
	c_achase_s(&pitch, 0x1000, 5 / FRAME_RATE_SCALER_INV);
	vec3f_set_dist_and_angle(sMarioCamState->pos, c->pos, dist, pitch, yaw);
}
...
CmdRet cutscene_dance_closeup(struct Camera* c)
{
	...
	Viewswing(HAND_CAM_SHAKE_CUTSCENE);
}
...
CmdRet cutscene_dance_fly_away_start(struct Camera* c)
{
	...
	if((gLastCompletedStarNum == 6) && (activeCourseNo == COURSE_CCM))
	{
		star_dance_bound_yaw(c, 0x5600, 0x800);
	}
	if((gLastCompletedStarNum == 2) && (activeCourseNo == COURSE_TTM))
	{
		star_dance_bound_yaw(c, 0x0, 0x800);
	}
	if((gLastCompletedStarNum == 1) && (activeCourseNo == COURSE_SL))
	{
		star_dance_bound_yaw(c, 0x2000, 0x800);
	}
	if((gLastCompletedStarNum == 3) && (activeCourseNo == COURSE_RR))
	{
		star_dance_bound_yaw(c, 0x0, 0x800);
	}
}
...
CmdRet cutscene_dance_fly_away_approach_mario(struct Camera* c)
{
	...
	vec3f_get_dist_and_angle(sMarioCamState->pos, c->pos, &dist, &pitch, &yaw);
	c_achase_f(&dist, 600.f, 0.3f * FRAME_RATE_SCALER);
	c_achase_s(&pitch, 0x1000, 16 / FRAME_RATE_SCALER_INV);
	c_achase_s(&yaw, c->yaw, 8 / FRAME_RATE_SCALER_INV);
	vec3f_set_dist_and_angle(sMarioCamState->pos, c->pos, dist, pitch, yaw);
}
...
CmdRet cutscene_dance_fly_away(struct Camera* c)
{
	...
	set_fov_function(CAM_FOV_DEFAULT);
	Viewswing(HAND_CAM_SHAKE_STAR_DANCE);
}
...
{
	vec3f_set_dist_and_angle(sCutsceneVars[2].point, c->pos, sCutsceneVars[3].point[2], sCutsceneVars[3].angle[0], sCutsceneVars[3].angle[1]);
...
CmdRet cutscene_bowser_arena_focus_sine(UNUSED struct Camera* c)
{
	...
	sCutsceneVars[4].angle[1] -= 0x200 / FRAME_RATE_SCALER_INV;
	c_achase_f(&sCutsceneVars[0].point[1], yOff, 0.5f * FRAME_RATE_SCALER);
}
...
{
	c_achase_s(&sCutsceneVars[3].angle[0], 0x6C8, 30 / FRAME_RATE_SCALER_INV);
	c_achase_f(&sCutsceneVars[0].point[2], -200.f, 0.02f * FRAME_RATE_SCALER);
	c_achase_f(&sCutsceneVars[3].point[2], 550.f, 0.02f * FRAME_RATE_SCALER);
}
...
{
	c_achase_f(&sCutsceneVars[0].point[2], 0.f, 0.05f * FRAME_RATE_SCALER);
}
...
CmdRet cutscene_bowser_arena_start(struct Camera* c)
{
	...
	SetSVector(sCutsceneVars[2].angle, gSecondCameraFocus->oMoveAnglePitch, gSecondCameraFocus->oMoveAngleYaw, gSecondCameraFocus->oMoveAngleRoll);
...
CmdRet cutscene_star_spawn_focus_star(struct Camera* c)
{
	...
	{
		Srou_pos2FVector(starPos, gCutsceneFocus);
		starPos[1] += gCutsceneFocus->hitboxHeight;
		c_achase_FVector(c->focus, starPos, 0.1f, 0.1f, 0.1f);
	}
}
...
CmdRet cutscene_star_spawn_update_boss_fight(struct Camera* c)
{
	...
	c_achase_FVector(c->focus, focus, 0.2f, 0.2f, 0.2f);
	c_achase_FVector(c->pos, pos, 0.2f, 0.2f, 0.2f);
}
...
{
	if((c->mode == CAMERA_MODE_BOSS_FIGHT) && (SpecialCamera(0) == CAM_ANGLE_LAKITU))
	{
...
CmdRet cutscene_exit_to_castle_grounds_focus_mario(struct Camera* c)
{
	...
	c_achase_FVector(c->focus, sMarioCamState->pos, 0.05f, 0.4f, 0.05f);
}
...
{
	Srou_pos2FVector(sCutsceneVars[1].point, gCutsceneFocus);
	store_info_star(c);
...
{
	c_achase_f(&c->focus[0], gCutsceneFocus->oPosX, 0.15f * FRAME_RATE_SCALER);
	c_achase_f(&c->focus[2], gCutsceneFocus->oPosZ, 0.15f * FRAME_RATE_SCALER);
}
...
{
	c_achase_f(&c->focus[1], gCutsceneFocus->oPosY, 0.1f * FRAME_RATE_SCALER);
}
...
void cutscene_goto_cvar_pos(struct Camera* c, f32 goalDist, s16 goalPitch, s16 rotPitch, s16 rotYaw)
{
	...
	if((nextDist > 8000.f) && (c->cutscene == CUTSCENE_PREPARE_CANNON))
	{
		...
		nextPitch = goalPitch;
		CopyFVector(sCutsceneVars[0].point, sCutsceneVars[3].point);
		gameCameraFlag &= ~CAM_FLAG_SMOOTH_MOVEMENT;

		if(activeStageNo == LEVEL_TTM)
		{
	...
	else
	{
		if(c->cutscene == CUTSCENE_PREPARE_CANNON)
		{
			...
			c_achase_f(&curDist, cannonDist, 0.1f * FRAME_RATE_SCALER);
			c_achase_s(&curPitch, cannonPitch, 15 / FRAME_RATE_SCALER_INV);
			c_achase_s(&curYaw, cannonYaw, 15 / FRAME_RATE_SCALER_INV);
		...
		{
			c_achase_FVector(sCutsceneVars[0].point, sCutsceneVars[3].point, 0.1f, 0.1f, 0.1f);
		}
	}

	c_achase_f(&nextDist, goalDist, 0.05f * FRAME_RATE_SCALER);
	c_achase_s(&nextPitch, goalPitch, 0x20 / FRAME_RATE_SCALER_INV);

	vec3f_set_dist_and_angle(sCutsceneVars[3].point, c->pos, nextDist, nextPitch, nextYaw);
	CopyFVector(c->focus, sCutsceneVars[0].point);
...
{
	store_info_cannon(c);
	CopyFVector(sCutsceneVars[0].point, c->focus);
	sCutsceneVars[2].point[0] = 30.f;
	// Store the cannon door's position in sCutsceneVars[3]'s point
	Srou_pos2FVector(sCutsceneVars[3].point, gCutsceneFocus);
	SetSVector(sCutsceneVars[5].angle, 0, 0, 0);
}
...
CmdRet cutscene_prepare_cannon_fly_to_cannon(struct Camera* c)
{
	...
	c_fchase_s(&sCutsceneVars[5].angle[1], 0x400, 17 / FRAME_RATE_SCALER_INV);
	Viewswing(HAND_CAM_SHAKE_CUTSCENE);
}
...
CmdRet cutscene_prepare_cannon_fly_back(struct Camera* c)
{
	...
	{
		cannon_approach_prev(&c->pos[0], sCameraStoreCutscene.pos[0] * FRAME_RATE_SCALER_INV);
		cannon_approach_prev(&c->pos[1], sCameraStoreCutscene.pos[1] * FRAME_RATE_SCALER_INV);
		cannon_approach_prev(&c->pos[2], sCameraStoreCutscene.pos[2] * FRAME_RATE_SCALER_INV);
		cannon_approach_prev(&c->focus[0], sCameraStoreCutscene.focus[0] * FRAME_RATE_SCALER_INV);
		cannon_approach_prev(&c->focus[1], sCameraStoreCutscene.focus[1] * FRAME_RATE_SCALER_INV);
		cannon_approach_prev(&c->focus[2], sCameraStoreCutscene.focus[2] * FRAME_RATE_SCALER_INV);
	}
	else
	{
...
void water_death_move_to_mario_side(struct Camera* c)
{
	...
	vec3f_get_dist_and_angle(sMarioCamState->pos, c->pos, &dist, &pitch, &yaw);
	c_achase_s(&yaw, (sMarioCamState->faceAngle[1] - 0x3000), 8 / FRAME_RATE_SCALER_INV);
	vec3f_set_dist_and_angle(sMarioCamState->pos, c->pos, dist, pitch, yaw);
}
...
{
	CopyFVector(sCutsceneVars[0].point, c->focus);
	CopyFVector(sCutsceneVars[3].point, sMarioCamState->pos);
...
CmdRet cutscene_suffocation_fly_away(UNUSED struct Camera* c)
{
	...
	c_achase_FVector(sCutsceneVars[3].point, target, 0.1f, 0.1f, 0.1f);
}
...
CmdRet cutscene_suffocation_stay_above_gas(struct Camera* c)
{
	...
	cutscene_goto_cvar_pos(c, 400.f, 0x2800, 0x200, 0);
	gasLevel = mcGasCheck(sMarioCamState->pos[0], sMarioCamState->pos[2]);
...
CmdRet cutscene_pyramid_top_explode_start(struct Camera* c)
{
	...
	CopyFVector(sCutsceneVars[1].point, c->focus);
...
{
	c_achase_FVector(c->focus, sCutsceneVars[3].point, 0.02f, 0.02f, 0.02f);
	gameCameraFlag |= CAM_FLAG_SMOOTH_MOVEMENT;
}
...
CmdRet cutscene_pyramid_top_explode_warp(struct Camera* c)
{
	...
	CopyFVector(sCutsceneVars[4].point, c->pos);
	CopyFVector(sCutsceneVars[5].point, c->focus);
	CopyFVector(c->focus, sCutsceneVars[3].point);
...
CmdRet cutscene_pyramid_top_explode_closeup(struct Camera* c)
{
	...
	c_achase_f(&dist, 2000.f, 0.1f * FRAME_RATE_SCALER);
...
{
	Viewshake(SHAKE_ENV_PYRAMID_EXPLODE);
}
...
CmdRet cutscene_pyramid_top_explode_warp_back(struct Camera* c)
{
	...
	CopyFVector(c->pos, sCutsceneVars[4].point);
	CopyFVector(c->focus, sCutsceneVars[5].point);
	Viewshake(SHAKE_ENV_BOWSER_JUMP);
}

/**
 * An size cutscene for when the pyramid explodes.
 */
...
CmdRet cutscene_dialog_start(struct Camera* c)
{
	...
	s_begin_enemydemo(TIME_STOP_ENABLED | TIME_STOP_DIALOG);
	...
	CopyFVector(sCutsceneVars[8].point, sMarioCamState->pos);
	sCutsceneVars[8].point[1] += 125.f;

	// Store gCutsceneFocus's position and yaw
	Srou_pos2FVector(sCutsceneVars[9].point, gCutsceneFocus);
...
CmdRet cutscene_dialog_move_mario_shoulder(struct Camera* c)
{
	...
	c_achase_s(&yaw, sCutsceneVars[9].angle[1], 0x10 / FRAME_RATE_SCALER_INV);
	c_achase_f(&dist, 180.f, 0.05f * FRAME_RATE_SCALER);
	...
	c_achase_f(&c->pos[1], pos[1], 0.05f * FRAME_RATE_SCALER);
	c->pos[0] = pos[0];
	c->pos[2] = pos[2];
}
...
CmdRet cutscene_dialog_create_dialog_box(struct Camera* c)
{
	...
	{
		CallMessageYesNo(sCutsceneDialogID);
	}
	else
	{
		CallMessage(sCutsceneDialogID);
	}
...
CmdRet cutscene_dialog(struct Camera* c)
{
	...
	if((GetMessageNo() == -1) && (sCutsceneVars[8].angle[0] != 0))
	{
		if(c->cutscene != CUTSCENE_RACE_DIALOG)
		{
...
CmdRet cutscene_dialog_end(struct Camera* c)
{
	gameCameraFlag |= CAM_FLAG_UNUSED_CUTSCENE_ACTIVE;
	c->cutscene = 0;
	s_end_enemydemo(TIME_STOP_ENABLED | TIME_STOP_DIALOG);
}
...
static void unused_cam_to_mario(struct Camera* c)
{
	...
	SetSVector(dir, 0, sMarioCamState->faceAngle[1], 0);
...
{
	CopyFVector(sCutsceneVars[7].point, sMarioCamState->pos);
...
{
	c_achase_f(&sCutsceneVars[6].point[0], -24.f, 0.05f * FRAME_RATE_SCALER);
}
...
CmdRet cutscene_non_painting_death_start(UNUSED struct Camera* c)
{
	...
	// This is always overwritten, except in the size cutscene_exit_bowser_death()
 ...
 *
 * This cutscene is size, dying in bowser's arena spawns mario near the warp pipe, not back in the
 * hub.
 */
...
CmdRet cutscene_cap_switch_press_approach_mario(struct Camera* c)
{
	...
	vec3f_get_dist_and_angle(sMarioCamState->pos, c->pos, &dist, &pitch, &yaw);
	c_achase_f(&dist, 195.f, 0.2f * FRAME_RATE_SCALER);
	c_achase_s(&pitch, 0, 0x10 / FRAME_RATE_SCALER_INV);
	vec3f_set_dist_and_angle(sMarioCamState->pos, c->pos, dist, pitch, yaw);

	c_achase_f(&c->focus[0], sMarioCamState->pos[0], 0.1f * FRAME_RATE_SCALER);
	c_achase_f(&c->focus[1], sMarioCamState->pos[1] + 110.f, 0.1f * FRAME_RATE_SCALER);
	c_achase_f(&c->focus[2], sMarioCamState->pos[2], 0.1f * FRAME_RATE_SCALER);
}
...
CmdRet cutscene_cap_switch_press_pan_left(struct Camera* c)
{
	CopyFVector(c->focus, sMarioCamState->pos);
	c->focus[1] += 110.f;
	c_fchase_s(&sCutsceneVars[0].angle[1], 0x800, 0x20 / FRAME_RATE_SCALER_INV);
...
{
	CallMessageYesNo(gCutsceneFocus->oBehParams2ndByte + DIALOG_010);
}
...
CmdRet cutscene_cap_switch_press(struct Camera* c)
{
	...
	if((GetMessageNo() == -1) && (sCutsceneVars[4].angle[0] != 0))
	{
...
CmdRet cutscene_unlock_key_door_start(struct Camera* c)
{
	...
	CopyFVector(sCutsceneVars[0].point, c->pos);
	CopyFVector(sCutsceneVars[1].point, c->focus);
...
{
	c_achase_FVector(c->pos, sCutsceneVars[2].point, 0.1f, 0.1f, 0.1f);
	c_achase_FVector(c->focus, sCutsceneVars[3].point, 0.1f, 0.1f, 0.1f);
}
...
{
	c_achase_f(&sCutsceneVars[3].point[1], sMarioCamState->pos[1] + 140.f, 0.07f * FRAME_RATE_SCALER);
}
...
{
	c_achase_FVector(c->pos, sCutsceneVars[0].point, 0.1f, 0.1f, 0.1f);
	c_achase_FVector(c->focus, sCutsceneVars[1].point, 0.1f, 0.1f, 0.1f);
}
...
s32 intro_peach_move_camera_start_to_pipe(struct Camera* c, struct CutsceneSplinePoint positionSpline[], struct CutsceneSplinePoint focusSpline[])
{
	 ...
	 */
	posReturn   = Crou_DosplineZ(c->pos, positionSpline, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
	focusReturn = Crou_DosplineZ(c->focus, focusSpline, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
...
{
	CallMessage(DIALOG_020);
}

CmdRet play_sound_peach_reading_letter(UNUSED struct Camera* c)
{
	AudStartSound(SOUND_PEACH_DEAR_MARIO, gDefaultSoundArgs);
}
...
CmdRet cutscene_intro_peach_dialog(struct Camera* c)
{
	if(GetMessageNo() == -1)
	{
		CopyFVector(gLakituState.goalPos, c->pos);
		CopyFVector(gLakituState.goalFocus, c->focus);
		gameCameraFlag |= (CAM_FLAG_SMOOTH_MOVEMENT | CAM_FLAG_UNUSED_CUTSCENE_ACTIVE);
		gCutsceneTimer = CUTSCENE_STOP;
		c->cutscene    = 0;
	}
}
...
{
	Crou_DosplineZ(c->pos, sIntroPipeToDialogPosition, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
	Crou_DosplineZ(c->focus, sIntroPipeToDialogFocus, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
}
...
{
	Viewswing(HAND_CAM_SHAKE_OFF);
}

CmdRet intro_pipe_exit_text(UNUSED struct Camera* c)
{
	CallMessage(DIALOG_033);
}
...
CmdRet cutscene_intro_peach_fly_to_pipe(struct Camera* c)
{
	...
	Crou_CamAngLimit(c->pos, c->focus, 0x3B00, -0x3B00);
...
CmdRet cutscene_intro_peach_mario_appears(struct Camera* c)
{
	...
	c_achase_f(&sCutsceneVars[1].point[1], 80.f + sMarioGeometry.currFloorHeight + (sMarioCamState->pos[1] - sMarioGeometry.currFloorHeight) * 1.1f, 0.4f * FRAME_RATE_SCALER);
...
CmdRet cutscene_intro_peach_letter(struct Camera* c)
{
	...
	if((gCutsceneTimer > 120 * FRAME_RATE_SCALER_INV) && (GetMessageNo() == -1))
	{
...
struct CutsceneSplinePoint gIntroLakituStartToPipeFocus[] = {
    {0, 32, {58, -250, 346}},  {1, 50, {-159, -382, 224}}, {2, 37, {0, -277, 237}},   {3, 15, {1, -44, 245}},	   {4, 35, {0, -89, 228}},     {5, 15, {28, 3, 259}},	   {6, 25, {-38, -201, 371}},  {7, 20, {-642, 118, 652}},  {8, 25, {103, -90, 861}},
    {9, 25, {294, 145, 579}},  {10, 30, {220, -42, 500}},  {11, 20, {10, -134, 200}}, {12, 20, {-143, -145, 351}}, {13, 14, {-256, -65, 528}}, {14, 20, {-251, -52, 459}}, {15, 25, {-382, 520, 395}}, {16, 25, {-341, 240, 653}}, {17, 5, {-262, 700, 143}},
    {18, 15, {-760, 32, 27}},  {19, 20, {-756, -6, -26}},  {20, 20, {-613, 5, 424}},  {21, 20, {-22, -100, 312}},  {22, 25, {212, 80, 61}},    {23, 20, {230, -28, 230}},  {24, 35, {-83, -51, 303}},  {25, 17, {126, 90, 640}},   {26, 9, {158, 95, 763}},
    {27, 8, {113, -25, 1033}}, {28, 20, {57, -53, 1291}},  {29, 15, {73, -34, 1350}}, {30, 7, {0, 96, 1400}},	   {31, 8, {-59, 269, 1450}},  {32, 15, {57, 1705, 1500}}, {0, 15, {-227, 511, 1550}}, {-1, 15, {-227, 511, 1600}}};

struct CutsceneSplinePoint gIntroLakituStartToPipeOffsetFromCamera[] = {{0, 0, {-46, 87, -15}},	  {1, 0, {-38, 91, -11}},  {2, 0, {-31, 93, -13}},  {3, 0, {-50, 84, -16}}, {4, 0, {-52, 83, -17}}, {5, 0, {-10, 99, 3}},   {6, 0, {-54, 83, -10}},
									{7, 0, {-31, 85, -40}},	  {8, 0, {-34, 91, 19}},   {9, 0, {-9, 95, 28}},    {10, 0, {17, 72, 66}},  {11, 0, {88, -7, 45}},  {12, 0, {96, -6, -26}}, {13, 0, {56, -1, -82}},
									{14, 0, {40, 65, -63}},	  {15, 0, {-26, -3, -96}}, {16, 0, {92, 82, 19}},   {17, 0, {92, 32, 19}},  {18, 0, {92, 32, 19}},  {19, 0, {92, 102, 19}}, {20, 0, {-69, 59, -70}},
									{21, 0, {-77, 109, -61}}, {22, 0, {-87, 59, -46}}, {23, 0, {-99, -3, 11}},  {24, 0, {-99, -11, 5}}, {25, 0, {-97, -6, 19}}, {26, 0, {-97, 22, -7}}, {27, 0, {-98, -11, -13}},
									{28, 0, {-97, -11, 19}},  {29, 0, {-91, -11, 38}}, {30, 0, {-76, -11, 63}}, {31, 0, {-13, 33, 93}}, {32, 0, {51, -11, 84}}, {33, 0, {51, -11, 84}}, {-1, 0, {51, -11, 84}}};

struct CutsceneSplinePoint gEndWavingPos[] = {{0, 0, {-5, 975, -917}},	  {0, 0, {-5, 975, -917}},    {0, 0, {-5, 975, -917}},	  {0, 0, {-76, 1067, 742}},  {0, 0, {-105, 1576, 3240}},
					      {0, 0, {-177, 1709, 5586}}, {0, 0, {-177, 1709, 5586}}, {0, 0, {-177, 1709, 5586}}, {0, 0, {-177, 1709, 5586}}};

struct CutsceneSplinePoint gEndWavingFocus[] = {{0, 50, {18, 1013, -1415}},  {0, 100, {17, 1037, -1412}}, {0, 100, {16, 1061, -1408}}, {0, 100, {-54, 1053, 243}}, {0, 100, {-84, 1575, 2740}},
						{0, 50, {-156, 1718, 5086}}, {0, 0, {-156, 1718, 5086}},  {0, 0, {-156, 1718, 5086}},  {0, 0, {-156, 1718, 5086}}};

CmdRet cutscene_end_waving(struct Camera* c)
{
	cutscene_event(cutscene_end_waving_start, c, 0, 0);
	Crou_DosplineZ(c->pos, gEndWavingPos, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
	Crou_DosplineZ(c->focus, gEndWavingFocus, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
...
#define SET_CREDITS_SPLINE(casenum, spline)                                                                                                                                                                                                                    \
	...
	switch(gCurrLevelArea)
	{
		...
		case AREA_CCM_OUTSIDE:
			...
			if(BuGetStarFlag(activePlayerNo - 1, activeCourseNo - 1) & 0x10)
			{
...
#undef SET_CREDITS_SPLINE
	...
	Crou_DosplineZ(c->pos, sCurCreditsSplinePos, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
	Crou_DosplineZ(c->focus, sCurCreditsSplineFocus, &sCutsceneSplineSegment, &sCutsceneSplineSegmentProgress);
...
{
	CopyFVector(sCutsceneVars[1].point, sMarioCamState->pos);
...
{
	c_fchase_f(&sCutsceneVars[0].point[1], 75.f, 10.f * FRAME_RATE_SCALER);
}
...
{
	c_fchase_f(&sCutsceneVars[0].point[1], 125.f, 10.f * FRAME_RATE_SCALER);
}
...
CmdRet cutscene_sliding_doors_follow_mario(struct Camera* c)
{
	...
	c_achase_f(&sCutsceneVars[0].point[0], 0, 0.1f * FRAME_RATE_SCALER);
	c_fchase_f(&sCutsceneVars[0].point[2], 125.f, 50.f * FRAME_RATE_SCALER);
	// Update cvar0's angle
	c_achase_SVector(sCutsceneVars[0].angle, sMarioCamState->faceAngle, 16, 16, 16);

	// Apply the offset to the camera's position
	offset_rotated(pos, sCutsceneVars[1].point, sCutsceneVars[0].point, sCutsceneVars[0].angle);
	c_achase_FVector(c->pos, pos, 0.15f, 0.05f, 0.15f);
...
CmdRet cutscene_enter_painting(struct Camera* c)
{
	...
	if(ripplingPainting != NULL)
	{
		...
		c_achase_FVector(c->focus, focus, 0.1f, 0.1f, 0.1f);
		focusOffset[2] = -(((ripplingPainting->vSize * 1000.f) / 2.0f) / 307.f);
		offset_rotated(focus, paintingPos, focusOffset, paintingAngle);
		floorHeight = mcBGGroundCheck(focus[0], focus[1] + 500.f, focus[2], &highFloor) + 125.f;
		...
		{
			c_achase_FVector(c->pos, focus, 0.2f, 0.1f, 0.2f);
		}
		else
		{
			c_achase_FVector(c->pos, focus, 0.9f, 0.9f, 0.9f);
		}

		mcBGGroundCheck(sMarioCamState->pos[0], sMarioCamState->pos[1] + 50.f, sMarioCamState->pos[2], &floor);
...
CmdRet cutscene_exit_painting_start(struct Camera* c)
{
	...
	CopyFVector(sCutsceneVars[0].point, sMarioCamState->pos);
	sCutsceneVars[0].angle[0] = 0;
	...
	floorHeight = mcBGGroundCheck(c->pos[0], c->pos[1] + 10.f, c->pos[2], &floor);
...
CmdRet cutscene_exit_painting_move_to_mario(struct Camera* c)
{
	...
	c_achase_f(&sCutsceneVars[2].point[0], 178.f, 0.05f);
	c_achase_f(&sCutsceneVars[2].point[2], 889.f, 0.05f);
...
CmdRet cutscene_exit_painting_move_to_floor(struct Camera* c)
{
	...
	CopyFVector(floorHeight, sMarioCamState->pos);
	floorHeight[1] = mcBGGroundCheck(sMarioCamState->pos[0], sMarioCamState->pos[1] + 10.f, sMarioCamState->pos[2], &floor);

	if(floor != NULL)
	{
		floorHeight[1] = floorHeight[1] + (sMarioCamState->pos[1] - floorHeight[1]) * 0.7f + 125.f;
		c_achase_FVector(c->focus, floorHeight, 0.2f, 0.2f, 0.2f);

		if(floorHeight[1] < c->pos[1])
		{
			c_achase_f(&c->pos[1], floorHeight[1], 0.05f);
		}
	}
}
...
CmdRet cutscene_unused_exit_start(struct Camera* c)
{
	...
	SetSVector(marioAngle, 0, sMarioCamState->faceAngle[1], 0);
...
CmdRet cutscene_unused_exit_focus_mario(struct Camera* c)
{
	...
	set_focus_rel_mario(c, 0.f, 125.f, 0.f, 0);
	c_achase_FVector(c->focus, focus, 0.02f, 0.001f, 0.02f);
	update_camera_yaw(c);
}
...
CmdRet cutscene_enter_cannon_raise(struct Camera* c)
{
	...
	gameCameraFlag |= CAM_FLAG_SMOOTH_MOVEMENT;
	c_fchase_s(&sCutsceneVars[1].angle[0], 0, 0x80 / FRAME_RATE_SCALER_INV);
	c_fchase_s(&sCutsceneVars[2].angle[0], 0, 0x80 / FRAME_RATE_SCALER_INV);
	...
	sCutsceneVars[1].point[2] = approach_f32(sCutsceneVars[1].point[2], 400.f, 5.f * FRAME_RATE_SCALER, 5.f * FRAME_RATE_SCALER);
...
CmdRet cutscene_enter_cannon_start(struct Camera* c)
{
	...
	vec3f_get_dist_and_angle(sCutsceneVars[0].point, c->pos, &sCutsceneVars[1].point[2], &sCutsceneVars[1].angle[0], &sCutsceneVars[1].angle[1]);
	sCutsceneVars[3].point[1] = 0.f;
	//! cvar4 is size in this cutscene
...
{
	CopyFVector(c->pos, sCutsceneVars[0].point);
	CopyFVector(c->focus, sCutsceneVars[1].point);
}
...
CmdRet cutscene_door_move_behind_mario(struct Camera* c)
{
	...
	set_focus_rel_mario(c, 0.f, 125.f, 0.f, 0);
	SetSVector(sCutsceneVars[0].angle, 0, sMarioCamState->faceAngle[1] + doorRotation, 0);
...
CmdRet cutscene_door_follow_mario(struct Camera* c)
{
	...
	c_fchase_f(&dist, 150.f, 7.f * FRAME_RATE_SCALER);
	vec3f_set_dist_and_angle(c->focus, c->pos, dist, pitch, yaw);
	update_camera_yaw(c);
}
...
CmdRet cutscene_door_mode(struct Camera* c)
{
	...
	// Loop until mario is no longer using the door
	if(sMarioCamState->action != ACT_ENTERING_STAR_DOOR && sMarioCamState->action != ACT_PULLING_DOOR && sMarioCamState->action != ACT_PUSHING_DOOR)
	{
		gCutsceneTimer = CUTSCENE_STOP;
		c->cutscene    = 0;
	}
 ...
 */
struct Cutscene sCutsceneDoorPull[] = {{cutscene_door_start, 1}, {cutscene_door_fix_cam, 30}, {cutscene_door_move_behind_mario, 1}, {cutscene_door_follow_mario, 50}, {cutscene_door_end, 0}};

/**
 * Cutscene that plays when mario pushes open a door.
 */
struct Cutscene sCutsceneDoorPush[] = {{cutscene_door_start, 1}, {cutscene_door_fix_cam, 20}, {cutscene_door_move_behind_mario, 1}, {cutscene_door_follow_mario, 50}, {cutscene_door_end, 0}};

/**
 * Cutscene that plays when mario pulls open a door that has some special mode requirement on the other
 * side.
 */
struct Cutscene sCutsceneDoorPullMode[] = {{cutscene_door_start, 1}, {cutscene_door_fix_cam, 30}, {cutscene_door_mode, CUTSCENE_LOOP}};

/**
 * Cutscene that plays when mario pushes open a door that has some special mode requirement on the other
 * side.
 */
struct Cutscene sCutsceneDoorPushMode[] = {{cutscene_door_start, 1}, {cutscene_door_fix_cam, 20}, {cutscene_door_mode, CUTSCENE_LOOP}};

/**
 * Cutscene that plays when mario enters the cannon and it rises out of the hole.
 */
struct Cutscene sCutsceneEnterCannon[] = {{cutscene_enter_cannon_start, 1}, {cutscene_enter_cannon_raise, 121}, {cutscene_enter_cannon_end, 0}};

/**
 * Cutscene that plays when a star spawns from ie a box or after a boss fight.
 */
struct Cutscene sCutsceneStarSpawn[] = {{cutscene_star_spawn, CUTSCENE_LOOP}, {cutscene_star_spawn_back, 15}, {cutscene_star_spawn_end, 0}};

/**
 * Cutscene for the red coin star spawning. Compared to a regular star, this cutscene can warp long
 * distances.
 */
struct Cutscene sCutsceneRedCoinStarSpawn[] = {{cutscene_red_coin_star, CUTSCENE_LOOP}, {cutscene_red_coin_star_end, 0}};
...
struct Cutscene sCutsceneUnusedExit[] = {{cutscene_unused_exit_start, 1}, {cutscene_unused_exit_focus_mario, 60}, {cutscene_exit_painting_end, 0}};

/**
 * The intro of the game. Peach reads her letter and lakitu flies down to mario's warp pipe.
 */
struct Cutscene sCutsceneIntroPeach[] = {{cutscene_intro_peach_letter, CUTSCENE_LOOP}, {cutscene_intro_peach_reset_fov, 35}, {cutscene_intro_peach_fly_to_pipe, 820}, {cutscene_intro_peach_mario_appears, 270}, {cutscene_intro_peach_dialog, CUTSCENE_LOOP}};
 ...
 */
struct Cutscene sCutsceneExitWaterfall[] = {{cutscene_exit_waterfall, 52}, {cutscene_exit_to_castle_grounds_end, 0}};

/**
 * Cutscene that plays when mario falls from WMOTR.
 */
struct Cutscene sCutsceneFallToCastleGrounds[] = {{cutscene_exit_fall_to_castle_grounds, 73}, {cutscene_exit_to_castle_grounds_end, 0}};

/**
 * Cutscene that plays when mario enters the pyramid through the hole at the top.
 */
struct Cutscene sCutsceneEnterPyramidTop[] = {{cutscene_enter_pyramid_top, 90}, {cutscene_exit_to_castle_grounds_end, 0}};

/**
 * Unused cutscene for when the pyramid explodes.
 */
struct Cutscene sCutscenePyramidTopExplode[] = {{cutscene_mario_dialog, CUTSCENE_LOOP}, {cutscene_pyramid_top_explode, 150}, {cutscene_pyramid_top_explode_end, 0}};
 ...
 */
struct Cutscene sCutsceneEnterBowserArena[] = {{cutscene_bowser_arena, 180}, {cutscene_bowser_arena_dialog, CUTSCENE_LOOP}, {cutscene_bowser_arena_end, 0}};

// The dance cutscenes are automatically stopped since InitGameCamera() is called after mario warps.
 ...
 */
struct Cutscene sCutsceneExitSpecialSuccess[] = {{cutscene_exit_non_painting_succ, 163}, {cutscene_non_painting_end, 0}};

/**
 * Cutscene that plays when mario exits from dying in a non-painting course, like HMC.
 */
struct Cutscene sCutsceneNonPaintingDeath[] = {{cutscene_non_painting_death, 120}, {cutscene_non_painting_end, 0}};

/**
 * Cutscene that plays when mario talks to a creature.
 */
struct Cutscene sCutsceneDialog[] = {{cutscene_dialog, CUTSCENE_LOOP}, {cutscene_dialog_set_flag, 12}, {cutscene_dialog_end, 0}};

/**
 * Cutscene that plays when mario reads a sign or message.
 */
struct Cutscene sCutsceneReadMessage[] = {{cutscene_read_message, CUTSCENE_LOOP}, {cutscene_read_message_set_flag, 15}, {cutscene_read_message_end, 0}};
 ...
 */
s32 cutscene_event(CameraEvent event, struct Camera* c, const Duration start, const Duration end)
{
 ...
 */
s32 cutscene_spawn_obj(s32 obj, Duration frame)
{
...
{
	amplitude = Crou_AnglePerCamDist(amplitude, maxDist, posX, posY, posZ);

	if(amplitude != 0)
	{
		if(amplitude > sFOVState.shakeAmplitude)
		{ // literally use the function above you silly nintendo, smh
			sFOVState.shakeAmplitude = amplitude;
			sFOVState.decay		 = decay;
			sFOVState.shakeSpeed	 = shakeSpeed;
		}
...
void shake_camera_fov(struct GraphNodePerspective* perspective)
{
	if(sFOVState.shakeAmplitude != 0.f)
	{
		...
		c_fchase_f(&sFOVState.shakeAmplitude, 0.f, sFOVState.decay * FRAME_RATE_SCALER);
...
static UNUSED void unused_deactivate_sleeping_camera(UNUSED struct PlayerRecord* m)
{
	gameCameraFlag &= ~CAM_FLAG_SLEEPING;
}

void set_fov_30(UNUSED struct PlayerRecord* m)
{
	sFOVState.fov = 30.f;
}

void approach_fov_20(UNUSED struct PlayerRecord* m)
{
	c_fchase_f(&sFOVState.fov, 20.f, 0.3f);
}

void set_fov_45(UNUSED struct PlayerRecord* m)
{
	sFOVState.fov = 45.f;
}

void set_fov_29(UNUSED struct PlayerRecord* m)
{
	sFOVState.fov = 29.f;
}

void zoom_fov_30(UNUSED struct PlayerRecord* m)
{
	// Pretty sure c_achase_f would do a much nicer job here, but you do you,
	// Nintendo.
	c_fchase_f(&sFOVState.fov, 30.f, ((30.f - sFOVState.fov) / 60.f) * FRAME_RATE_SCALER);
}
 ...
 * mario falls a sleep.
 */
void fov_default(struct PlayerRecord* m)
{
	gameCameraFlag &= ~CAM_FLAG_SLEEPING;

	if((m->status == ACT_SLEEPING) || (m->status == ACT_START_SLEEPING))
	{
		c_fchase_f(&sFOVState.fov, 30.f, ((30.f - sFOVState.fov) / 30.f) * FRAME_RATE_SCALER);
		gameCameraFlag |= CAM_FLAG_SLEEPING;
	}
	else
	{
		c_fchase_f(&sFOVState.fov, 45.f, ((45.f - sFOVState.fov) / 30.f) * FRAME_RATE_SCALER);
		sFOVState.unusedIsSleeping = 0;
	}
...
static UNUSED void unused_approach_fov_30(UNUSED struct PlayerRecord* m)
{
	c_fchase_f(&sFOVState.fov, 30.f, 1.f * FRAME_RATE_SCALER);
}

void approach_fov_30(UNUSED struct PlayerRecord* m)
{
	c_fchase_f(&sFOVState.fov, 30.f, 1.f * FRAME_RATE_SCALER);
}

void approach_fov_60(UNUSED struct PlayerRecord* m)
{
	c_fchase_f(&sFOVState.fov, 60.f, 1.f * FRAME_RATE_SCALER);
}

void approach_fov_45(struct PlayerRecord* m)
{
	...
	sFOVState.fov = approach_f32(sFOVState.fov, targetFoV, 2.f * FRAME_RATE_SCALER, 2.f * FRAME_RATE_SCALER);
}

void approach_fov_80(UNUSED struct PlayerRecord* m)
{
	c_fchase_f(&sFOVState.fov, 80.f, 3.5f * FRAME_RATE_SCALER);
}
 ...
 */
void set_fov_bbh(struct PlayerRecord* m)
{
	f32 targetFoV = sFOVState.fov;
...
void obj_rotate_towards_point(struct Object* o, Vec3f point, s16 pitchOff, s16 yawOff, s16 pitchDiv, s16 yawDiv)
{
	...
	Srou_pos2FVector(oPos, o);
	vec3f_get_dist_and_angle(oPos, point, &dist, &pitch, &yaw);
	o->oMoveAnglePitch = c_Achase_s(o->oMoveAnglePitch, pitchOff - pitch, pitchDiv);
	o->oMoveAngleYaw   = c_Achase_s(o->oMoveAngleYaw, yaw + yawOff, yawDiv);
}
