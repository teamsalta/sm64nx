...
const void* sTextureTransitionID[] = {
...
s32 render_textured_transition(s8 fadeTimer, s8 transTime, struct WarpTransitionData* transData, s8 texID, s8 transTexType)
{
	...
	Vtx* verts	   = (Vtx*)AllocDynamic(8 * sizeof(*verts));
	...
	if(verts != NULL)
	{
		...
		{
			case TRANS_TYPE_MIRROR:
				gDPLoadTextureBlock(gDisplayListHead++, sTextureTransitionID[texID], G_IM_FMT_IA, G_IM_SIZ_8b, 32, 64, 0, G_TX_WRAP | G_TX_MIRROR, G_TX_WRAP | G_TX_MIRROR, 5, 6, G_TX_NOLOD, G_TX_NOLOD);
				break;
			case TRANS_TYPE_CLAMP:
				gDPLoadTextureBlock(gDisplayListHead++, sTextureTransitionID[texID], G_IM_FMT_IA, G_IM_SIZ_8b, 64, 64, 0, G_TX_CLAMP, G_TX_CLAMP, 6, 6, G_TX_NOLOD, G_TX_NOLOD);
				break;
		}
...
int Wipe(s8 fadeTimer, s8 transType, u8 transTime, struct WarpTransitionData* transData)
{
	switch(transType)
	{
...
{
	Vtx* verts = (Vtx*)AllocDynamic(8 * sizeof(*verts));
	Gfx* dlist = (Gfx*)AllocDynamic(20 * sizeof(*dlist));
	Gfx* g	   = dlist;
...
Gfx* geo_cannon_circle_base(s32 callContext, struct GraphNode* node, UNUSED f32 mtx[4][4])
{
	...
	if(callContext == GEO_CONTEXT_RENDER && snSceneInfo != NULL && snSceneInfo->camera->mode == CAMERA_MODE_INSIDE_CANNON)
	{
