...
void envfx_update_flower(Vec3s centerPos)
{
	...
	for(i = 0; i < sBubbleParticleMaxCount; i++)
	{
		...
		if((gEnvFxBuffer + i)->isAlive == 0)
		{
			...
			(gEnvFxBuffer + i)->yPos      = GroundCheck((gEnvFxBuffer + i)->xPos, 10000.0f, (gEnvFxBuffer + i)->zPos, &floorGeo);
...
s32 envfx_is_jestream_bubble_alive(s32 index)
{
	...
	if(!particle_is_laterally_close(index, gEnvFxBubbleConfig[ENVFX_STATE_SRC_X], gEnvFxBubbleConfig[ENVFX_STATE_SRC_Z], 1000) || gEnvFxBubbleConfig[ENVFX_STATE_SRC_Y] + 1500 < (gEnvFxBuffer + index)->yPos)
	{
		return 0;
	}
...
s32 envfx_init_bubble(s32 mode)
{
	...
	gEnvFxBuffer = (EnvFxParticle*)mem_pool_alloc(gEffectsMemoryPool, sBubbleParticleCount * sizeof(struct EnvFxParticle));
	if(!gEnvFxBuffer)
	{
		return 0;
	}
