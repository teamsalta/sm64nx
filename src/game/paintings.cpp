...
struct Painting* insideCastlePaintings[] = {
    &bob_painting, &ccm_painting, &wf_painting, &jrb_painting, &lll_painting, &ssl_painting, &hmc_painting, &ddd_painting, &wdw_painting, &thi_tiny_painting, &ttm_painting, &ttc_painting, &sl_painting, &thi_huge_painting, NULL,
};
...
void stopAllRippleExcept(s16* idptr, struct Painting* paintingGroup[])
{
	...
	{
		struct Painting* painting = (Painting*)segmented_to_virtual(paintingGroup[index]);
		if(painting->id != id)
		{
...
float find_mario_y_position_on_painting(struct Painting* painting)
{
	...
	else if(marioYOffsetFromPainting > painting->vSize)
	{ // If Mario is above the top, return the top
...
void painting_set_ripple_type(s8 intendedStatus, struct Painting* painting, struct Painting* paintingGroup[], s8 hRippleSpot, s8 vRippleSpot, s8 resetTimer)
{
...
void vertical_proximity_ripple_painting_ripple(
    ...
    struct Painting* paintingGroup[]) // For paintings aligned vertically that follow RIPPLE_TRIGGER_PROXIMITY, set
...
void vertical_continuous_ripple_painting_ripple(struct Painting* painting, struct Painting* paintingGroup[]) // For paintings aligned vertically that follow RIPPLE_TRIGGER_CONTINUOUS (DDD only), set some flags depending on where Mario is
{
	if(painting->floorEntered & STAND_LEFT)
	{
...
void vertical_continuous_ripple_painting_ripple_if_mario_enters(
    struct Painting* painting,
    struct Painting* paintingGroup[]) // For paintings aligned vertically that follow RIPPLE_TRIGGER_CONTINUOUS (DDD
...
void horizontal_proximity_ripple_painting_ripple(
    struct Painting* painting,
    struct Painting* paintingGroup[]) // For paintings aligned horizontally that follow RIPPLE_TRIGGER_PROXIMITY
...
void horizontal_continuous_ripple_painting_ripple(
    struct Painting* painting,
    struct Painting* paintingGroup[]) // For paintings aligned horizontally that follow RIPPLE_TRIGGER_CONTINUOUS
...
void painting_update_floors(struct Painting* painting)
{
	...
	{ // check if Mario's current floor is one of the
	  // special floors
	...
	painting->floorEntered = (painting->lastFloor ^ painting->currFloor) & painting->currFloor;			      // floorEntered is true iff currFloor is true and lastFloor is false
	...
	painting->marioNewlyUnderPainting = (painting->lastMarioUnderPainting ^ painting->currMarioUnderPainting) & painting->currMarioUnderPainting; // Again, marioNewlyUnderPainting is true iff he is under it
...
void painting_update_ripple_status(struct Painting* painting)
{
	if(gPaintingUpdateCounter != gLastPaintingUpdateCounter)
	{
		...
		painting->rippleTimer += 1.0; //! After ~6.47 days, paintings with RIPPLE_TRIGGER_CONTINUOUS will increment this to
	...
	{
		if(painting->rippleStatus == RIPPLE_STATE_ENTRY && painting->currRippleMag <= painting->passiveRippleMag) // if the painting is doing the entry ripple but the
...
s16 painting_calculate_point_ripple(
    struct Painting* painting, float xpos,
		...
		// face of the painting, not actual axes
{
	...
	float rippleRate       = painting->currRippleRate * FRAME_RATE_SCALER;
	...
	scaledDistance	     = distanceToRippleSpot / dispersionFactor;								 // scale distance by dispersion factor so that ripples
...
s16 painting_conditionally_calculate_point_ripple(struct Painting* painting, s16 condition, s16 xpos, s16 ypos)
{
	s16 rippleHeight = 0;
...
void Print1(struct Painting* painting, s16* b, s16 c)
{
	...
	D_8035FFA0 = (Thing*)mem_pool_alloc(gEffectsMemoryPool, c * sizeof(struct Thing));
	if(D_8035FFA0 == NULL)
	{
	}
...
void Print2(s16* a, s16 b, s16 c)
{
	...
	D_8035FFA4 = (Vec3f*)mem_pool_alloc(gEffectsMemoryPool, c * sizeof(Vec3f));
	if(D_8035FFA4 == NULL)
	{
	}
...
void* func_802D3CF0(u8* img, s16 tWidth, s16 tHeight, s16* d, s16 e, s16 f, u8 g)
{
	...
	gDPTileSync(sp7C++);
	gDPSetTile(sp7C++, G_IM_FMT_RGBA, G_IM_SIZ_16b, 0, 0, G_TX_LOADTILE, 0, G_TX_WRAP | G_TX_NOMIRROR, G_TX_NOMASK, G_TX_NOLOD, G_TX_WRAP | G_TX_NOMIRROR, G_TX_NOMASK, G_TX_NOLOD);
	gDPLoadSync(sp7C++);
...
Gfx* func_802D45FC(struct Painting* painting)
{
	...
	{
		sp5C = (s16*)segmented_to_virtual(meshArray[sp62]);
		sp66 = sp5C[0];
		sp64 = sp5C[sp66 * 3 + 1];
...
Gfx* func_802D4874(struct Painting* painting)
{
	...
	gSPDisplayList(sp44++, painting->displayList68);
	sp58 = (s16*)segmented_to_virtual(meshArray[0]);
	sp5E = sp58[0];
	sp5C = sp58[sp5E * 3 + 1];
...
{
	s16* sp34 = (s16*)segmented_to_virtual(seg2_triangle_mesh);
	s16* sp30 = (s16*)segmented_to_virtual(seg2_mesh_order);
	s16 sp2E  = sp34[0];
...
{
	if(painting->rippleTrigger == RIPPLE_TRIGGER_PROXIMITY) // make the painting ripple using a different function based on its
...
Gfx* Geo18_802D5B98(s32 run, struct GraphNode* node, UNUSED void* context)
{
	...
	struct Painting* painting	= (Painting*)segmented_to_virtual(paintingGroup[id]);
	...
	{
		if(sp28 == 1 && id == PAINTING_ID_DDD)
		{ // painting is DDD painting
