...
u32 main_pool_push_state(void)
{
	...
	gMainPoolState		  = (MainPoolState*)main_pool_alloc(sizeof(*gMainPoolState), MEMORY_POOL_LEFT);
...
u32 main_pool_pop_state(void)
{
	...
	gMainPoolState = (MainPoolState*)gMainPoolState->prev;
	return sPoolFreeSpace;
}
...
struct AllocOnlyPool* alloc_only_pool_resize(struct AllocOnlyPool* pool, u32 size)
{
	...
	size	= ALIGN4(size);
	newPool = (AllocOnlyPool*)main_pool_realloc(pool, size + sizeof(struct AllocOnlyPool));
	if(newPool != NULL)
	{
...
{
	struct MarioAnimDmaRelatedThing* sp1C = (MarioAnimDmaRelatedThing*)dynamic_dma_read(srcAddr, srcAddr + sizeof(u32), MEMORY_POOL_LEFT);
	u32 size			      = sizeof(u32) + (sizeof(u8*) - sizeof(u32)) + sizeof(u8*) + sp1C->count * sizeof(struct OffsetSizePair);
	main_pool_free(sp1C);

	sp1C	      = (MarioAnimDmaRelatedThing*)dynamic_dma_read(srcAddr, srcAddr + size, MEMORY_POOL_LEFT);
