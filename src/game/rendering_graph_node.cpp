 ...
 * First the root of the scene graph is processed when DrawScene
...
u8 gCurAnimEnabled;
s16 gCurrAnimFrame;
s32 gCurrAnimFrameAssist;
s32 gCurrAnimFrameAccel;
Animation gCurrentAnim;
s32 gCurrAnimId;
...
static void geo_append_display_list(void* displayList, s16 layer)
{
	...
	{
		struct DisplayListNode* listNode = (DisplayListNode*)alloc_only_pool_alloc(gDisplayListHeap, sizeof(struct DisplayListNode));
...
static void geo_process_camera(struct GraphNodeCamera* node)
{
	...
	Mtx* rollMtx = (Mtx*)AllocDynamic(sizeof(*rollMtx));
	Mtx* mtx     = (Mtx*)AllocDynamic(sizeof(*mtx));
...
static void geo_process_background(struct GraphNodeBackground* node)
{
	...
	{
		Gfx* gfxStart = (Gfx*)AllocDynamic(sizeof(Gfx) * 8);
		Gfx* gfx      = gfxStart;

		gDPPipeSync(gfx++);
		gDPSetCycleType(gfx++, G_CYC_FILL);
		gDPSetFillColor(gfx++, node->background);
		gDPFillRectangle(gfx++, GFX_DIMENSIONS_RECT_FROM_LEFT_EDGE(0), BORDER_HEIGHT, GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(0) - 1, SCREEN_HEIGHT - BORDER_HEIGHT - 1);
		gDPPipeSync(gfx++);
...
static s16 interpolateAngles(s16 a1, s16 a2, u16 ratio)
{
	const s32 diff = s16(a1 - a2);

	return a1 - s16((diff * ratio) / 0xFFFF);
}

s16 interpolatePositions(s16 a1, s16 a2, u16 ratio)
{
	return (a1 * (0xFFFF - ratio) / 0xFFFF) + (a2 * ratio / 0xFFFF);
}
...
static void geo_process_animated_part(struct GraphNodeAnimatedPart* node)
{
	...
	Vec3s rotation;
	Vec3f translation;
	Mtx* matrixPtr = (Mtx*)AllocDynamic(sizeof(*matrixPtr));

	u16 partial = GET_LOW_U16_OF_32(gCurrAnimFrameAssist);

	u8 interpolate = partial != 0;
	s16 nextFrame  = gCurrAnimFrame + (gCurrAnimFrameAccel > 0 ? 1 : -1);

	if(nextFrame >= gCurrentAnim.lastFrame)
	{
		if(gCurrentAnim.flags & Animation::Flags::NOLOOP)
		{
			// nextFrame = gCurrAnimFrame;
			interpolate = 0;
		}
		else
		{
	...
	else if(nextFrame < 0)
	{
		...
		{
			nextFrame = gCurrentAnim.lastFrame - 1;
		}
	}
	...
	if(gCurAnimType == ANIM_TYPE_TRANSLATION)
	{
		if(interpolate)
		{
			u16* tmpAnimAttribute = gCurrAnimAttribute;
			auto x1		      = gCurAnimData[retrieve_animation_index(gCurrAnimFrame, &gCurrAnimAttribute)] * gCurAnimTranslationMultiplier;
			auto y1		      = gCurAnimData[retrieve_animation_index(gCurrAnimFrame, &gCurrAnimAttribute)] * gCurAnimTranslationMultiplier;
			auto z1		      = gCurAnimData[retrieve_animation_index(gCurrAnimFrame, &gCurrAnimAttribute)] * gCurAnimTranslationMultiplier;

			auto x2 = gCurAnimData[retrieve_animation_index(nextFrame, &tmpAnimAttribute)] * gCurAnimTranslationMultiplier;
			auto y2 = gCurAnimData[retrieve_animation_index(nextFrame, &tmpAnimAttribute)] * gCurAnimTranslationMultiplier;
			auto z2 = gCurAnimData[retrieve_animation_index(nextFrame, &tmpAnimAttribute)] * gCurAnimTranslationMultiplier;

			translation[0] += interpolatePositions(x1, x2, partial);
			translation[1] += interpolatePositions(y1, y2, partial);
			translation[2] += interpolatePositions(z1, z2, partial);
		}
		else
		{
	...
	if(gCurAnimType == ANIM_TYPE_ROTATION)
	{
		...
		{
			u16* tmpAnimAttribute = gCurrAnimAttribute;
			auto x1		      = gCurAnimData[retrieve_animation_index(gCurrAnimFrame, &gCurrAnimAttribute)];
			auto y1		      = gCurAnimData[retrieve_animation_index(gCurrAnimFrame, &gCurrAnimAttribute)];
			auto z1		      = gCurAnimData[retrieve_animation_index(gCurrAnimFrame, &gCurrAnimAttribute)];

			auto x2 = gCurAnimData[retrieve_animation_index(nextFrame, &tmpAnimAttribute)];
			auto y2 = gCurAnimData[retrieve_animation_index(nextFrame, &tmpAnimAttribute)];
			auto z2 = gCurAnimData[retrieve_animation_index(nextFrame, &tmpAnimAttribute)];

			rotation[0] = interpolateAngles(x1, x2, partial);
			rotation[1] = interpolateAngles(y1, y2, partial);
			rotation[2] = interpolateAngles(z1, z2, partial);
		}
		else
		{
...
void geo_set_animation_globals(struct GraphNodeObject_sub* node, s32 hasAnimation)
{
	...
	if(hasAnimation != 0)
	{
		geo_update_animation_frame(node, &node->animFrameAccelAssist);
	}

	node->animTimer = animationCounter;
	...
	gCurrAnimFrame	     = node->frame();
	gCurrAnimFrameAssist = node->animFrameAccelAssist;
	gCurrAnimId	     = node->animID;
	gCurrAnimFrameAccel  = node->animAccel;

	if(gCurrAnimFrame < 0)
	{
		gCurrAnimFrame	     = 0;
		gCurrAnimFrameAssist = 0;
	}

	if(node->curAnim)
	{
		gCurrentAnim = *node->curAnim;
	}

	if(node->animID == MARIO_ANIM_RUNNING)
	{
		// gCurAnimInterpolate = 0;
		auto timer = node->animTimer;
		int blah   = 0;
	}
	...
	if(anim->yTransDivisor == 0)
	{
		gCurAnimTranslationMultiplier = 1.0f;
	}
	else
	{
		gCurAnimTranslationMultiplier = (f32)node->animYTrans / (f32)anim->yTransDivisor;
	}
}
...
static void geo_process_shadow(struct GraphNodeShadow* node)
{
	...
	if(gCurGraphNodeCamera != NULL && gCurGraphNodeObject != NULL)
	{
		if(gCurGraphNodeHeldObject != NULL)
		{
			get_pos_from_transform_mtx(shadowPos, gMatStack[gMatStackIndex], (const f32(*)[4])gCurGraphNodeCamera->matrixPtr);
			shadowScale = node->shadowScale;
		}
		else
		{
			CopyFVector(shadowPos, gCurGraphNodeObject->pos);
		...
		if(shadowList != NULL)
		{
			...
			mtxf_mul(gMatStack[gMatStackIndex], mtxf, (const f32(*)[4])gCurGraphNodeCamera->matrixPtr);
			mtxf_to_mtx(mtx, gMatStack[gMatStackIndex]);
			gMatStackFixed[gMatStackIndex] = mtx;
...
static void geo_process_object(struct Object* node)
{
	...
	if(node->header.gfx.unk18 == gCurGraphNodeRoot->areaIndex)
	{
		...
		{
			mtxf_mul(gMatStack[gMatStackIndex + 1], (const f32(*)[4])node->header.gfx.throwMatrix, gMatStack[gMatStackIndex]);
		}
...
void geo_process_held_object(struct GraphNodeHeldObject* node)
{
	...
	if(node->objNode != NULL && node->objNode->header.gfx.sharedChild != NULL)
	{
		...
		mtxf_copy(gMatStack[gMatStackIndex + 1], (const f32(*)[4])gCurGraphNodeObject->throwMatrix);
		...
		gCurAnimType			    = 0;
		gCurGraphNodeHeldObject		    = (GraphNodeHeldObject*)node;
 ...
 */
void DrawScene(struct GraphNodeRoot* node, Vp* b, Vp* c, s32 clearColor)
{
	UNUSED s32 unused;
	...
	{
		Mtx* initialMatrix;
		Vp* viewport = (Vp*)AllocDynamic(sizeof(*viewport));

		gDisplayListHeap = alloc_only_pool_init(main_pool_available() - sizeof(struct AllocOnlyPool), MEMORY_POOL_LEFT);
		initialMatrix	 = (Mtx*)AllocDynamic(sizeof(*initialMatrix));
		gMatStackIndex	 = 0;
		gCurAnimType	 = 0;

		short3_set(viewport->vp.vtrans, node->x * 4, node->y * 4, 511);
		short3_set(viewport->vp.vscale, node->width * 4, node->height * 4, 511);

		if(b != NULL)
		{
			SoftwareBlanking(clearColor);
			SetTrimmingRectangle(b);
			*viewport = *b;
		}
