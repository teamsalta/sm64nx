...
static void obj_perform_position_op(s32 op)
{
	switch(op)
	{
		...
		case POS_OP_COMPUTE_VELOCITY:
			o->oVelX = (o->oPosX - sObjSavedPosX) * FRAME_RATE_SCALER_INV;
			o->oVelY = (o->oPosY - sObjSavedPosY) * FRAME_RATE_SCALER_INV;
			o->oVelZ = (o->oPosZ - sObjSavedPosZ) * FRAME_RATE_SCALER_INV;
			break;
...
static void platform_on_track_update_pos_or_spawn_ball(s32 ballIndex, f32 x, f32 y, f32 z)
{
	...
	if(ballIndex == 0 || ((u16)(o->oBehParams >> 16) & 0x0080))
	{
		...
		if(ballIndex != 0)
		{
			amountToMove = 300.0f * ballIndex * FRAME_RATE_SCALER;
		}
		else
		{
			obj_perform_position_op(POS_OP_SAVE_POSITION);
			o->oPlatformOnTrackPrevWaypointFlags = 0;
			amountToMove			     = o->oForwardVel * FRAME_RATE_SCALER;
		}
...
static s32 func_802F9378(s8 arg0, s8 arg1, u32 sound)
{
	...
	if((val04 = o->header.gfx.unk38.animAccel / (0x10000 * FRAME_RATE_SCALER_INV)) <= 0)
	{
		val04 = 1;
	}
...
static void obj_update_blinking(s32* blinkTimer, s16 baseCycleLength, s16 cycleLengthRange, s16 blinkLength)
{
	...
	{
		*blinkTimer = random_linear_offset(baseCycleLength, cycleLengthRange) * FRAME_RATE_SCALER_INV;
	}
...
static void obj_act_knockback(UNUSED f32 baseScale)
{
	...
	if((o->oMoveFlags & (OBJ_MOVE_MASK_ON_GROUND | OBJ_MOVE_MASK_IN_WATER | OBJ_MOVE_HIT_WALL | OBJ_MOVE_ABOVE_LAVA)) || (o->oAction == OBJ_ACT_VERTICAL_KNOCKBACK && o->oTimer >= 9 * FRAME_RATE_SCALER_INV))
	{
		obj_die_if_health_non_positive();
	}
...
void obj_spit_bouncing_fire(s16 relativePosX, s16 relativePosY, s16 relativePosZ, f32 scale, s32 model, f32 startSpeed, f32 endSpeed, s16 movePitch)
{
	...
	if(obj != NULL)
	{
		obj->oBouncingFireBallFlameSpeedX   = 50.0f;
		obj->oBouncingFireBallFlameSpeedY   = 50.0f;
		obj->oSmallPiranhaFlameModel	    = model;
