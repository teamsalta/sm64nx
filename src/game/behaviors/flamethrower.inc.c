...
void bhv_flamethrower_flame_loop(void)
{
	...
	if(o->oBehParams2ndByte == 2)
		...
		size = o->oTimer * FRAME_RATE_SCALER * (o->oForwardVel - 20.0) / 100.0 + 1.0;
...
void bhv_flamethrower_loop(void)
{
	...
	{
		if(o->oTimer % FRAME_RATE_SCALER_INV == 0)
		{
			...
			else if(o->oTimer < 74 * FRAME_RATE_SCALER_INV)
			{
				sp34 = 75 - (o->oTimer / FRAME_RATE_SCALER_INV); // Range: [15..2]
			}
			else
			{
				o->oAction++;
...
{
	o->oMoveAngleYaw -= 0x80 / FRAME_RATE_SCALER_INV;
