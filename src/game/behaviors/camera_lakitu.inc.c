...
/**
 * Behavior for sm64::bhv::bhvCameraLakitu(). This includes both the intro lakitu and the
 * lakitu visible in the mirror room.
 * TODO: Processing order relative to sm64::bhv::bhvCloud()
 */
			...
			else
			{
				...
				if(o->oDistanceToMario < 1000.0f)
				{
					...
					if(o->oDistanceToMario < 500.0f && abs_angle_diff(gMarioObject->oFaceAngleYaw, o->oFaceAngleYaw) > 0x7000)
					{
						...
						// to a stop
						approach_f32_ptr(&o->oCameraLakituSpeed, 0.0f, 5.0f * FRAME_RATE_SCALER);
					}
				}
			}
		}
