/**
 * Behavior for sm64::bhv::bhvAlphaBooKey() and sm64::bhv::bhvBetaBooKey().
 ...
 * BBH and the castle courtyard. It is used in a macro preset with sm64::bhv::bhvAlphaBooKey(),
 * which is also grouped near other boo/BBH-related macros. This is evidence that
 * sm64::bhv::bhvAlphaBooKey() was supposed to be a key. sm64::bhv::bhvBetaBooKey() has code similar to
 * sm64::bhv::bhvAlphaBooKey()'s for rotation and collection, and functions correctly when
 ...
 * sm64::bhv::bhvAlphaBooKey() was abandoned for reasons unknown and replaced with sm64::bhv::bhvBetaBooKey().
 */

/**
 * Update function for sm64::bhv::bhvAlphaBooKey().
 * It rotates the key, and deletes it when collected.
 * The code in this function is similar to that found in
 * sm64::bhv::bhvBetaBooKey() code, which implies that these are 2 versions
 * of the same object. It is a less developed version of
 * sm64::bhv::bhvBetaBooKey(), hence the "alpha" moniker.
 */
...
static void beta_boo_key_dropped_loop(void)
{
	...
	{
		o->oGraphYOffset += 2.0f * FRAME_RATE_SCALER;
	}
...
static void beta_boo_key_inside_boo_loop(void)
{
	...
	o->oPosY += 40.0f * FRAME_RATE_SCALER;
...
/**
 * Update function for sm64::bhv::bhvBetaBooKey().
 */
