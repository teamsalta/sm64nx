...
/**
 * Behavior for sm64::bhv::bhvWigglerHead() and sm64::bhv::bhvWigglerBody().
 * The sm64::bhv::bhvWigglerHead() object controls the wiggler's behavior, and physically manifests
 * as the wiggler's head. The sm64::bhv::bhvWigglerBody() objects represent the 3 tail body
 * parts, numbered 1 closest to the head, and 3 at the end of the tail.
 * Processing order is sm64::bhv::bhvWigglerHead(), then sm64::bhv::bhvWigglerBody() 1, 2, then 3.
 */
...
/**
 * Update function for sm64::bhv::bhvWigglerBody().
...
static void wiggler_init_segments(void)
{
	...
	segments = (ChainSegment*)mem_pool_alloc(gObjectMemoryPool, 4 * sizeof(struct ChainSegment));
	...
	else
	{
		...
		else
		{
			...
			if(obj_bounce_off_walls_edges_objects(&o->oWigglerTargetYaw))
			{
				...
				o->oWigglerWalkAwayFromWallTimer = random_linear_offset(30, 30) * FRAME_RATE_SCALER_INV;
			}
			else
			{
				if(o->oHealth < 4)
...
static void wiggler_act_jumped_on(void)
{
	...
	if(approach_f32_ptr(&o->oWigglerSquishSpeed, 0.0f, 0.05f * FRAME_RATE_SCALER))
	{
		// Note that 4 is the default scale
		approach_f32_ptr(&o->header.gfx.scale[1], 4.0f, 0.2f * FRAME_RATE_SCALER);
	}
	else
	{
		o->header.gfx.scale[1] -= o->oWigglerSquishSpeed * FRAME_RATE_SCALER;
	}
...
static void wiggler_act_knockback(void)
{
	if(o->oVelY > 0.0f)
	{
		o->oFaceAnglePitch -= o->oVelY * 30.0f * FRAME_RATE_SCALER;
	}
	else
	{
		obj_face_pitch_approach(0, 0x190 / FRAME_RATE_SCALER_INV);
	}

	if(obj_forward_vel_approach(0.0f, 1.0f * FRAME_RATE_SCALER) && o->oFaceAnglePitch == 0)
	{
		o->oAction	 = WIGGLER_ACT_WALK;
		o->oMoveAngleYaw = o->oFaceAngleYaw;
	}
...
/**
 * Update function for sm64::bhv::bhvWigglerHead().
 */
