 ...
 */
static const void* sPlatformOnTrackCollisionModels[] = {
 ...
 */
static const void* sPlatformOnTrackPaths[] = {
    rr_seg7_trajectory_0702EC3C,  rr_seg7_trajectory_0702ECC0,	ccm_seg7_trajectory_0701669C, bitfs_seg7_trajectory_070159AC, hmc_seg7_trajectory_0702B86C,
    lll_seg7_trajectory_0702856C, lll_seg7_trajectory_07028660, rr_seg7_trajectory_0702ED9C,  rr_seg7_trajectory_0702EEE0,
};
...
void bhv_platform_on_track_init(void)
{
	if(!(o->activeFlags & ACTIVE_FLAG_IN_DIFFERENT_ROOM))
	{
		...
		o->oPlatformOnTrackStartWaypoint = (Waypoint*)segmented_to_virtual(sPlatformOnTrackPaths[pathIndex]);
...
static void platform_on_track_act_move_along_track(void)
{
	...
	if(o->oPlatformOnTrackPrevWaypointFlags == WAYPOINT_FLAGS_END && !((u16)(o->oBehParams >> 16) & PLATFORM_ON_TRACK_BP_RETURN_TO_START))
	{
	...
	else
	{
		...
		else
		{
			...
			if(!o->oPlatformOnTrackIsNotSkiLift)
			{
				obj_forward_vel_approach(10.0, 0.1f * FRAME_RATE_SCALER);
			}
			else
			{
				o->oForwardVel = 10.0f;
			...
			if(approach_f32_ptr(&o->oPlatformOnTrackDistMovedSinceLastBall, 300.0f, o->oForwardVel * FRAME_RATE_SCALER))
			{
				o->oPlatformOnTrackDistMovedSinceLastBall -= 300.0f * FRAME_RATE_SCALER;
		...
		if(!((u16)(o->oBehParams >> 16) & PLATFORM_ON_TRACK_BP_DONT_TURN_YAW))
		{
			...
			obj_face_yaw_approach(targetFaceYaw, yawSpeed * FRAME_RATE_SCALER);
			o->oAngleVelYaw = ((s16)o->oFaceAngleYaw - initialAngle) * FRAME_RATE_SCALER_INV;
		}
		...
		if(((u16)(o->oBehParams >> 16) & PLATFORM_ON_TRACK_BP_DONT_TURN_ROLL))
		{
			...
			obj_face_roll_approach(o->oMoveAnglePitch, rollSpeed * FRAME_RATE_SCALER);
...
static void platform_on_track_rock_ski_lift(void)
{
	...
	o->oFaceAngleRoll += (s32)o->oPlatformOnTrackSkiLiftRollVel / FRAME_RATE_SCALER_INV;

	// Tilt away from the moving direction and toward mario
	if(gMarioObject->platform == o)
	{
		targetRoll = o->oForwardVel * sins(o->oMoveAngleYaw) * -50.0f + (s32)(o->oDistanceToMario * sins(o->oAngleToMario - o->oFaceAngleYaw) * -4.0f);
	}
...
void bhv_platform_on_track_update(void)
{
	...
	else if(o->oPlatformOnTrackType == PLATFORM_ON_TRACK_TYPE_CARPET)
	{
		...
		approach_f32_ptr(&o->oPlatformOnTrackOffsetY, 0.0f, 0.5f * FRAME_RATE_SCALER);
		o->oPosY += o->oPlatformOnTrackOffsetY / FRAME_RATE_SCALER_INV;
	}
}
