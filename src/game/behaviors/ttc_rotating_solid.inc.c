 ...
 */
static const void* sTTCRotatingSolidCollisionModels[] = {
...
void bhv_ttc_rotating_solid_update(void)
{
	...
	if(gTTCSpeedSetting != TTC_SPEED_STOPPED && o->oTimer > o->oTTCRotatingSolidRotationDelay * FRAME_RATE_SCALER_INV)
	{
		if(o->oTTCRotatingSolidSoundTimer != 0)
		{
			...
			if(--o->oTTCRotatingSolidSoundTimer == 0)
			{
				objsound(SOUND_GENERAL2_ROTATING_BLOCK_ALERT);
			}
		}
		else if(o->oTTCRotatingSolidVelY > 0.0f && o->oPosY >= o->oHomeY)
		{
			...
			obj_face_roll_approach(targetRoll, 0x4B0 / FRAME_RATE_SCALER_INV);

			o->oAngleVelRoll = o->oFaceAngleRoll - startRoll;

			if(o->oAngleVelRoll == 0)
			{
				objsound(SOUND_GENERAL2_ROTATING_BLOCK_CLICK);
				...
				{
					o->oTTCRotatingSolidRotationDelay = random_mod_offset(5, 20, 7) * FRAME_RATE_SCALER_INV;
				}
			}
		}
		else
		{
			...
			o->oTTCRotatingSolidVelY += 0.5f * FRAME_RATE_SCALER;

			if((o->oPosY += o->oTTCRotatingSolidVelY * FRAME_RATE_SCALER) >= o->oHomeY)
			{
				o->oPosY		       = o->oHomeY;
				o->oTTCRotatingSolidSoundTimer = 6 * FRAME_RATE_SCALER_INV;
			}
		}
	}
	else
