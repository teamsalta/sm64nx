...
void CheckWaterRingCollection(f32 avgScale, struct Object* ringManager)
{
	...
	if(!is_point_close_to_object(o, gMarioObject->header.gfx.pos[0], gMarioObject->header.gfx.pos[1] + 80.0f, gMarioObject->header.gfx.pos[2], (avgScale + 0.2) * 120.0))
	{
	...
	if(o->oWaterRingMarioDistInFront * marioDistInFront < 0)
	{
		...
		if(ringSpawner)
		{
			if((o->oWaterRingIndex == ringManager->oWaterRingMgrLastRingCollected + 1) || (ringSpawner->oWaterRingSpawnerRingsCollected == 0))
			{
...
void SetWaterRingScale(f32 avgScale)
{
	...
	o->oWaterRingScalePhaseX += 0x1700 / FRAME_RATE_SCALER_INV;
	o->oWaterRingScalePhaseY += 0x1700 / FRAME_RATE_SCALER_INV;
	o->oWaterRingScalePhaseZ += 0x1700 / FRAME_RATE_SCALER_INV;
}

void WaterRingCollectedLoop(void)
{
	f32 avgScale = (f32)(o->oTimer / FRAME_RATE_SCALER_INV) * 0.2 + o->oWaterRingAvgScale;
...
{
	f32 avgScale = (f32)(o->oTimer / FRAME_RATE_SCALER_INV) / 225.0 * 3.0 + 0.5;
	...
	if(o->oTimer >= 226 * FRAME_RATE_SCALER_INV)
	{
...
{
	f32 avgScale		   = (f32)o->oTimer * FRAME_RATE_SCALER / 50.0f * 1.3 + 0.1;
	struct Object* ringSpawner = o->parentObj;
	struct Object* ringManager = ringSpawner->parentObj;
