...
void snufit_act_0(void)
{
	...
	{
		o->oSnufitUnk100 += 400 / FRAME_RATE_SCALER_INV;
	}
}
...
void bhv_snufit_loop(void)
{
	if(!(o->activeFlags & 0x0008))
	{
		...
		{
			obj_turn_pitch_toward_mario(120.0f, 2000 / FRAME_RATE_SCALER_INV);
		...
		{
			obj_move_pitch_approach(0, 0x200 / FRAME_RATE_SCALER_INV);
			o->oMoveAngleYaw += 200 / FRAME_RATE_SCALER_INV;
		}
