...
{
	o->oForwardVel = 0.0f;
	if(approach_f32_ptr(&o->header.gfx.scale[0], 1.5f, 0.02f * FRAME_RATE_SCALER))
	{
		...
		else
		{
			...
			if(o->oFlyGuyIdleTimer >= 3 * FRAME_RATE_SCALER_INV || o->oFlyGuyIdleTimer == ((RandomU16() & 1) + 2) * FRAME_RATE_SCALER_INV)
			{
...
static void fly_guy_act_approach_mario(void)
{
	...
	else if(obj_forward_vel_approach(0.0f, 0.2f * FRAME_RATE_SCALER))
	{
...
static void fly_guy_act_lunge(void)
{
	...
	else
	{
		...
		obj_face_pitch_approach(0, 0x100 / FRAME_RATE_SCALER_INV);
		obj_face_roll_approach(o->oFlyGuyTargetRoll, 0x12C / FRAME_RATE_SCALER_INV);

		// Twirl in a spiral with curvature proportional to oFaceAngleRoll
		o->oMoveAngleYaw -= o->oFaceAngleRoll * FRAME_RATE_SCALER / 4;
		...
		{
			obj_y_vel_approach(20.0f, 0.5f * FRAME_RATE_SCALER);
		}
		else if(obj_y_vel_approach(0.0f, 0.5f * FRAME_RATE_SCALER))
		{
...
static void fly_guy_act_shoot_fire(void)
{
	...
	if(obj_face_yaw_approach(o->oAngleToMario, 0x800 / FRAME_RATE_SCALER_INV))
	{
...
void bhv_fly_guy_update(void)
{
	...
	if(!(o->activeFlags & ACTIVE_FLAG_IN_DIFFERENT_ROOM))
	{
		...
		o->oFlyGuyOscTimer += 1;
		o->oPosY += coss(0x400 * o->oFlyGuyOscTimer) * 1.5f * FRAME_RATE_SCALER;
