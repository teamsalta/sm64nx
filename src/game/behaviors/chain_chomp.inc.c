...
/**
 * Behavior for sm64::bhv::bhvChainChomp(), sm64::bhv::bhvChainChompChainPart(), sm64::bhv::bhvWoodenPost(), and sm64::bhv::bhvChainChompGate().
 * sm64::bhv::bhvChainChomp() spawns its sm64::bhv::bhvWoodenPost() in its behavior script. It spawns 5 chain
 ...
 * Processing order is sm64::bhv::bhvWoodenPost(), sm64::bhv::bhvChainChompGate(), sm64::bhv::bhvChainChomp(), sm64::bhv::bhvChainChompChainPart().
...
static void chain_chomp_act_uninitialized(void)
{
	...
	{
		segments = (ChainSegment*)mem_pool_alloc(gObjectMemoryPool, 5 * sizeof(struct ChainSegment));
		if(segments != NULL)
		{
...
static void chain_chomp_update_chain_segments(void)
{
	...
	for(i = 1; i <= 4; i++)
	{
		...
		if((segment->posY += segmentVelY * FRAME_RATE_SCALER) < 0.0f)
		{
			segment->posY = 0.0f;
		}
...
{
	approach_f32_ptr(&o->oChainChompMaxDistFromPivotPerChainPart, 750.0f / 5, 4.0f * FRAME_RATE_SCALER);
...
static void chain_chomp_sub_act_turn(void)
{
	...
	obj_move_pitch_approach(0, 0x100 / FRAME_RATE_SCALER_INV);
...
static void chain_chomp_sub_act_lunge(void)
{
	...
	obj_face_pitch_approach(o->oChainChompTargetPitch, 0x400 / FRAME_RATE_SCALER_INV);
	...
	else
	{
		...
		{
			approach_f32_ptr(&o->oChainChompUnk104, 0.0f, 0.8f * FRAME_RATE_SCALER);
		}
		else
		{
...
static void chain_chomp_act_move(void)
{
	...
	if(o->oChainChompReleaseStatus == CHAIN_CHOMP_NOT_RELEASED && o->oDistanceToMario > 4000.0f && !sm64::config().camera().disableDistanceClip())
	{
		o->oAction     = CHAIN_CHOMP_ACT_UNLOAD_CHAIN;
		o->oForwardVel = o->oVelY = 0.0f;
	}
