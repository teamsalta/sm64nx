...
/**
 * Behavior for sm64::bhv::bhvPokey() and sm64::bhv::bhvPokeyBodyPart().
 * sm64::bhv::bhvPokey() is responsible for the behavior of the pokey itself, as well as
...
void bhv_pokey_body_part_update(void)
{
	...
	if(obj_update_standard_actions(3.0f))
	{
		...
		else
		{
			...
			{
				o->parentObj->oPokeyAliveBodyPartFlags = o->parentObj->oPokeyAliveBodyPartFlags | 1 << (o->oBehParams2ndByte - 1);

				o->parentObj->oPokeyAliveBodyPartFlags = o->parentObj->oPokeyAliveBodyPartFlags & ((1 << o->oBehParams2ndByte) ^ 0xFFFFFFFF);
			...
			else
			{
				...
				if(!(o->oPokeyTurningAwayFromWall = obj_bounce_off_walls_edges_objects(&o->oPokeyTargetYaw)))
				{
					...
					else if(o->oDistanceToMario > 2000.0f)
					{
						...
						o->oPokeyChangeTargetTimer = random_linear_offset(30, 50) * FRAME_RATE_SCALER_INV;
					}
					else
					{
