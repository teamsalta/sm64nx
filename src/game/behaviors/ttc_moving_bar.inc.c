...
void bhv_ttc_moving_bar_init(void)
{
	...
	// bars
	o->oTTCMovingBarStoppedTimer = 10 * o->oBehParams2ndByte * FRAME_RATE_SCALER_INV;
...
{
	if(o->oTTCMovingBarDelay != 0 && o->oTimer > o->oTTCMovingBarDelay * FRAME_RATE_SCALER_INV)
	{
		...
		else
		{
			if(gTTCSpeedSetting == TTC_SPEED_RANDOM)
			{
				...
				if(RandomU16() % 2 == 0)
				{
					o->oTTCMovingBarStoppedTimer = random_linear_offset(20, 100) * FRAME_RATE_SCALER_INV;
				}
			}
...
static void ttc_moving_bar_act_pull_back(void)
{
	...
	if((o->oTTCMovingBarSpeed += 0.73f * FRAME_RATE_SCALER) > 0.0f)
	{
...
static void ttc_moving_bar_act_extend(void)
{
	...
	if((o->oTTCMovingBarOffset == 250.0f || (250.0f - o->oTTCMovingBarOffset) * (250.0f - o->oTTCMovingBarStartOffset) < 0.0f) && o->oTTCMovingBarSpeed > -8.0f && o->oTTCMovingBarSpeed < 8.0f)
	{
		// Begin retracting
	...
	else
	{
		...
		o->oTTCMovingBarSpeed += accel * FRAME_RATE_SCALER;

		// When we pass neutral on random setting, then stop immediately with
		// 25% probability (fake out)
		if(gTTCSpeedSetting == TTC_SPEED_RANDOM && o->oTTCMovingBarOffset * o->oTTCMovingBarStartOffset < 0.0f && RandomU16() % 4 == 0)
		{
...
void bhv_ttc_moving_bar_update(void)
{
	...
	o->oTTCMovingBarOffset += o->oTTCMovingBarSpeed * FRAME_RATE_SCALER;
