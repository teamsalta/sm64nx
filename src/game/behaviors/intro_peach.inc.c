...
void intro_peach_set_pos_and_opacity(struct Object* o, f32 targetOpacity, f32 increment)
{
	...
	Srou_FVector2pos(o, newPos);
	newOpacity = o->oOpacity;
	c_fchase_f(&newOpacity, targetOpacity, increment * FRAME_RATE_SCALER);
...
void bhv_intro_peach_loop(void)
{
	switch(gCurrentObject->oAction)
	{
		case 0:
			...
			gCurrentObject->header.gfx.unk38.setFrameRaw(100);
			break;
		case 1:
			intro_peach_set_pos_and_opacity(gCurrentObject, 0.f, 0.f);

			if(gCurrentObject->oTimer * FRAME_RATE_SCALER_INV > 20)
				gCurrentObject->oAction += 1;
			break;
		case 2:
