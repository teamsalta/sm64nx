...
void bhv_ttc_pit_block_init(void)
{
	...
	{
		o->oPosY += 330.0f * FRAME_RATE_SCALER;
	}
}
...
void bhv_ttc_pit_block_update(void)
{
	if(o->oTimer > o->oTTCPitBlockWaitTime * FRAME_RATE_SCALER_INV)
	{
		...
		if(clamp_f32(&o->oPosY, o->oHomeY, o->oTTCPitBlockPeakY))
		{
			...
			if((o->oTTCPitBlockWaitTime = sTTCPitBlockProperties[gTTCSpeedSetting][o->oTTCPitBlockDir & 0x1].waitTime * FRAME_RATE_SCALER_INV) < 0)
			{
