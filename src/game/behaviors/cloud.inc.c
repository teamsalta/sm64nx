...
/**
 * Behavior for sm64::bhv::bhvCloud() and sm64::bhv::bhvCloudPart().
 * sm64::bhv::bhvCloud() includes both fwoosh and the cloud that lakitu rides (both nice and
 * evil).
 * sm64::bhv::bhvCloudPart() is spawned by sm64::bhv::bhvCloud() and is either a "chunk" of cloud, or fwoosh's
...
static void cloud_fwoosh_update(void)
{
	...
	else
	{
		if(o->oCloudBlowing)
		{
			o->header.gfx.scale[0] += o->oCloudGrowSpeed * FRAME_RATE_SCALER;

			if((o->oCloudGrowSpeed -= 0.005f * FRAME_RATE_SCALER) < -0.16f)
			{
		...
		else
		{
			...
			approach_f32_ptr(&o->header.gfx.scale[0], 3.0f, 0.012f * FRAME_RATE_SCALER);
			o->oCloudFwooshMovementRadius += 0xC8 / FRAME_RATE_SCALER_INV;
...
/**
 * Main update function for sm64::bhv::bhvCloud(). This controls the cloud's movement, when it
...
/**
 * Update function for sm64::bhv::bhvCloud().
 */
...
/**
 * Update function for sm64::bhv::bhvCloudPart(). Follow the parent cloud with some oscillation.
 */
