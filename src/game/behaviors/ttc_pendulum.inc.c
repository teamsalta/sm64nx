...
void bhv_ttc_pendulum_update(void)
{
	if(gTTCSpeedSetting != TTC_SPEED_STOPPED)
	{
		...
		if(o->oTTCPendulumSoundTimer != 0)
		{
			if(--o->oTTCPendulumSoundTimer == 0)
			{
				objsound(SOUND_GENERAL_PENDULUM_SWING);
			}
		}
		...
		else
		{
			...
			o->oTTCPendulumAngleVel += (o->oTTCPendulumAngleAccel * o->oTTCPendulumAccelDir) * FRAME_RATE_SCALER;
			...
			if(o->oTTCPendulumAngleVel == 0.0f)
			{
				...
				o->oTTCPendulumSoundTimer = (o->oTTCPendulumDelay + 15) * FRAME_RATE_SCALER_INV;
			}

			o->oTTCPendulumAngle += o->oTTCPendulumAngleVel * FRAME_RATE_SCALER;
		}
	}
	else
	{
