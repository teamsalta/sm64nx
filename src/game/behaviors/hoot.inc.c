...
void HootFloorBounce(void)
{
	...
	if(floorY + 75.0f > o->oPosY)
		o->oMoveAnglePitch -= 3640.8888 * FRAME_RATE_SCALER;

	floorY = HootFindNextFloor(&sp1c, 200.0f);
	if(floorY + 125.0f > o->oPosY)
		o->oMoveAnglePitch -= 7281.7776 * FRAME_RATE_SCALER;
...
void HootFreeStep(s16 fastOscY, s32 speed)
{
	...
	if(fastOscY == 0)
		o->oPosY -= (o->oVelY + coss((s32)(sp26 * 3276.8)) * 50.0f / 4.0f) * FRAME_RATE_SCALER;
	else
		o->oPosY -= (o->oVelY + coss((s32)(sp26 * 6553.6)) * 50.0f / 4.0f) * FRAME_RATE_SCALER;
...
void PlayerSetHootYaw(void)
{
	...
	o->oMoveAngleYaw -= (5 * stickX) / FRAME_RATE_SCALER_INV;
}
...
void HootCarryStep(s32 speed, UNUSED f32 xPrev, UNUSED f32 zPrev)
{
	s16 yaw	  = o->oMoveAngleYaw;
	s16 pitch = o->oMoveAnglePitch;
	s16 sp22  = o->header.gfx.unk38.frame();
	f32 hSpeed;
	...
	o->oPosX += o->oVelX * FRAME_RATE_SCALER;
	o->oPosY -= (o->oVelY + coss((s32)(sp22 * 6553.6)) * 50.0f / 4.0f) * FRAME_RATE_SCALER;
	o->oPosZ += o->oVelZ * FRAME_RATE_SCALER;
