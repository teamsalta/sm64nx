...
/**
 * Behavior for sm64::bhv::bhvKoopa() and sm64::bhv::bhvKoopaRaceEndpoint().
 * sm64::bhv::bhvKoopa() includes normal, unshelled, tiny, and Koopa the Quick.
 * When the race begins, koopa the quick sets his parent to sm64::bhv::bhvKoopaRaceEndpoint()
 ...
 */
static struct KoopaTheQuickProperties sKoopaTheQuickProperties[] = {{DIALOG_005, DIALOG_007, bob_seg7_trajectory_koopa, {3030, 4500, -4600}}, {DIALOG_009, DIALOG_031, thi_seg7_trajectory_koopa, {7100, -1300, -6000}}};
...
{
	if(o->oKoopaDistanceToMario < 300.0f && s_calc_dangle(o->oKoopaAngleToMario, o->oMoveAngleYaw) < 0x3000)
	{
...
static void koopa_shelled_act_walk(void)
{
	...
	else
	{
		...
		s_chase_angleY(o->oKoopaTargetYaw, 0x200 / FRAME_RATE_SCALER_INV);
	}
...
static void koopa_shelled_act_run_from_mario(void)
{
	...
	if(o->oTimer > 30 * FRAME_RATE_SCALER_INV && o->oDistanceToMario > 800.0f)
	{
	...
	else
	{
		...
		obj_forward_vel_approach(17.0f, 1.0f * FRAME_RATE_SCALER);
	}
}
...
static void koopa_unshelled_act_run(void)
{
	...
	else
	{
		...
		shell = s_search_nearobject(sm64::bhv::bhvKoopaShell(), &distToShell);
		if(shell != NULL)
		{
		...
		if(o->oDistanceToMario > 800.0f || (shell != NULL && s_calc_dangle(o->oKoopaTargetYaw, o->oAngleToMario + 0x8000) < 0x2000))
		{
			// then turn toward the shell
			s_chase_angleY(o->oKoopaTargetYaw, 0x600 / FRAME_RATE_SCALER_INV);
		}
		else
		{
			// otherwise continue running from mario
			s_chase_angleY(o->oAngleToMario + 0x8000, 0x600 / FRAME_RATE_SCALER_INV);
		}
	}

	// If we think we have a shot, dive for the shell
	if(obj_forward_vel_approach(20.0f, 1.0f) && distToShell < 600.0f && s_calc_dangle(o->oKoopaTargetYaw, o->oMoveAngleYaw) < 0xC00)
	{
...
static s32 koopa_the_quick_detect_bowling_ball(void)
{
	...
	ball = s_search_nearobject(sm64::bhv::bhvBowlingBall(), &distToBall);
	if(ball != NULL)
	{
		...
		if(s_calc_dangle(o->oMoveAngleYaw, angleToBall) < 0x4000)
		{
...
static void koopa_the_quick_act_race(void)
{
	...
	if(obj_begin_race(FALSE))
	{
		...
		else
		{
			...
			s_chase_angleY(o->oPathedTargetYaw, (s32)(o->oKoopaAgility * 150.0f * FRAME_RATE_SCALER));
			...
			switch(o->oSubAction)
			{
				...
				case KOOPA_THE_QUICK_SUB_ACT_RUN:
					...
					if(o->parentObj->oKoopaRaceEndpointRaceStatus != 0 && o->oDistanceToMario > 1500.0f && (o->oPathedPrevWaypointFlags & WAYPOINT_MASK_00FF) < 28)
					{
...
void bhv_koopa_update(void)
{
	...
	obj_face_yaw_approach(o->oMoveAngleYaw, 0x600 / FRAME_RATE_SCALER_INV);
}

/**
 * Update function for sm64::bhv::bhvKoopaRaceEndpoint().
 */
