...
/**
 * Behavior for sm64::bhv::bhvEnemyLakitu().
 * Lakitu comes before it spawned spinies in processing order.
 * TODO: sm64::bhv::bhvCloud() processing oredr
 */
...
static void enemy_lakitu_update_vel_y(f32 offsetY)
{
	...
	{
		obj_y_vel_approach(4.0f, 0.4f * FRAME_RATE_SCALER);
	}
	else
	{
		obj_y_vel_approach(-4.0f, 0.4f * FRAME_RATE_SCALER);
...
static void enemy_lakitu_update_speed_and_angle(void)
{
	...
	{
		obj_face_yaw_approach(o->oAngleToMario, 0x600 / FRAME_RATE_SCALER_INV);
	}
	...
	s_chase_angleY(o->oAngleToMario, turnSpeed / FRAME_RATE_SCALER_INV);
}
...
static void enemy_lakitu_sub_act_no_spiny(void)
{
	...
	else if(o->oEnemyLakituNumSpinies < 3 && o->oDistanceToMario < 800.0f && s_calc_dangle(o->oAngleToMario, o->oFaceAngleYaw) < 0x4000)
	{
...
static void enemy_lakitu_sub_act_hold_spiny(void)
{
	...
	else if(o->oDistanceToMario > o->oDrawingDistance - 100.0f || (o->oDistanceToMario < 500.0f && s_calc_dangle(o->oAngleToMario, o->oFaceAngleYaw) < 0x2000))
	{
...
/**
 * Update function for sm64::bhv::bhvEnemyLakitu().
 */
