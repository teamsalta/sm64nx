...
{
	o->parentObj = s_find_obj(sm64::bhv::bhvFloorSwitchAnimatesObject());
}
...
void bhv_animates_on_floor_switch_press_loop(void)
{
	if(o->oFloorSwitchPressAnimationUnk100 != 0)
	{
		...
		{
			o->oFloorSwitchPressAnimationUnkF4 = D_80331ACC[o->oBehParams2ndByte] * FRAME_RATE_SCALER_INV;
		}
		else
		{
	...
	if(o->oFloorSwitchPressAnimationUnkF4 != 0)
	{
		...
		{
			objsound_level(SOUND_GENERAL2_SWITCH_TICK_SLOW);
		}
		else
		{
			objsound_level(SOUND_GENERAL2_SWITCH_TICK_FAST);
		}
	...
	o->collisionData = segmented_to_virtual(D_80331A54[o->oBehParams2ndByte][o->oFloorSwitchPressAnimationUnkF8 / 2].unk00);

	s_change_shape(D_80331A54[o->oBehParams2ndByte][o->oFloorSwitchPressAnimationUnkF8 / 2].unk04);
}
