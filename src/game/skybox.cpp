...
const SkyboxTexture* sSkyboxTextures[10] = {
    water_skybox_ptrlist, bitfs_skybox_ptrlist, wdw_skybox_ptrlist, cloud_floor_skybox_ptrlist, ccm_skybox_ptrlist, ssl_skybox_ptrlist, bbh_skybox_ptrlist, bidw_skybox_ptrlist, clouds_skybox_ptrlist, bits_skybox_ptrlist,
};
...
void draw_skybox_tile_grid(Gfx** dlist, s8 background, s8 player, s8 colorIndex)
{
	...
	for(row = 0; row < 3; row++)
	{
		for(col = 0; col < 3; col++)
		{
			...
			const u8* const texture = (*(SkyboxTexture*)segmented_to_virtual(sSkyboxTextures[background]))[tileIndex]; // CHECK CRASH TODO
			Vtx* vertices		= make_skybox_rect(tileIndex, colorIndex);

			gDPSetTextureImage((*dlist)++, G_IM_FMT_RGBA, G_IM_SIZ_16b, 1, texture);
			gDPTileSync((*dlist)++);
			gDPSetTile((*dlist)++, G_IM_FMT_RGBA, G_IM_SIZ_16b, 0, 0, G_TX_LOADTILE, 0, G_TX_WRAP | G_TX_NOMIRROR, G_TX_NOMASK, G_TX_NOLOD, G_TX_WRAP | G_TX_NOMIRROR, G_TX_NOMASK, G_TX_NOLOD);
			gDPLoadSync((*dlist)++);
 ...
 */
Gfx* create_skybox_facing_camera(s8 player, s8 background, f32 fov, f32 posX, f32 posY, f32 posZ, f32 focX, f32 focY, f32 focZ)
{
