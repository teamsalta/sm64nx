 ...
 */
#define ZOOMOUT_AREA_MASK(level1Area1, level1Area2, level1Area3, level1Area4, level2Area1, level2Area2, level2Area3, level2Area4)                                                                                                                              \
	((level2Area4) << 7 | (level2Area3) << 6 | (level2Area2) << 5 | (level2Area1) << 4 | (level1Area4) << 3 | (level1Area3) << 2 | (level1Area2) << 1 | (level1Area1) << 0)
...
struct TransitionInfo
{
	...
	/*0x10*/ s32 framesLeft;
	/*0x14*/ Vec3f marioPos;
	/*0x20*/ u8 pad; // for the structs to align, there has to be an extra size variable here. type is unknown.
};
...
struct CameraFOVStatus
{
	...
	/*0x08*/ f32 fovOffset;
	/// A bool set in fov_default() but size otherwise
...
struct LakituState
{
	...
	/*0x30*/ u8 filler30[12]; // extra size Vec3f?
	...
	/*0x48*/ f32 focusDistance; // size
	/*0x4C*/ s16 oldPitch;	    // size
	/*0x4E*/ s16 oldYaw;	    // size
	/*0x50*/ s16 oldRoll;	    // size
...
extern void Crou_camBlockofs(f32 displacementX, f32 displacementY, f32 displacementZ);
...
s16 next_lakitu_state(Vec3f& newPos, Vec3f& newFoc, const Vec3f curPos, const Vec3f curFoc, const Vec3f oldPos, const Vec3f oldFoc, const s16 yaw);
void set_fixed_cam_axis_sa_lobby(s16 preset);	     // postdefined
...
extern s32 cutscene_event(CameraEvent event, struct Camera* c, const Duration start, const Duration end);
extern s32 cutscene_spawn_obj(s32 phase, Duration frame);
