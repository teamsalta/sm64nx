...
static s32 beh_cmd_call(void)
{
	...
	jumpAddress = (BehaviorScript*)segmented_to_virtual((const BehaviorScript*)gBehCommand[0]);
...
{
	gBehCommand++;
	gBehCommand = (BehaviorScript*)segmented_to_virtual((const BehaviorScript*)gBehCommand[0]);
	return BEH_CONTINUE;
}
...
{
	auto behavior_proc = sm64::hook::bhv::function((BhvCallback)gBehCommand[1]);
...
static s32 beh_cmd_sum_float(void)
{
	...
	cur_object_set_float(objectOffsetDst, cur_object_get_float(objectOffsetSrc1) + cur_object_get_float(objectOffsetSrc2));
...
static s32 beh_cmd_sum_int(void)
{
	...
	cur_object_set_int(objectOffsetDst, cur_object_get_int(objectOffsetSrc1) + cur_object_get_int(objectOffsetSrc2));
...
{
	u32* collisionData	      = (u32*)segmented_to_virtual((void*)gBehCommand[1]);
...
void cur_object_exec_behavior(void)
{
	...
	{
		(void)(gCurrentObject->oTimer = 0, gCurrentObject->oSubAction = 0, gCurrentObject->oPrevAction = gCurrentObject->oAction);
	}
