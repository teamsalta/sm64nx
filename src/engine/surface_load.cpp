...
void alloc_surface_pools(void)
{
	...
	sSurfaceNodePool = (SurfaceNode*)main_pool_alloc(7000 * sizeof(struct SurfaceNode), MEMORY_POOL_LEFT);
...
static void load_object_surfaces(s16** data, s16* vertexData)
{
	...
	if(gCurrentObject->behavior == sm64::bhv::bhvDddWarp())
	{
		room = 5;
	}
	else
...
void stMainMoveBG(void)
{
	...
	if(!(gTimeStopState & TIME_STOP_ACTIVE) && marioDist < tangibleDist && !(gCurrentObject->activeFlags & ACTIVE_FLAG_IN_DIFFERENT_ROOM))
	{
		collisionData++;
	...
	if(marioDist < gCurrentObject->oDrawingDistance || sm64::config().camera().disableDistanceClip())
	{
