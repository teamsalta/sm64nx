 ...
 */
void mtxf_align_terrain_normal(Mat4& dest, const Vec3f up, const Vec3f pos, const s16 yaw)
{
	Vec3f lateralDir;
	Vec3f leftDir;
	Vec3f forwardDir;
 ...
 */
s32 approach_s32(const s32 value, const s32 target, const s32 inc, const s32 dec)
{
	f32 current = value;
 ...
 * most 'inc' and going down at most 'dec'.
 */
f32 approach_f32(const f32 value, const f32 target, const f32 inc, const f32 dec)
{
	f32 current = value;
...
s32 anim_spline_poll(Vec3f& result)
{
	...
	if((gSplineKeyframeFraction += gSplineKeyframe[0][0] / (1000.0f / FRAME_RATE_SCALER)) >= 1)
	{
		gSplineKeyframe++;
