...
static uint8_t rgba32_buf[2048 * 2048 * 4];
...
namespace sm64::gfx
{
	...
	template<class WND, class RND>
	class Fast64T : public Fast64
	{
	public:
		Fast64T() : Fast64()
		{
		...
		static void generate_cc(struct ColorCombiner *comb, uint32_t cc_id)
		{
			uint8_t c[2][4];
		...
		static const ColorCombiner* gfx_lookup_or_create_color_combiner(uint32_t cc_id)
		{
		...
		static bool gfx_texture_cache_lookup(int tile, struct TextureNode **n, const uint8_t *orig_addr, uint32_t fmt, uint32_t siz)
		{
		...
		//static inline void gfx_sp_matrix(uint8_t parameters, const Matrix matrix)
		...
		{
			return x * gfx_current_dimensions.adjust_ratio;
		}
		...
		static VertexState vertexState;
		...
		static void gfx_sp_vertex(size_t n_vertices, size_t dest_index, const Vtx *vertices)
		{
			for(size_t i = 0; i < n_vertices; i++, dest_index++)
			{
				...
				float x = (v.ob[0] * rsp.MP_matrix[0][0] + v.ob[1] * rsp.MP_matrix[1][0] + v.ob[2] * rsp.MP_matrix[2][0] + rsp.MP_matrix[3][0]) * gfx_current_dimensions.adjust_ratio;
				...
				d.clip_rej = 0;
				if(x < -w) d.clip_rej |= 1;
				else if(x > w) d.clip_rej |= 2;

				if(y < -w) d.clip_rej |= 4;
				else if(y > w) d.clip_rej |= 8;

				if(z < -w) d.clip_rej |= 16;
				else if(z > w) d.clip_rej |= 32;
		...
		static void gfx_sp_tri1(u32 vtx1_idx, u32 vtx2_idx, u32 vtx3_idx)
		{
			...
			const ColorCombiner* comb = gfx_lookup_or_create_color_combiner(cc_id);
		...
		static void gfx_sp_tri_buffer(const TriIndex* triIndex, u32 count)
		{
			uint32_t cc_id = rdp.combine_mode;
		...
		static void dp_load_tile(uint8_t tile, uint32_t uls, uint32_t ult, uint32_t lrs, uint32_t lrt)
		{
			if(tile == 1) return;
			SUPPORT_CHECK(tile == G_TX_LOADTILE);
			SUPPORT_CHECK(uls == 0);
		...
		static void dp_set_combine_mode(uint32_t rgb, uint32_t alpha)
		{
			rdp.combine_mode = rgb | (alpha << 12);
		}

		static void dp_set_env_color(uint8_t r, uint8_t g, uint8_t b, uint8_t a)
		{
			rdp.env_color.r = r;
			rdp.env_color.g = g;
			rdp.env_color.b = b;
		...
		static void dp_set_prim_color(uint8_t r, uint8_t g, uint8_t b, uint8_t a)
		{
			rdp.prim_color.r = r;
			rdp.prim_color.g = g;
			rdp.prim_color.b = b;
		...
		static void set_fog_color(uint8_t r, uint8_t g, uint8_t b, uint8_t a)
		{
			rdp.fog_color.r = r;
			rdp.fog_color.g = g;
			rdp.fog_color.b = b;
		...
		static void dp_set_fill_color(uint32_t packed_color)
		{
		...
		static void draw_rectangle(int32_t ulx, int32_t uly, int32_t lrx, int32_t lry)
		{
		...
		static void dp_texture_rectangle(int32_t ulx, int32_t uly, int32_t lrx, int32_t lry, uint8_t tile, int16_t uls, int16_t ult, int16_t dsdx, int16_t dtdy, bool flip)
		{
		...
		static void dp_fill_rectangle(int32_t ulx, int32_t uly, int32_t lrx, int32_t lry)
		{
		...
		static void gfx_dp_set_color_image(uint32_t format, uint32_t size, uint32_t width, const void* address)
		{
...
#define G_VTX_BUF_PARAM	0x0B
		...
		static void run_dl(Gfx* cmd)
		{
			...
			for(;;)
			{
				...
				switch(opcode)
				{
					...
					case G_VTX:
#ifdef F3DEX_GBI_2
						gfx_sp_vertex(C0(12, 8), C0(1, 7) - C0(12, 8), (const Vtx*)cmd->words.w1);
#elif defined(F3DEX_GBI) || defined(F3DLP_GBI)
					...
					case G_MTX:
#ifdef F3DEX_GBI_2
						gfx_sp_matrix(C0(0, 8) ^ G_MTX_PUSH, (const int32_t*)(cmd->words.w1));
#else
					...
					case G_VTX_BUF:
#ifdef F3DEX_GBI_2
						gfx_sp_vertex(g_vtx_buf_size, g_vtx_buf_dest, (const Vtx*)cmd->words.w1);
#elif defined(F3DEX_GBI) || defined(F3DLP_GBI)
						gfx_sp_vertex(C0(10, 6), C0(16, 8) / 2, seg_addr(cmd->words.w1));
#else
						gfx_sp_vertex((C0(0, 16)) / sizeof(Vtx), C0(16, 4), seg_addr(cmd->words.w1));
					...
					case G_SETPRIMCOLOR:
						...
						break;
					case G_SETFOGCOLOR:
						set_fog_color(C1(24, 8), C1(16, 8), C1(8, 8), C1(0, 8));
						break;
					case G_SETFILLCOLOR:
...
#else
						dp_fill_rectangle(C1(12, 12), C1(0, 12), C0(12, 12), C0(0, 12));
						break;
#endif
					case G_SETSCISSOR:
		...
		void run(Gfx *commands) override
		{
			gfx_sp_reset();
