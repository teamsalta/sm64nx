...
namespace sm64::gfx
{
	...
	void init()
	{
		gfx_wapi = new platform::Sdl("SM64", sm64::config().game().fullscreen());
	...
	static void gfx_sp_vertex(size_t n_vertices, size_t dest_index, const Vtx* vertices, bool transform = false)
	{
		...
		for (size_t i = 0; i < n_vertices; i++, dest_index++)
		{
			...
			if (transform)
			{
				...
				else if (d.x > d.w) d.clip_rej |= 2;

				if (d.y < -d.w) d.clip_rej |= 4;
				else if (d.y > d.w) d.clip_rej |= 8;

				if (d.z < -d.w) d.clip_rej |= 16;
				else if (d.z > d.w) d.clip_rej |= 32;
			}
			else
			{
				d.x = vertex.ob[0];
