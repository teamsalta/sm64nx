...
#define aSetLoadBufferPair(pkt, c, off)                                                                                                                                                                                                                        \
	...
	aLoadBuffer(pkt, (u16*)VIRTUAL_TO_PHYSICAL2(&gSynthesisReverb.ringBuffer.left[off]));                                                                                                                                                                  \
	aSetBuffer(pkt, 0, c + DMEM_ADDR_WET_RIGHT_CH, 0, DEFAULT_LEN_1CH - c);                                                                                                                                                                                \
	aLoadBuffer(pkt, (u16*)VIRTUAL_TO_PHYSICAL2(&gSynthesisReverb.ringBuffer.right[off]));

#define aSetSaveBufferPair(pkt, c, d, off)                                                                                                                                                                                                                     \
	aSetBuffer(pkt, 0, 0, c + DMEM_ADDR_WET_LEFT_CH, d);                                                                                                                                                                                                   \
	aSaveBuffer(pkt, (u16*)VIRTUAL_TO_PHYSICAL2(&gSynthesisReverb.ringBuffer.left[off]));                                                                                                                                                                  \
	aSetBuffer(pkt, 0, 0, c + DMEM_ADDR_WET_RIGHT_CH, d);                                                                                                                                                                                                  \
	aSaveBuffer(pkt, (u16*)VIRTUAL_TO_PHYSICAL2(&gSynthesisReverb.ringBuffer.right[off]));
...
void prepare_reverb_ring_buffer(s32 chunkLen, u32 updateIndex)
{
	...
	if(gReverbDownsampleRate != 1)
	{
		if(gSynthesisReverb.framesLeftToIgnore == 0)
		{
			...
			for(srcPos = 0, dstPos = 0; dstPos < item->lengths[0] / 2; srcPos += gReverbDownsampleRate, dstPos++)
			{
	...
	else
	{
		...
		excessiveSamples		   = (nSamples + gSynthesisReverb.nextRingBufferPos) - gSynthesisReverb.bufSizePerChannel;
...
u64* synthesis_do_one_audio_update(u16* aiBuf, s32 bufLen, u64* cmd, u32 updateIndex)
{
	...
	else
	{
		...
		else
		{
			...
			aResample(cmd++, gSynthesisReverb.resampleFlags, (u16)gSynthesisReverb.resampleRate, (u16*)VIRTUAL_TO_PHYSICAL2(gSynthesisReverb.resampleStateLeft));
			aSetBuffer(cmd++, 0, t4 + DMEM_ADDR_WET_RIGHT_CH, DMEM_ADDR_RIGHT_CH, bufLen << 1);
			aResample(cmd++, gSynthesisReverb.resampleFlags, (u16)gSynthesisReverb.resampleRate, (u16*)VIRTUAL_TO_PHYSICAL2(gSynthesisReverb.resampleStateRight));
			aSetBuffer(cmd++, 0, 0, 0, DEFAULT_LEN_2CH);
			aMix(
			    cmd++, 0, /*gain*/ 0x8000 + gSynthesisReverb.reverbGain, /*in*/ DMEM_ADDR_LEFT_CH,
		...
		else
		{
			...
			aSaveBuffer(cmd++, (u16*)VIRTUAL_TO_PHYSICAL2(gSynthesisReverb.items[gSynthesisReverb.curFrame][updateIndex].toDownsampleLeft));
...
u64* synthesis_process_notes(u16* aiBuf, s32 bufLen, u64* cmd)
{
	...
	for(noteIndex = 0, curLoadedBook = NULL; noteIndex < gMaxSimultaneousNotes; noteIndex++)
	{
		...
		else if(note->enabled)
		{
			...
			else
			{
				...
				for(curPart = 0; curPart < nParts; curPart++)
				{
					...
					{ // nParts == 1
						if(1)
						{ // shouldn't be here, but it makes things line up better...
					...
					if(curLoadedBook != audioBookSample->book->book)
					{
						...
						aLoadADPCM(cmd++, nEntries * 16, (u16*)VIRTUAL_TO_PHYSICAL2(curLoadedBook));
					}
					...
					while(nAdpcmSamplesProcessed != samplesLenAdjusted)
					{
						...
						if(t0 != 0)
						{
							...
							v0_2 = (u8*)dma_sample_data((uintptr_t)(sampleAddr + (note->samplePosInt - s2 + 0x10) / 16 * 9), t0 * 9, flags, &note->sampleDmaIndex);
							a3   = (u32)((uintptr_t)v0_2 & 0xf);
							aSetBuffer(cmd++, 0, DMEM_ADDR_COMPRESSED_ADPCM_DATA, 0, t0 * 9 + a3);
							aLoadBuffer(cmd++, (u16*)VIRTUAL_TO_PHYSICAL2(v0_2 - a3));
						}
						else
						{
							s0 = 0;
						...
						{
							aSetLoop(cmd++, (u16*)VIRTUAL_TO_PHYSICAL2(audioBookSample->loop->state));
							flags	      = A_LOOP; // = 2
							note->restart = FALSE;
						}
						...
						if(nAdpcmSamplesProcessed == 0)
						{
							...
							aADPCMdec(cmd++, flags, (u16*)VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->adpcmdecState));
							sp130 = s2 * 2;
						}
						else
						{
							...
							aADPCMdec(cmd++, flags, (u16*)VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->adpcmdecState));
							aDMEMMove(cmd++, DMEM_ADDR_UNCOMPRESSED_NOTE + ALIGN(s5, 5) + (s2 * 2), DMEM_ADDR_UNCOMPRESSED_NOTE + s5, (s0 + s6_2 - s3) * 2);
						}
						...
						if(noteFinished)
						{
							aClearBuffer(cmd++, DMEM_ADDR_UNCOMPRESSED_NOTE + s5, (samplesLenAdjusted - nAdpcmSamplesProcessed) * 2);
							note->samplePosInt = 0;
							note->finished	   = 1;
							note->enabled	   = 0;
						...
						case 2:
							switch(curPart)
							{
								case 0:
									...
									aResample(cmd++, A_INIT, 0xff60, (u16*)VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->dummyResampleState));
									resampledTempLen		    = samplesLenAdjusted + 4;
									noteSamplesDmemAddrBeforeResampling = DMEM_ADDR_ADPCM_RESAMPLED + 4;
									if(note->finished != 0)
									{
										aClearBuffer(cmd++, DMEM_ADDR_ADPCM_RESAMPLED + samplesLenAdjusted + 4, samplesLenAdjusted + 0x10);
									}
									break;

								case 1:
									aSetBuffer(cmd++, 0, DMEM_ADDR_UNCOMPRESSED_NOTE + sp130, DMEM_ADDR_ADPCM_RESAMPLED2, samplesLenAdjusted + 8);
									aResample(cmd++, A_INIT, 0xff60, (u16*)VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->dummyResampleState));
									aDMEMMove(cmd++, DMEM_ADDR_ADPCM_RESAMPLED2 + 4, DMEM_ADDR_ADPCM_RESAMPLED + resampledTempLen, samplesLenAdjusted + 4);
									break;
							}
					}
		...
		out:
			...
			cmd = final_resample(cmd, note, bufLen * 2, resamplingRateFixedPoint, noteSamplesDmemAddrBeforeResampling, flags);
	...
	t9 *= 2;
	aSetBuffer(cmd++, 0, 0, DMEM_ADDR_TEMP, t9);
	aSaveBuffer(cmd++, (u16*)VIRTUAL_TO_PHYSICAL2(aiBuf));
	return cmd;
}
...
u64* load_wave_samples(u64* cmd, struct Note* note, s32 nSamplesToLoad)
{
	...
	s32 i;
	aSetBuffer(
	    cmd++, /*flags*/ 0, /*dmemin*/ DMEM_ADDR_UNCOMPRESSED_NOTE, /*dmemout*/ 0,
	    /*count*/ sizeof(note->synthesisBuffers->samples)); // interesting that it's 128...
	aLoadBuffer(cmd++, (u16*)VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->samples));
...
u64* final_resample(u64* cmd, struct Note* note, s32 count, u16 pitch, u16 dmemIn, u32 flags)
{
	...
	aResample(cmd++, flags, pitch, (u16*)VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->finalResampleState));
	return cmd;
}
...
u64* process_envelope_inner(u64* cmd, struct Note* note, s32 nSamples, u16 inBuf, s32 headsetPanSettings, struct VolumeChange* vol)
{
	...
	else
	{
		...
		else if(note->stereoStrongLeft)
		{
			aClearBuffer(cmd++, DMEM_ADDR_STEREO_STRONG_TEMP_DRY, DEFAULT_LEN_2CH);
			aSetBuffer(cmd++, 0, inBuf, DMEM_ADDR_LEFT_CH, nSamples * 2);
			aSetBuffer(cmd++, A_AUX, DMEM_ADDR_STEREO_STRONG_TEMP_DRY, DMEM_ADDR_WET_LEFT_CH, DMEM_ADDR_STEREO_STRONG_TEMP_WET);
		}
		else
		{
			aSetBuffer(cmd++, 0, inBuf, DMEM_ADDR_LEFT_CH, nSamples * 2);
	...
	if(vol->sourceLeft == vol->targetLeft && vol->sourceRight == vol->targetRight && !note->envMixerNeedsInit)
	{
	...
	{
		aEnvMixer(cmd++, mixerFlags | A_AUX, (u16*)VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->mixEnvelopeState));
		if(note->stereoStrongRight)
		{
			aSetBuffer(cmd++, 0, 0, 0, nSamples * 2);
			// 0x8000 is -100%, so subtract sound instead of adding...
			aMix(
			    cmd++, 0, /*gain*/ 0x8000, /*in*/ DMEM_ADDR_STEREO_STRONG_TEMP_DRY,
			    /*out*/ DMEM_ADDR_LEFT_CH);
			aMix(
			    cmd++, 0, /*gain*/ 0x8000, /*in*/ DMEM_ADDR_STEREO_STRONG_TEMP_WET,
			    /*out*/ DMEM_ADDR_WET_LEFT_CH);
		}
		else if(note->stereoStrongLeft)
		{
	...
	{
		aEnvMixer(cmd++, mixerFlags, (u16*)VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->mixEnvelopeState));
		if(note->stereoStrongRight)
		{
			aSetBuffer(cmd++, 0, 0, 0, nSamples * 2);
			aMix(
...
u64* note_apply_headset_pan_effects(u64* cmd, struct Note* note, s32 bufLen, s32 flags, s32 leftRight)
{
	...
	if(flags != 1) // A_INIT?
	{
		...
		if(prevPanShift == 0)
		{
			...
			aSaveBuffer(cmd++, (u16*)VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->panResampleState));
			...
			aResample(cmd++, 0, pitch, (u16*)VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->panResampleState));
		}
		else
		{
			pitch = (panShift == 0) ? (bufLen << 0xf) / (bufLen - prevPanShift - 4) : (bufLen << 0xf) / (bufLen + panShift - prevPanShift);
		...
		if(prevPanShift != 0)
		{
			...
			aLoadBuffer(cmd++, (u16*)VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->panSamplesBuffer));
	...
	if(panShift)
	{
		...
		aSaveBuffer(cmd++, (u16*)VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->panSamplesBuffer));
	}
