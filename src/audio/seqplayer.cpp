...
s32 seq_channel_set_layer(struct SequenceChannel* seqChannel, s32 layerIndex)
{
	...
	{
		layer			       = (SequenceChannelLayer*)audio_list_pop_back(&gLayerFreeList);
...
void seq_channel_layer_process_script(struct SequenceChannelLayer* layer)
{
	...
	for(;;)
	{
		...
		switch(cmd)
		{
			...
			case 0xc6: // layer_setinstr
				...
				adsr = &layer->adsr;
				if(((uintptr_t)gBankLoadedPool.persistent.pool.start <= (uintptr_t)inst && (uintptr_t)inst <= (uintptr_t)(gBankLoadedPool.persistent.pool.start + gBankLoadedPool.persistent.pool.size)) ||
				   ((uintptr_t)gBankLoadedPool.temporary.pool.start <= (uintptr_t)inst && (uintptr_t)inst <= (uintptr_t)(gBankLoadedPool.temporary.pool.start + gBankLoadedPool.temporary.pool.size)))
				{
			...
			case 0xc7: // layer_portamento
				...
				old		       = state->pc++;
				portamentoTargetNote   = *old + seqChannel->transposition + layer->transposition + seqPlayer->transposition;
	...
	else
	{
		...
		else
		{
			...
			{ // instrument
				semitone = cmdSemitone + seqPlayer->transposition + seqChannel->transposition + layer->transposition;
				if(semitone >= 0x80)
				{
					layer->stopSomething = TRUE;
				}
				else
				{
					...
					if(layer->portamento.mode != 0)
					{
						...
						if(PORTAMENTO_IS_SPECIAL(layer->portamento))
						{
							portamento->speed = US_FLOAT(32512.0) * FLOAT_CAST(seqPlayer->tempo) / ((f32)layer->delay * (f32)gTempoInternalToExternal * FLOAT_CAST(layer->portamentoTime));
						}
						else
						{
