...
void note_pool_fill(struct NotePool* pool, s32 count)
{
	...
	for(i = 0, j = 0; j < count; i++)
	{
		...
		while(j < count)
		{
			note = (Note*)audio_list_pop_back(source);
			if(note == NULL)
			{
				break;
			}
...
{
	struct Note* note = (Note*)audio_list_pop_back(&pool->disabled);
	if(note != NULL)
	{
...
{
	struct Note* note = (Note*)audio_list_pop_back(&pool->decaying);
	if(note != NULL)
	{
...
struct Note* alloc_note(struct SequenceChannelLayer* seqLayer)
{
	...
	{
		if(!(ret = alloc_note_from_disabled(&seqLayer->seqChannel->notePool, seqLayer)) && !(ret = alloc_note_from_decaying(&seqLayer->seqChannel->notePool, seqLayer)) && !(ret = alloc_note_from_active(&seqLayer->seqChannel->notePool, seqLayer)))
		{
			seqLayer->status = SOUND_LOAD_STATUS_NOT_LOADED;
			return NULL;
		}
	...
	{
		if(!(ret = alloc_note_from_disabled(&seqLayer->seqChannel->notePool, seqLayer)) && !(ret = alloc_note_from_disabled(&seqLayer->seqChannel->seqPlayer->notePool, seqLayer)) &&
		   !(ret = alloc_note_from_decaying(&seqLayer->seqChannel->notePool, seqLayer)) && !(ret = alloc_note_from_decaying(&seqLayer->seqChannel->seqPlayer->notePool, seqLayer)) &&
		   !(ret = alloc_note_from_active(&seqLayer->seqChannel->notePool, seqLayer)) && !(ret = alloc_note_from_active(&seqLayer->seqChannel->seqPlayer->notePool, seqLayer)))
		{
			seqLayer->status = SOUND_LOAD_STATUS_NOT_LOADED;
			return NULL;
		}
	...
	{
		if(!(ret = alloc_note_from_disabled(&gNoteFreeLists, seqLayer)) && !(ret = alloc_note_from_decaying(&gNoteFreeLists, seqLayer)) && !(ret = alloc_note_from_active(&gNoteFreeLists, seqLayer)))
		{
			seqLayer->status = SOUND_LOAD_STATUS_NOT_LOADED;
			return NULL;
		}
	...
	if(!(ret = alloc_note_from_disabled(&seqLayer->seqChannel->notePool, seqLayer)) && !(ret = alloc_note_from_disabled(&seqLayer->seqChannel->seqPlayer->notePool, seqLayer)) && !(ret = alloc_note_from_disabled(&gNoteFreeLists, seqLayer)) &&
	   !(ret = alloc_note_from_decaying(&seqLayer->seqChannel->notePool, seqLayer)) && !(ret = alloc_note_from_decaying(&seqLayer->seqChannel->seqPlayer->notePool, seqLayer)) && !(ret = alloc_note_from_decaying(&gNoteFreeLists, seqLayer)) &&
	   !(ret = alloc_note_from_active(&seqLayer->seqChannel->notePool, seqLayer)) && !(ret = alloc_note_from_active(&seqLayer->seqChannel->seqPlayer->notePool, seqLayer)) && !(ret = alloc_note_from_active(&gNoteFreeLists, seqLayer)))
	{
		seqLayer->status = SOUND_LOAD_STATUS_NOT_LOADED;
		return NULL;
	}
...
void reclaim_notes(void)
{
	...
	for(i = 0; i < gMaxSimultaneousNotes; i++)
	{
		...
		if(note->parentLayer != NO_LAYER)
		{
			...
			{
				if(note->parentLayer->seqChannel->muteBehavior & (MUTE_BEHAVIOR_STOP_SCRIPT | MUTE_BEHAVIOR_STOP_NOTES))
				{
					cond = TRUE;
				}
			}
...
void note_init_all(void)
{
	...
	for(i = 0; i < gMaxSimultaneousNotes; i++)
	{
		...
		note->synthesisBuffers	    = (NoteSynthesisBuffers*)soundAlloc(&gNotesAndBuffersPool, sizeof(struct NoteSynthesisBuffers));
	}
}
