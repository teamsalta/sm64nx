...
static u8* soundDataADSR()
{
	static u8* p = load(sm64::hook::sound::Id::gSoundDataADSR);
	return p;
}

static u8* soundDataRaw()
{
	static u8* p = load(sm64::hook::sound::Id::gSoundDataRaw);
	return p;
}

static u8* musicData()
{
	static u8* p = load(sm64::hook::sound::Id::gMusicData);
	return p;
}

static u8* bankSetsData()
{
	static u8* p = load(sm64::hook::sound::Id::gBankSetsData);
	return p;
}
...
#ifdef NON_MATCHING
void initSound(uintptr_t memBase, uintptr_t offsetBase, struct AudioBankSound* sound)
{
	struct AudioBankSample** itSample = &sound->sample;
	if((*itSample) != 0)
	{
		volatile struct AudioBankSample *sample, *sample2;
		*itSample = (AudioBankSample*)(memBase + (uintptr_t)(*itSample));
		sample	  = *itSample;
		sample2	  = *itSample;
		...
		if(sample2->loaded == FALSE)
		{
			...
			void* b		   = sample->loop;
			void* c		   = sample->book;
			sample->sampleAddr = (u8*)(offsetBase + (uintptr_t)a);
			sample->loop	   = (AdpcmLoop*)(memBase + (uintptr_t)b);
			sample->book	   = (AdpcmBook*)(memBase + (uintptr_t)c);
			sample->loaded	   = TRUE;
		}
	}
}
...
#define INIT_SOUND(sound)                                                                                                                                                                                                                                      \
				...
				for(i = 0; i < numDrums; i++)
				{
					...
					if(h != 0)
					{
						...
						if(drum->loaded == FALSE)
						{
							...
							d		= drum->envelope;
							drum2->loaded	= TRUE;
							drum2->envelope = (AdsrEnvelope*)(memBase + (uintptr_t)d);
						}
					}
#endif
				}
	...
	if((numInstruments >= 1) && (numInstruments >= 1) != 0)
	{
		...
		do
		{
			if(*itInstrs)
			{
				*itInstrs  = (Instrument*)(memBase + (uintptr_t)*itInstrs);
				instrument = *itInstrs;
				...
				if(instrument->loaded == FALSE)
				{
					...
					initSound(memBase, offsetBase, &instrument->lowNotesSound);
					initSound(memBase, offsetBase, &instrument->normalNotesSound);
					initSound(memBase, offsetBase, &instrument->highNotesSound);

					instrument->loaded   = TRUE;
					instrument->envelope = (AdsrEnvelope*)(memBase + (uintptr_t)instrument->envelope);
				}
			}
			itInstrs++;
...
struct AudioBank* bank_load_immediate(s32 bankId, s32 arg1)
{
	...
	ret	= (AudioBank*)alloc_bank_or_seq(&gBankLoadedPool, 1, alloc, arg1, bankId);
	if(ret == NULL)
	{
		return NULL;
	}
...
struct AudioBank* bank_load_async(s32 bankId, s32 arg1, struct SequencePlayer* seqPlayer)
{
	...
	audio_dma_partial_copy_async(&seqPlayer->bankDmaCurrDevAddr, &seqPlayer->bankDmaCurrMemAddr, &seqPlayer->bankDmaRemaining, mesgQueue, &seqPlayer->bankDmaIoMesg);
...
void audio_init()
{
	...
	for(i = 0; i < NUMAIBUFFERS; i++)
	{
		gAiBuffers[i] = (u16*)soundAlloc(&gAudioInitPool, 0xa00);
	...
	size	     = ALIGN16(size);
	gCtlEntries  = (CtlEntry*)soundAlloc(&gAudioInitPool, gAlCtlHeader->seqCount * sizeof(struct CtlEntry));
	...
	audio_dma_copy_immediate((uintptr_t)bankSetsData(), gAlBankSets, 0x100);
